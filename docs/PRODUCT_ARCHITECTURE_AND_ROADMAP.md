...
- CS-MAP.2 read-only Staff Structure Map + Mapping Health completed at `/staff/structure/`: permission-protected read-only staff page rendering the active `ChurchStructureUnit` hierarchy with bilingual names, counts-only membership/mapping context, and setup-readiness indicators; no write actions, no schema/migration changes, no runtime visibility changes, no setup/edit UI. Django Admin remains the structure write surface.
- The next candidate product implementation slice after CS-MAP.2 is SE-AS.4 (ServiceEvent audience runtime visibility rule with legacy fallback), which still requires its own separate approval per `docs/SERVICE_EVENT_AUDIENCE_RUNTIME_MIGRATION_PLAN.md`. SE-AS.5, Community Activities, and CS-SETUP.1 are not pulled forward by CS-MAP.2 completion.
...
- Later SE-AS.4 runtime visibility rule, SE-AS.5 staff audience UI selector, audience filtering, and ServiceEvent visibility/consumer migration only after separate approval (numbering per `docs/SERVICE_EVENT_AUDIENCE_RUNTIME_MIGRATION_PLAN.md`).

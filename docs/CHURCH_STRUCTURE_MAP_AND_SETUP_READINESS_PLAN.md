...
- `ChurchStructureMembership` (`accounts/models.py`): belonging foundation with requested/active/ended/rejected/cancelled lifecycle, one-active-primary-per-user enforcement, date windows, approval audit fields, and `active_for_user()` / `current_primary_for_user()` helpers. It is the runtime source for ServiceEvent audience-row matching after CS-CORE.2B-A. It is not the runtime source for Bible Study visibility/generation, reading/progress/privacy, permissions, roles, My Serving, or legacy-fallback ServiceEvents. Requested/rejected/ended/cancelled rows grant nothing.
...
Ordinary-user matching still depends on legacy `Profile.small_group` for Bible Study member visibility and ServiceEvent zero-row legacy fallback. On top of that: ServiceEvent now uses `ServiceEventAudienceScope` as the event-level audience source when an event has rows and matches those rows by active primary `ChurchStructureMembership`; Bible Study Schedule uses structure audience rows plus legacy mapping resolution for meeting-generation eligibility, with ordinary member visibility still resolved through `Profile.small_group`.
...
| Legacy `Profile.small_group` | `Profile.small_group` | Current runtime belonging source for Bible Study member visibility, reading/progress/privacy consumers, and ServiceEvent zero-row legacy fallback; no longer grants ServiceEvent structure-audience row visibility after CS-CORE.2B-A. |
...
- No runtime visibility change was added by CS-SETUP.1A/1B itself. Later CS-CORE.2B-A changed only ServiceEvent audience-row matching; study/reading visibility and ServiceEvent zero-row fallback remain untouched here.
- No automatic unit↔legacy sync (one-way or two-way).
- No new membership-driven visibility in this setup/edit slice; `ChurchStructureMembership` became the ServiceEvent audience-row matching source only later in CS-CORE.2B-A.

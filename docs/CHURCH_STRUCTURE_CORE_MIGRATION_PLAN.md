...
Status: CS-CORE.0A is complete with this document. CS-CORE.0B.1, CS-CORE.1A, CS-CORE.1B, CS-CORE.1C, CS-CORE.2B-A, and CS-CORE.2C-A are complete as narrow, separately approved slices. CS-CORE.2B-A is the first consumer source switch: ServiceEvent structure-audience matching (events with `ServiceEventAudienceScope` rows) now uses the active primary `ChurchStructureMembership` as its runtime belonging source; ServiceEvents with zero audience rows still use legacy fallback and `Profile.small_group`. It also adds membership-core selector helpers, a preserved legacy comparison matcher, and a read-only `--fail-on-drift` guard on `audit_structure_belonging`. This is one consumer switch, not full legacy retirement: Bible Study meeting generation and member visibility, reading progress, reflection/comment privacy, group progress permissions, TeamAssignment / My Serving, and roles are unchanged, and legacy `Profile.small_group`, `SmallGroup`, `District`, and `MinistryContext` are not retired. CS-CORE.2C-A is a Bible Study shadow/readiness audit only: it adds read-only comparison helpers (`studies/structure_readiness.py`) and the `audit_bible_study_membership_readiness` command, which compare current legacy Bible Study member visibility against the future membership-core candidate; it does not switch Bible Study runtime visibility — Bible Study still uses legacy `SmallGroup` meeting generation and `Profile.small_group` member visibility, and a future separately approved milestone may switch Bible Study visibility only after clean audit results. Later CS-CORE milestones still require their own separate approval.
...
- **CS-CORE.2C-A — Bible Study membership-core readiness / shadow audit.** Complete. Read-only shadow layer only; no runtime visibility switch. `studies/structure_readiness.py` adds `get_small_group_structure_unit`, `user_matches_bible_study_meeting_legacy` (delegates to `BibleStudyMeeting.can_be_seen_by` so it cannot drift from runtime), `user_matches_bible_study_meeting_membership_core` (future candidate: the meeting small group's mapped `ChurchStructureUnit` matched by the single active primary `ChurchStructureMembership`, with the same descendant semantics as ServiceEvent matching; `Profile.small_group` alone grants nothing; requested/rejected/cancelled/ended/future/expired memberships grant nothing; multiple active primaries and unmapped meeting groups fail closed), and `compare_bible_study_meeting_visibility` (`same_visible` / `same_hidden` / `would_gain` / `would_lose` plus reason codes). The `audit_bible_study_membership_readiness` command (no `--apply`, writes nothing) reports those pair classifications over upcoming member-visible meetings (`--include-past` widens the window) plus readiness categories (`meeting_unmapped_small_group`, `user_group_without_active_primary_membership`, `user_active_primary_without_profile_group`, `user_profile_membership_mismatch`, `user_profile_group_unmapped`, `multiple_active_primary_memberships`) with `--verbose`/`--limit` detail and a read-only `--fail-on-drift` guard. Bible Study runtime behavior is unchanged: meeting generation still resolves to legacy `SmallGroup` rows and member visibility still uses `Profile.small_group`; legacy `BibleStudySession` is documented out of scope as a retirement target. A future separately approved milestone may switch Bible Study visibility only after clean audit results.

...
`NOTIFY.1F` (notification app/model/admin/Core delivery-port foundation,
recipient-scoped center/bell UI, the ministry-owned explicit ServiceEvent
...
prepared closure docs/checklist plus a missing focused regression test, and
`CHURCH-CALENDAR.2A` adds the `ministry`-owned read-only personal `my_serving`
overlay of the viewer's own explicit team-assignment serving, and
`CHURCH-CALENDAR.2A-FU4` groups the base `ServiceEvent` row and the viewer's own
serving rows for that same event into one presentation occurrence (shared
`occurrence_key`), so a served event is not duplicated on the calendar. FU4
...
Calendar V1 limited-trial/current-state use; `CHURCH-CALENDAR.2B-QA-CLOSURE`
records the product-owner manual QA pass for `CHURCH-CALENDAR.2B` Bible Study
serving, which also confirmed `CHURCH-CALENDAR.2A-FU4` ServiceEvent serving
...
attention consumers use that effective set; Team Schedule and Sunday Board
...
scheduling-revision primitives; ministry consumes those facts for exact
destination-team membership review, assignment-state classification, and its
owned `TeamAssignment`/`TeamAssignmentMember` mutation. No reverse dependency
or parser ownership move is added. The adapter uses canonical
`ServiceEvent.service_profile` FK/Profile identity and exact active
`TeamMembership` identity on the configured active assignable team, and reads
no other assignment column. 1A's staff/superuser-only upload, mapping, and
...
| `community_events` | `community_events` | Community Activities / 活动 | Independent browse/detail, signup/cancel with an optional participant limit, complete validated member drafts, member submission, user-linked co-organizers with bounded pre-publication edit permission, a lightweight staff review inbox + request-changes loop, and a low-noise Today provider for signed-up activities happening today plus creator `changes_requested` reminders. `NOTIFY.1E` adds only primary-creator review outcomes for applied staff request-changes, publish, and cancel/reject transitions through the Core port. Drafts are creator/co-organizer/staff preparation only; published visibility uses app-owned audience rows + active primary membership, and zero rows fail closed. Draft, signup, capacity, co-organizer permission, and notifications are not serving. No larger approval dashboard, My Serving, serving action-center contribution, waitlist, attendee list, check-in, Staff Overview, setup/readiness, broader Community Activity notification fanout/reminders, or `ServiceEvent` link. |
...
| `notifications` | `notifications` | Notifications / 通知 | Registered and default-enabled gateable module owning directed Notification persistence and recipient-scoped read-state UI. Source modules resolve recipients through the Core delivery port and have no hard dependency on Notifications. `NOTIFY.1B` adds authenticated `/notifications/`, POST-only mark-read actions, and a notifications-owned bilingual utility bell/unread count; `NOTIFY.1F` adds recipient-scoped POST Open/read behavior, 25 newest-first rows per page, and retain-for-now with no cleanup command; `NOTIFY.1C`/`NOTIFY.1D`/`NOTIFY.1E` are the only approved ministry, studies, and Community Activities producers. Audience/belonging/manager/staff inference is forbidden. It is not an ordinary primary-nav entry and contributes no Today, setup/readiness, or Staff Overview surface. Stored targets remain permission-neutral and source-owned permissions remain authoritative. |
...
hidden and hidden detail returns 404. `ANNOUNCEMENTS.1C` adds the bounded
staff/superuser management list, atomic draft create/edit with active
structure-unit audience rows, and POST-only publish/archive actions. Publishing
requires a valid window and at least one active audience row; archiving preserves
the record and audience rows. The module declares `contributes_nav`,
`contributes_today`, and `requires_structure_core`.
`ANNOUNCEMENTS.1D-SLIM` adds its explicitly registered module-owned Today
provider and safe `None` default. When enabled, it reuses member/public
visibility, filters to `important`, and selects at most one row by newest
`publish_start` for a localized title/detail link only. When disabled, Today
...
* `MODULAR-CORE.3B` moves the original Today provider bodies into their owning
  ...
  site. `ANNOUNCEMENTS.1D-SLIM` later adds
  `announcements/today_provider.py` at that same explicit site; it contributes
  only the newest visible active important announcement, capped at one, with
  a safe `None` default when disabled.
...
1. **Modules depend on Core, not on each other.** Avoid new direct
   ...
   `NOTIFY.0B` applies this rule to directed notifications: future source
   modules may import only the minimal Core notification delivery port, while
   the registered `notifications` module registers its persistence sink
   through Core. Source modules must not import
...
7. **New modules require explicit approval.** `COMMUNITY-EVENTS.1A` is the
   ...
   broader Community Activity notifications beyond the implemented primary-
   creator review outcomes, comments, payments, Community Activity-owned writable
   ...
   or serving.
8. **Church Calendar has a model-free read-only foundation.**
   [`CHURCH_CALENDAR_V1_PLAN.md`](CHURCH_CALENDAR_V1_PLAN.md) is the approved
   V1 boundary. `CHURCH-CALENDAR.1A` adds the independent `church_calendar`
   app/key, default enablement, module-gated bilingual navigation, authenticated
   read-only `/calendar/` and `/calendar/<year>/<month>/<day>/` routes, basic
   month/day templates with safe empty states, local-date range helpers, and a
   ...
   dashboard, or CommunityActivity-to-ServiceEvent relationship.
   `CHURCH-CALENDAR.1C` renders the member-facing month grid and day detail on
   top of the 1B providers: presentation-only local-date bucketing (half-open
   overlap), multi-day items on every overlapping day, point-in-time Bible
   Study meetings, active-window announcements shown as active communication,
   an explicit bilingual "more" link that keeps every item discoverable on the
   day detail, and a complete uncapped day list splitting timed items from
   announcements with owning member-facing detail links. Arrangement lives in
   `church_calendar.presentation` (no source query, visibility re-check, or
   `CalendarItem` mutation); type uses a dot plus text label / legend and
   screen-reader text, not color alone. 1C adds no model, migration, Today, My
   Serving, serving, attendance, or data write; the UI is read-only and carries
   no edit/publish/review/assignment/staff-management control.
   `CHURCH-CALENDAR.1D-A` prepares tests/docs closure and the manual QA
   checklist. It records 1A/1B/1C as implemented, adds only a missing focused
   co-organizer member-calendar bypass regression test, and creates
   [`CHURCH_CALENDAR_V1_QA_CHECKLIST.md`](CHURCH_CALENDAR_V1_QA_CHECKLIST.md)
   as an initially unchecked product-owner checklist. No new feature scope was
   added.
   `CHURCH-CALENDAR.2A` adds the `ministry`-owned read-only personal
   `my_serving` provider. It shows only the viewer's own explicit
   `TeamAssignmentMember` serving, is registered at
   `church_calendar.registration` after the four source providers, and is gated
   by `ministry` enablement, so disabled `ministry` means no serving provider
   call or query. Serving is never inferred from `ChurchStructureMembership`,
   audience, visibility, staff/superuser/manager authority, or other users'
   ...
   Activities, Announcements, and My Serving items, day detail works,
   `my_serving` items still target the viewer's specific assignment anchor
   (`/my-serving/?tab=all#serving-assignment-<TeamAssignmentMember.id>`), and
   `/my-serving/?tab=past` no longer returns 500 after the serving-card template
   syntax hotfix. The leaked template comment text is gone. Calendar remains a
...
9. **Notifications is a registered module behind a small Core delivery port
   (`NOTIFY.1A`–`NOTIFY.1F`).** Module key `notifications` owns the
   ...
   and a notifications-owned bilingual utility bell/unread count. `NOTIFY.1F`
   adds recipient-scoped POST Open/read behavior, standard newest-first 25-row
   pagination, and retain-for-now with no cleanup command.
   Core owns only the small
   ...
   The registry entry has no `depends_on` relationship to `events`,
   ...
   second feature flag exists. Disabled notifications means the Core emit
   ...
   Delivery runs through `transaction.on_commit()` so a source
   ...
   or a broad production/security/accessibility/hosting certification.

   `NOTIFY.1D` adds the second source emitter: a studies-owned helper used only
   after approved interactive `BibleStudyMeetingRole` create/edit saves. Its
   only recipient is the active linked `role.user`. New linked roles,
   display-only-to-linked changes, and reassignment emit
   `bible_study_role.assigned` to the current user; same-user role-type changes
   emit `bible_study_role.updated`, with recipient change taking priority.
   Display-name-only, audience, belonging, coworker-role, manager, and staff
   users are never inferred. Notes/display-name edits, unchanged saves,
   confirmation, delete/removal, lifecycle changes, admin/direct ORM,
   generation/setup/import, and failed writes remain non-notifying. Eligibility
   matches the existing published/completed meeting/lesson/series explicit
   serving lifecycle without an audience gate. The member-facing meeting detail
   target uses the existing exact-meeting read-only serving gate; it creates no
   audience/membership row and grants no management permission.

   The product owner completed deployed `NOTIFY.1D` smoke QA successfully. This
   is the user-confirmed result of a narrow deployed smoke test, not browser
   automation or a broad production/security/accessibility/hosting
   certification.

   `NOTIFY.1E` adds the third source emitter: a Community Activities-owned
   helper invoked inside the existing locked staff review transaction only after
   an applied request-changes, publish, or cancel/reject save. Its sole possible
   recipient is the active primary creator. Co-organizers, selected audience,
   signup users, membership, manager/staff authority, and organizer display text
   never infer recipients. Submission/resubmission and ordinary edits,
   missing-note or stale/disallowed review actions, signup lifecycle,
   admin/direct ORM/setup/import writes, and reads remain non-notifying. The
   target is the existing creator-safe member detail; its localized stored
   snapshot excludes `review_note` and other private review/audience data. This
   adds no lifecycle, permission, visibility, schema, UI, Calendar, serving, My
   Serving, or `ServiceEvent` relationship.

   The product owner completed deployed `NOTIFY.1E` smoke QA successfully for
   the implemented primary-creator review-notification workflow. This is a
   narrow user-confirmed result, not browser automation or a production,
   security, accessibility, or hosting certification.

   `NOTIFY.1F` keeps source targets permission-authoritative: recipient-scoped
   POST Open marks an unread record read and redirects only to its stored safe
   relative internal target, without source lookup. Invalid stored targets fail
   ...
   The stored target remains permission-neutral and never grants target access. The
   ...
   exists outside the bounded ministry assignment, studies meeting-role, and
   Community Activity primary-creator review-outcome sources. Later producers,
   retention cleanup, and broader notification behavior remain separately
   deferred.

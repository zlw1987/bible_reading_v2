...
- **CS-CORE.4C.1 — Reflection edit re-bind policy decision. Complete as docs-only.** Records the deliberate decision (Policy C: preserve the original `small_group_at_post` snapshot when editing an existing group-shared reflection; stamp the current group only when newly changing private/church → group; never re-home replies) for the top-level group-edit re-bind, to be implemented only in a future runtime slice. No runtime/test change; the current re-bind behavior and its 4C lock test are unchanged. See `docs/REFLECTION_EDIT_REBIND_POLICY_DECISION.md`.

...
| `notifications` | Notifications | Active V0 limited-trial notification path through `NOTIFY.1F` | Recipient center/bell; no ordinary primary-nav surface | None | None | None | None |
...
`notifications`, and `ministry`; unknown module keys and unmet dependencies
raise `ImproperlyConfigured`, and `ministry` depends on `events`. Notifications
is registered and default-enabled, with the bounded recipient center/bell/read
state and three approved producers documented in `docs/NOTIFICATIONS_V0_PLAN.md`.
...
| `AUDIT-DEFER-001` | Notifications | `DEFERRED-BY-DESIGN` | Closed by later separately approved product implementation | The August 13 repository audit correctly recorded the then-current planning-only state. That finding was later superseded and closed by the separately approved `NOTIFY.1A` through `NOTIFY.1F` implementation: registered/default-enabled Notification V0, recipient center/bell/read state, the Core delivery port, retain-for-now persistence, and the three approved producers. This later implementation is not attributed to the repository audit itself. | Notification V0 now supports the bounded in-app limited-trial scope; broader product expansion remains separately deferred and unapproved. | Keep additional producers, fanout, external delivery, preferences, cleanup, and broader surface integrations behind separate approval. Do not backfill notifications into Calendar, Today, Announcements, or Community Activities ad hoc. | Closed by `NOTIFY.1A`-`NOTIFY.1F`; broader future Notification scope remains deferred |
| `AUDIT-DEFER-002` | Community expansion | `DEFERRED-BY-DESIGN` | Open by design | Community Activities V1 intentionally defers signup, cancellation/reactivation, capacity/waitlist, co-organizer, selected-audience, reminder, broadcast, Calendar-driven, external-delivery, and other broader Community notification behavior. `NOTIFY.1E` implements only successful staff-review request-changes, publish, and cancel/reject outcomes to the primary creator. | Product expectations can exceed the intentionally bounded V1 implementation if not communicated. | Future separately approved Community V2 slices. Preserve Community Activities as separate from serving and official events; do not infer any additional notification producer from `NOTIFY.1E`. | Future approved Community V2 slice |
...
- Notification expansion beyond the closed `NOTIFY.1F` limited-trial V0 scope;
- Community waitlist;
- attendee management;
- Community check-in;
- payments;

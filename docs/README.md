...
| Community signup cancellation policy | [`COMMUNITY_SIGNUP_CANCELLATION_POLICY_PLAN.md`](COMMUNITY_SIGNUP_CANCELLATION_POLICY_PLAN.md) | Docs-only repository audit and V1 policy recommendation for member signup cancellation: retain `ActivitySignup` rows with `signed_up` / `cancelled` status, count active rows only, allow immediate self-service cancellation without review, and keep Community Activities separate from serving, official events, Calendar writes, and notifications. |

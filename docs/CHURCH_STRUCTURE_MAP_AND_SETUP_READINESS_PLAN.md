...
CS-SETUP.1C.1 shipped the initial read-only review slice of CS-SETUP.1C. It did not implement mapping edit at that time. CS-SETUP.1D.1 later added the separately approved one-row mapping edit action, and CS-SETUP.1D.2 later added display-only warning/conflict polish. Create/move/deactivate (CS-SETUP.1E) and membership management (CS-SETUP.1F) remain unapproved and unimplemented.
...
Targeted regression coverage expected for CS-SETUP.1D.1 / 1D.2:
...
- Therefore CS-SETUP.1D.1 uses warning copy, a narrow edit surface, strict type/active validation, audit, and explicit non-goals; CS-SETUP.1D.2 adds display-only warning/conflict cues without changing data behavior.

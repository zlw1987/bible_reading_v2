...
| 12 | **Generic Team Roster Workbook Import architecture — DOCS/READ-ONLY COMPLETE; RUNTIME UNIMPLEMENTED (`MO-S.6F.GENERAL.0A`)** | One team-neutral 1..N roster grammar and exact-team membership review/reconciliation core; deployment-named static source specs bind exact sheet/column/header/destination `team_key`; new generic mapping/preview/confirmation contracts; row-preserving safe diff; SQLite CAS/conditional barriers; atomic rollback; privacy-bounded audit; zero Notification | Future parameterized 1/2/3/>3 grammar, identity, set diff, create/add/remove/protected-state, history/audience, signature, concurrency, rollback, privacy, idempotency, and per-spec adapter tests | Sound/Projection future specs only; Video evidence is not enablement; Recording/Lighting unresolved; no runtime/schema/dependency/data/production mutation in this slice |

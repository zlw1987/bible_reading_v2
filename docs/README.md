...
Status: canonical documentation entry point, current through `NOTIFY.0B`
(docs-only architecture alignment after `REPOSITORY-AUDIT-CLOSEOUT.1A`).
...
| Notification V0 planning | [`NOTIFICATIONS_V0_PLAN.md`](NOTIFICATIONS_V0_PLAN.md) | Canonical docs-only boundary through `NOTIFY.0B`. Notifications remains a future registered/gateable module owning persistence and UX; Core owns only the planned small directed-delivery port/registered-sink contract; source modules resolve recipients and never import `notifications`. Disabled delivery is a safe no-op. `NOTIFY.1A` remains separately approved foundation-only work with no producer or UI. |
...
- The module registry contains `reading`, `prayers`, `studies`, `events`,
  ...
  rows fail closed.
- `NOTIFY.0B` changes documentation only. No `notifications` app/key/runtime
  exists yet. The future `notifications` module will own persistence and UX;
  source modules will resolve recipients and call a minimal Core delivery port,
  never import the notifications app. It will use existing
  `CMS_ENABLED_MODULES` semantics, have no source-module hard dependencies, and
  contribute no ordinary primary nav, Today, setup/readiness, or Staff Overview
  surface in `NOTIFY.1A`. That foundation, its later center/bell UI, and all
  producers still require separate approval.

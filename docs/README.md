...
| Sunday Ministry Scheduling | [`SUNDAY_MINISTRY_SCHEDULING_PLAN.md`](SUNDAY_MINISTRY_SCHEDULING_PLAN.md) | Current MO-S.6 plan through implemented Board/context, Campus, Worship pool, event-planner, governed exact-event selection, and operational-reachability work, plus docs-only workbook and batch-planner contracts. |
| Worship Rotation Governance | [`WORSHIP_ROTATION_GOVERNANCE_PLAN.md`](WORSHIP_ROTATION_GOVERNANCE_PLAN.md) | Canonical multi-campus Worship invariants through implemented exact-event governance and operational reachability; planner runtime, notifications, and import remain separately approved later slices. |
| Worship Rotation Planner | [`WORSHIP_ROTATION_PLANNER_PLAN.md`](WORSHIP_ROTATION_PLANNER_PLAN.md) | Docs-only `MO-S.6D-1D-D-0A` V1 batch contract: exact bounded insert/shift semantics, no tail loss, destination eligibility, per-event authority, roster blocker, roster-free downstream impact, signed proposal fingerprints, shared-operation LogEntry audit, and `1A`/`1B` runtime split. |

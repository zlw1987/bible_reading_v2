...
Status: TODAY-HOME.1A / 1A.1 docs-only product planning complete; TODAY-HOME.1B read-only IA restructure complete; TODAY-HOME.1D linked Bible Study role chips complete; TODAY-AGENDA.1A agenda/serving/management separation and MY-SERVING-BS.1B Bible Study serving integration are complete.
Revised by TODAY-HOME.1A.1 after user product decisions and updated through TODAY-AGENDA.1A / MY-SERVING-BS.1B: (1) the week strip shows **Church Gatherings / 教会聚会** — visible upcoming `ServiceEvent` rows from the Church Gatherings module — with no hardcoded "special `event_type`" guessing; (2) linked-user Bible Study roles remain read-only agenda context on Today and also join the action center while confirmation is pending; (3) My Serving remains the confirmation workspace.

Scope guardrails inherited from AGENTS.md and the task prompt:

- No runtime visibility change from Today itself. ServiceEvent visibility uses audience rows matched by active primary membership, with zero-row events failing closed after SE-RETIRE.1B, and Today uses the already-visible Bible Study v2 meeting selected by `BibleStudyMeeting.can_be_seen_by`.
- Since BS-STRUCT.2A, `BibleStudyMeetingAudienceScope` rows plus active primary `ChurchStructureMembership` are the runtime source for Bible Study v2 `BibleStudyMeeting` ordinary-member visibility; Today must not add any separate visibility rule on top of that.
...
1. **Needs your attention / 需要你留意** — only items with a real pending action *backed by a real model state*. The completed Today implementation includes pending ministry-team assignment confirmations (`TeamAssignmentMember.confirmed_at IS NULL` on upcoming non-cancelled assignments) and pending linked-user Bible Study roles. Today links to My Serving for confirmation and contains no inline confirmation workflow. Hidden entirely when empty (no empty state — absence of the section is the "all clear").
...
| Unconfirmed small-group Bible-study serving assignments | **Available in Today and My Serving** | `BibleStudyMeetingRole.confirmed_at IS NULL` for linked-user roles on visible published/completed V2 meetings | Do not infer these from small-group membership or audience visibility. Today includes the pending item in its action center; confirmation belongs to My Serving. |
...
- **Ordinary users.**
  ...
  - Bible Study meeting: only the user's audience-row-visible small-group meeting, gated by `BibleStudyMeeting.can_be_seen_by` (published meeting + published lesson + active published series + active primary `ChurchStructureMembership` matching a `BibleStudyMeetingAudienceScope` unit or descendant). Zero-row V2 meetings fail closed for ordinary users; `Profile.small_group` and the legacy `BibleStudyMeeting.small_group` mirror do not grant v2 meeting visibility.
  ...
  - Church Gatherings (`ServiceEvent`): only events passing `can_be_seen_by` — audience rows matched by active primary membership, while zero-row events fail closed for ordinary users after SE-RETIRE.1B. Today introduces **no new visibility rule**, only a 7-day time filter and draft/cancelled exclusion over already-visible events.
...
- **TODAY-AGENDA.1A / MY-SERVING-BS.1B.** Completed. Today keeps visible gatherings and Bible Study meetings as agenda, folds pending explicit team/Bible Study serving into its action center, and shows manager coverage separately; My Serving remains the confirmation workspace. Membership, audience visibility, display-name-only roles, and long-term ministry responsibility do not become personal serving.
...
Bible-study role *confirmation* is no longer deferred: it exists in My Serving.
Today may surface pending linked-user roles in its action center but remains
read-only; an inline Today confirmation workflow still requires separate
approval.

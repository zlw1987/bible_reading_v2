...
| Generic deployment configuration | [`GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md`](GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md) · [`Slice 5 implementation plan`](GENERIC_DEPLOYMENT_CONFIGURATION_SLICE5_PLAN.md) | Frozen architecture; 1A/2A implement Ministry Team key identity/configuration tooling and the product owner reports the current SVCA production deployment at 11 configured teams with 0 unconfigured/integrity problems (deployment data only; no key-text runtime behavior). 3A implements the exact ServiceProfile table plus nullable protected ServiceEvent FK expansion. 4A is implementation-complete, locally verified, and production-applied/post-audit-verified for one reviewed SVCA mapping: one profile, 52 exact dual-consistent events, zero drift, and revisions advanced `1 -> 2` exactly once. 5A completes the repository-wide audit/plan; 5B is implemented and locally verified with a static default-off integration registry, fail-closed Worship XLSX/Lighting gates, and lazy adapter imports; 5C is implemented and locally verified with the typed events-owned FK-authoritative inspection/strict-resolution and exact pair-write runtime seam. No current consumer adopted 5C. The current SVCA deployment must explicitly enable `svca_bethany_2026_worship_xlsx` before/with 5B deployment; Lighting remains disabled pending a retention/identity decision. `runtime_consumer_switched` remains false, `service_profile_key` remains current runtime authority outside the unused seam, and no signed workbook/reset contract changed. Profile defaults/materialization, MO-S.REQUIRED runtime, external-system mapping, consumer switching, and legacy-key retirement remain unimplemented. |

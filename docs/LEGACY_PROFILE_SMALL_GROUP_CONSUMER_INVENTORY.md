...
| Group progress accessible group list | `accounts.permissions.get_accessible_progress_groups()` | `ChurchRoleAssignment` legacy `District` / `SmallGroup` scopes plus the user's own `Profile.small_group` | Which groups a user may select for group progress | Yes. This is tied to legacy role scopes and progress permissions. Still legacy-driven after CS-CORE.4A/4B/4C/4E/4E.1; CS-CORE.4E added only a shadow comparison and CS-CORE.4E.1 only a focused read-only audit command (`audit_group_progress_shadow`), with no permission/source switch | High | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A/4B/4C/4E/4E.1); role-scope decision stays out of the 4-series |
| Group progress roster and default selected group | `reading.views.my_group_progress()` | Defaults to `Profile.small_group`; roster is `User.objects.filter(profile__small_group=selected_group)` | Member rows, enrollments, check-ins, and progress percentages for selected legacy groups | Yes, but do not migrate casually because this affects privacy-visible rosters. Still legacy-driven at runtime after CS-CORE.4A/4B/4C/4E/4E.1; CS-CORE.4E adds only a comparison-only membership-core shadow (`reading/group_progress_shadow.py`, internal `group_progress_shadow` context, not rendered) and CS-CORE.4E.1 adds only a focused read-only audit command (`reading.management.commands.audit_group_progress_shadow`, reusing `compute_group_progress_roster_shadow`) with no source/permission switch | High | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A/4B/4C/4E/4E.1) with privacy invariants and a shadow/switch sequence; CS-CORE.4E is the shadow step and CS-CORE.4E.1 the operational gate command |
...
| Reading/reflection privacy readiness audit | `reading.management.commands.audit_reading_privacy_membership_readiness` | Compares legacy `Profile.small_group` / `small_group_at_post` reflection visibility and legacy progress rosters against a membership-core candidate | Read-only diagnostic support for CS-CORE.4B; reports drift/risk categories only | Yes. This is diagnostic-only after CS-CORE.4B and covered by CS-CORE.4C tests; it writes nothing, and reading/progress/reflection runtime remains legacy-driven | Low | Keep as gate evidence for future CS-CORE.4F+ slices; do not use it from request/runtime code |
| Group-progress shadow audit (focused) | `reading.management.commands.audit_group_progress_shadow` | Compares legacy `Profile.small_group` rosters/defaults against the membership-core candidate per active legacy `SmallGroup` (reuses `reading.group_progress_shadow.compute_group_progress_roster_shadow`) | Read-only operational gate report for CS-CORE.4E.1; reports roster/default drift counters only | Yes. This is read-only after CS-CORE.4E.1; it writes nothing, has no `--apply`, never calls `my_group_progress()`, and group-progress runtime remains legacy-driven | Low | Run against real data as gate evidence before any future group-progress source switch; do not use it from request/runtime code |

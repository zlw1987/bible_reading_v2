...
- Legacy bridge mappings: nullable `church_structure_unit` fields on `MinistryContext`, `District`, and `SmallGroup` (`accounts/models.py`). These fields are not membership/source-of-truth rows and do not directly edit audience rows, but structure-based ServiceEvent / Bible Study matching can resolve through them during the transition.
...
| CS-SETUP.1D.1 | Legacy mapping maintenance implementation | Implemented: staff-only per-row edit of one legacy row's `church_structure_unit` to one active matching-type unit, gated by the legacy model's Django change permission, explicit POST, type/active/duplicate validation, and `LogEntry` audit (Section 12.8). It does not directly edit members, audience rows, serving schedules, or permissions, but it edits the same legacy-to-unit bridge that structure-based ServiceEvent / Bible Study scopes resolve through, so it can change which current groups/members an existing structure-based scope matches (acknowledgement added in CS-SETUP.1D.4) |
...
- Do not directly edit ServiceEvent audience rows, Bible Study audience rows, reading visibility, My Serving behavior, memberships, or serving rows. Note this is not an absolute "no visibility effect" claim: a mapping edit updates the legacy-to-unit bridge that structure-based ServiceEvent / Bible Study scope matching resolves through, so it may affect which current groups/members match those scopes. CS-SETUP.1D.4 discloses this in the visible copy and requires a staff acknowledgement before save.
...
- It shows a warning consistent with the shipped UI (CS-SETUP.1D.4): the edit does not directly edit members, event/Bible Study audience rows, serving schedules, permissions, or `Profile.small_group`, but because structure-based ServiceEvent / Bible Study scopes use these mapping links to match current groups, saving a mapping change may affect which current groups/members match those scopes. The shipped copy reads `This updates the setup link for this current record. It does not directly edit members, event audience rows, Bible Study audience rows, serving schedules, or permissions. Because structure-based scopes use these links to match current groups, saving this change may affect who matches existing structure-based event or Bible Study scopes.` / `这里会更新这条现有记录的设置对应关系。它不会直接编辑成员、聚会事件适用范围记录、查经适用范围记录、服事安排或权限；但使用教会结构适用范围的聚会事件和查经安排会参考这些对应关系来匹配现有小组，因此保存此更改可能影响哪些人被匹配到既有的结构适用范围。`
- It requires a staff acknowledgement checkbox (`I understand this mapping change may affect structure-based event or Bible Study matching.` / `我了解此对应关系更改可能影响结构适用范围对聚会事件或查经安排的匹配。`) before the save is accepted.
...
- Mapping edits can affect future helpers, setup-readiness indicators, staff interpretation, and which current groups/members match structure-based ServiceEvent / Bible Study scopes, because they update the legacy-to-unit bridge those scopes resolve through.

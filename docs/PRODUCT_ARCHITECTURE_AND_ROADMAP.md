...
PV-C.1 records pilot validation closure. Pilot validation passed on `v0.9-pilot-rc1`. CS-H.1 through CS-H.10, PP-SA.2 through PP-SA.5, MO-S.1 through MO-S.5B, SE-AS.1 through SE-AS.4, DOCS-AS.1, BS-AS.1 through BS-AS.2A, and CS-MAP.2 are complete. SE-AS.4 is complete as the ServiceEvent runtime visibility rule with legacy fallback: events with `ServiceEventAudienceScope` rows use those rows for ordinary-user visibility, events with no rows keep legacy `scope_type` / `district` / `small_group` plus `Profile.small_group` behavior, `ChurchStructureMembership` still does not grant ServiceEvent visibility, and legacy fields remain preserved as fallback. SE-AS.4 did not add the SE-AS.5 staff audience selector, any ServiceEvent form/template audience picker, Community Activities, CS-MAP.3, CS-SETUP.1, schema changes, migrations, consumer migration, or changes to ServiceEvent MinistryContext, Required Ministry Teams, Rotation Anchor Team, TeamAssignment, or My Serving behavior. Runtime consumers still primarily use legacy models and `Profile.small_group`; implementation beyond the completed ServiceEvent runtime fallback rule and Bible Study Schedule audience scope remains future and phased. See `docs/POST_PILOT_BACKLOG_TRIAGE.md`, `docs/FLEXIBLE_CHURCH_STRUCTURE_AND_AUDIENCE_SCOPE_DESIGN.md`, `docs/CHURCH_STRUCTURE_MAPPING_AND_MEMBERSHIP_STRATEGY.md`, `docs/CHURCH_STRUCTURE_SEEDING_VERIFICATION.md`, `docs/CHURCH_STRUCTURE_MEMBERSHIP_BACKFILL_VERIFICATION.md`, `docs/CHURCH_STRUCTURE_MEMBERSHIP_DESIGN.md`, `docs/STAFF_ADMIN_SURFACE_EXPANSION_PLAN.md`, `docs/MINISTRY_SCHEDULING_REQUIREMENTS_PLAN.md`, `docs/SERVICE_EVENT_AUDIENCE_SCOPE_REDESIGN_PLAN.md`, and `docs/SERVICE_EVENT_AUDIENCE_RUNTIME_MIGRATION_PLAN.md`.

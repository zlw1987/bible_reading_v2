...
- `ChurchStructureMembership` (`accounts/models.py`): belonging foundation with requested/active/ended/rejected/cancelled lifecycle, one-active-primary-per-user enforcement, date windows, approval audit fields, and `active_for_user()` / `current_primary_for_user()` helpers. It is the runtime source for ServiceEvent audience-row matching after CS-CORE.2B-A, Prayer group requests after PRAYER-STRUCT.1A, Bible Study v2 `BibleStudyMeeting` audience-row visibility / Today / role-worship pickers after BS-STRUCT.2A, and the separately switched reading/reflection/group-progress slices. It is not the runtime source for Bible Study generation, TeamAssignment / My Serving, or historical/retired V1/archive surfaces. Requested/rejected/ended/cancelled rows grant nothing.
- Legacy bridge mappings: the nullable `church_structure_unit` bridge fields on `MinistryContext`, `District`, and `SmallGroup` are removed with their legacy object tables by LEGACY-STRUCTURE-TABLE-RETIRE.1A. They were not membership/source-of-truth rows and did not directly edit audience rows. Since CS-CORE.2B-A, ServiceEvent audience rows match by active primary `ChurchStructureMembership`; since BS-SMALLGROUP-GENERATION-BRIDGE-RETIRE.1A, normal Bible Study V2 generation targets structure units directly.
...
| Concept | Source | Role |
| --- | --- | --- |
| Church Structure Unit | `ChurchStructureUnit` | Current flexible structure foundation (tree of church/ministry/district/group units) and canonical structure hierarchy. Runtime effects are consumer-specific through audience rows, snapshots, memberships, and role scopes rather than the tree alone. |
| Church Structure Membership | `ChurchStructureMembership` | Runtime source for ServiceEvent structure-audience row matching after CS-CORE.2B-A, Prayer group requests after PRAYER-STRUCT.1A, Bible Study v2 audience-row visibility / Today / role-worship pickers after BS-STRUCT.2A, and separately switched reading/reflection/group-progress slices. Not the runtime source for Bible Study generation, roles/capabilities, TeamAssignment / My Serving, or historical/retired V1/archive surfaces. Requested/rejected/ended/cancelled rows grant nothing. |
...
| Legacy `Profile.small_group` | `Profile.small_group` | Removed in PROFILE-SG-FIELD-RETIRE.1A. Historical bridge-period docs may mention it as a former belonging field, but it no longer exists in current models and must not be treated as a runtime source. |
| Legacy `District` / `SmallGroup` / `MinistryContext` | historical legacy structure models | Rows were purged under guarded apply; LEGACY-STRUCTURE-TABLE-RETIRE.1A removes the models/tables and bridge mapping FKs behind a zero-row migration guard. They are not ordinary-member runtime authority. |
...
| CS-SETUP.1F | Membership / belonging management UI | Narrow small-group My Units slice implemented separately as GROUP-MEMBERSHIP-MANAGE.1A / GROUP-MEMBERSHIP-REQUEST.1B (add/end plus pending-request approve/reject, QA-passed). Broad transfer/bulk/history tooling remains deferred (Section 12) |
...
| Risk | Mitigation |
| --- | --- |
| The map makes the unit tree look like a universal runtime source of truth; staff edit units or mappings in Django Admin without understanding module-specific effects | Staff wording must stay explicit: ordinary-user matching is consumer-specific. ServiceEvent audience rows, Prayer group requests, Bible Study V2 audience-row meetings, and completed reading/progress/reflection slices now match through active primary `ChurchStructureMembership`; Bible Study generation, roles/capabilities, TeamAssignment / My Serving, and historical/retired archive surfaces remain separate concerns. CS-SETUP.1 remains gated until unit moves, deactivation, membership drift, and stored audience-row effects are explicitly designed. |
...
- **`ChurchStructureMembership` is only a partial runtime source.** Editing membership/belonging rows can affect ServiceEvent structure-audience row matching, Prayer group requests, Bible Study V2 audience-row meetings/pickers, and completed reading/progress/reflection slices, but not Bible Study generation, roles/capabilities, TeamAssignment / My Serving, or historical/retired V1/archive surfaces. A UI that mixes membership edits with structure edits invites staff to misread which consumer changed or to assume a structure change is "just bookkeeping" (it can move stored audiences).
...
6. **Membership / belonging changes.** Create/approve/end `ChurchStructureMembership` rows. This is the source for ServiceEvent audience-row matching, Prayer group requests, Bible Study V2 audience-row meetings/pickers, and completed reading/progress/reflection slices, but it is not the runtime source for Bible Study generation, roles/capabilities, TeamAssignment / My Serving, or historical/retired V1/archive surfaces. It must be treated as a separate product surface from structure editing, with its own approval, precisely so staff do not read it as a universal visibility control.
...
- **CS-SETUP.1F** — narrow membership / belonging management was implemented separately as GROUP-MEMBERSHIP-MANAGE.1A / GROUP-MEMBERSHIP-REQUEST.1B: authorized small-group/ancestor leads and staff can add eligible unassigned users, end active membership, and approve/reject pending requests in My Units; the global staff queue remains. "Unassigned" is a no-membership/no-pending-request state, never a fake unit. Active-primary conflicts are blocked; full transfer/bulk/history tooling remains deferred. Belonging does not infer serving, roles, or permissions.
...
CS-SETUP.1C.1 shipped the initial read-only review slice of CS-SETUP.1C. It did not implement mapping edit at that time. CS-SETUP.1D.1 later added the separately approved one-row mapping edit action, CS-SETUP.1D.2 later added display-only warning/conflict polish, and CS-SETUP.1D.3 later added display-only conflict filter links for those overlays. Create/move/deactivate (CS-SETUP.1E) remains unapproved and unimplemented. Narrow small-group belonging management later shipped separately through GROUP-MEMBERSHIP-MANAGE.1A / GROUP-MEMBERSHIP-REQUEST.1B; broad transfer/bulk/history tooling remains deferred.

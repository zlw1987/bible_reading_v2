...
| `AUDIT-QA-001` | Portal UX and Help Center | `MANUAL-QA` | Implementation follow-up complete; awaiting final deployed mobile regression confirmation. | `docs/PORTAL_UX_AND_HELP_CENTER_PLAN.md` records implementation and regression coverage. Product-owner manual QA for `PORTAL-HELP-MANUAL-QA.1A` otherwise passed, with one `LOW` mobile drawer accordion issue found and addressed by `PORTAL-HELP-MANUAL-QA.1A-FU1`; final deployed mobile regression confirmation remains outstanding. | Visual navigation/content polish issues can survive automated route and permission tests. | Deploy through the normal approved path later, then run the quick mobile dropdown regression confirmation and record pass/fail evidence. | `PORTAL-HELP-MANUAL-QA.1A`; `PORTAL-HELP-MANUAL-QA.1A-FU1` |

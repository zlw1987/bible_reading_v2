...
Ministry scheduling requirements from real pilot feedback are recorded in `docs/MINISTRY_SCHEDULING_REQUIREMENTS_PLAN.md`. MO-S.1 is docs-only; it does not change ServiceEvent, TeamAssignment, TeamAssignmentMember, permissions, audience behavior, or runtime source of truth.
...
- Required-team coverage display and same-type event scheduling workflow, now recorded as MO-S.1 pilot feedback
...
- MO-S.2 event required-team model/design implementation should come before Community Activities and Checklist re-evaluation.
- MO-S.3 assignment coverage display for required teams should follow MO-S.2.
- MO-S.4 team-leader scheduling workspace for same-type events should follow coverage display.
- MO-S.5 limited rotation helper/copy-forward workflow should wait until the manual workflow is proven.
...
PV-C.1 records pilot validation closure. Pilot validation passed on `v0.9-pilot-rc1`. CS-H.1 Flexible Church Structure and Audience Scope Design Doc is complete, CS-H.2 adds the model-only `ChurchStructureUnit` foundation, CS-H.2A hardens it, CS-H.3 records mapping/membership strategy, CS-H.3B adds nullable legacy mapping fields, CS-H.3C adds explicit command-based seeding/mapping, CS-H.3D verifies GoDaddy production/staging seeding, CS-H.3E closes seeded structure data QA, CS-H.4 records membership design, CS-H.5A adds the membership model-only foundation, CS-H.5B hardens membership helpers/validation, CS-H.5C adds explicit command-based membership backfill, CS-H.5D records user-attested production/staging backfill verification, CS-H.5E improves Django Admin clarity, CS-H.6 through CS-H.7E complete requested-unit capture and staff approval/sync slices, CS-H.8 integration checkpoint is complete, CS-H.9 membership request UX hardening is complete, CS-H.10 CMS hardening checkpoint is complete, PP-SA.2 read-only staff overview is complete, PP-SA.3 membership/admin workflow polish is complete, PP-SA.4 moderation/admin queues are complete, PP-SA.5 ministry ops admin improvements are complete, and MO-S.1 ministry scheduling requirements planning is complete. PP-SA.4 adds a permission-protected read-only staff moderation queue at `/staff/moderation/` that summarizes existing report and hidden states only. It includes reported/hidden prayer request categories and reported/hidden reflection post/reply categories; prayer comment report/hidden categories are shown as not separately tracked by existing data. PP-SA.4 links to existing moderation workflows and adds no moderation actions or states. PP-SA.5 adds read-only ministry ops health indicators on `/staff/`: upcoming ServiceEvents, upcoming TeamAssignments, unconfirmed assignments, inactive teams, active teams missing playbook links, display-name-only active members, active teams with no active members, upcoming assignments without active members, and upcoming assignments using inactive teams. The PP-SA.5 aggregate is a sum of warning indicator buckets, not a unique problematic-record count; the same object may contribute to multiple indicators. PP-SA.5 links only to existing ServiceEvent, MinistryTeam, and TeamAssignment workflows. MO-S.1 records real pilot feedback that required MinistryTeams should be selected while creating or batch-creating ServiceEvents, required-team coverage should show assigned coworkers and confirmation status on TeamAssignment pages, missing required teams should show as unassigned, and ministry team leaders need a same-type event scheduling entry point for their own team. MO-S.1 preserves the boundary that ServiceEvent required teams are event-level expectations, TeamAssignment is the actual scheduled assignment, and TeamAssignmentMember is assigned people plus confirmation. PP-SA.2 through PP-SA.5 and MO-S.1 include no schema changes, write actions, new states, availability, swaps, reminders, attendance, checklist, scheduling automation, pastoral case management, sensitive notes, consumer migration, audience filtering, Community Activities, notifications, announcements, care workflows, file center, permission matrix expansion, or ChurchStructureMembership serving inference. Browser/mobile QA for PP-SA.2 was completed via local Chrome Playwright, and browser/mobile QA for PP-SA.3 and PP-SA.4 was completed via headless Chromium fallback because the in-app browser connector failed. Browser/mobile QA for PP-SA.5 had initial headless Chromium QA completed before wording clarification; the final wording change should be manually checked if browser automation remains unavailable. Root `AGENTS.md` now includes safe QA data seeding guidance: avoid long inline PowerShell `manage.py shell` commands, prefer tests/fixtures/app UI, and never bypass endpoint security. Runtime consumers still primarily use legacy models and `Profile.small_group`; implementation beyond that remains future and phased. See `docs/POST_PILOT_BACKLOG_TRIAGE.md`, `docs/FLEXIBLE_CHURCH_STRUCTURE_AND_AUDIENCE_SCOPE_DESIGN.md`, `docs/CHURCH_STRUCTURE_MAPPING_AND_MEMBERSHIP_STRATEGY.md`, `docs/CHURCH_STRUCTURE_SEEDING_VERIFICATION.md`, `docs/CHURCH_STRUCTURE_MEMBERSHIP_BACKFILL_VERIFICATION.md`, `docs/CHURCH_STRUCTURE_MEMBERSHIP_DESIGN.md`, `docs/STAFF_ADMIN_SURFACE_EXPANSION_PLAN.md`, and `docs/MINISTRY_SCHEDULING_REQUIREMENTS_PLAN.md`.
...
Large deferred items remain deferred pending feedback. MO-S.1 is elevated as near-term P2 ministry operations feedback, but Checklist V1 remains deferred unless pilot feedback proves checklist need separately from required-team coverage.
...
- Reminder automation
- Checklist engine
- Community Activities before MO-S.2 through MO-S.4 are considered

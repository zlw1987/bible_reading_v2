...
| Generic deployment configuration | [`GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md`](GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md) | Frozen architecture; 1A/2A implement Ministry Team key identity/configuration tooling and the product owner reports the current SVCA production deployment at 11 configured teams with 0 unconfigured/integrity problems (deployment data only; no key-text runtime behavior). `GENERIC-DEPLOYMENT-CONFIG.3A` locally verifies the exact ServiceProfile table plus nullable protected ServiceEvent FK expansion; existing event FKs remain `NULL`, the legacy key stays authoritative, and no profile data/backfill/consumer switch occurred. Mapping/backfill, defaults/materialization, opt-in adapters, and external-system mapping remain pending. |

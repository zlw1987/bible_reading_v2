...
- SE-AS.4 ServiceEvent Audience Runtime Visibility Rule. Completed as runtime visibility with legacy fallback: events with `ServiceEventAudienceScope` rows use audience rows for ordinary-user visibility; events with zero rows keep legacy `scope_type` / `district` / `small_group` plus `Profile.small_group` behavior. At SE-AS.4 time, `ChurchStructureMembership` still did not grant ServiceEvent visibility; CS-CORE.2B-A later switched ServiceEvent audience-row matching to active primary `ChurchStructureMembership`. Legacy fields remain preserved as zero-row fallback. No SE-AS.5 selector UI, form/template audience picker, Community Activities, CS-MAP.3, or CS-SETUP.1 was added.
...
- CS-CORE.2B-A was the first consumer switch (ServiceEvent structure-audience matching → active primary `ChurchStructureMembership`; ServiceEvents with no audience rows still use legacy fallback and `Profile.small_group`). Since then, additional consumers have switched to membership-core: Bible Study v2 meeting visibility and the `/studies/` / Today v2 pre-filter (CS-CORE.2C-B), the group-progress visible roster (CS-CORE.4F.1) and permission-fenced default selected group (CS-CORE.4F.2), the ordinary-member group reflection **read** path (CS-CORE.4G.2), the reflection **write** path/forms (CS-CORE.4G.3), and the group-progress permission/accessible group list (CS-CORE.2D-B, foundation CS-CORE.2D-A). Still legacy/pending: ServiceEvent zero-row fallback, Bible Study v2 meeting generation, TeamAssignment/My Serving (serving is never inferred from structure membership), backfilling role `structure_unit` to retire the legacy role-scope fallback, and the legacy `Profile.small_group` / `SmallGroup` / `District` / `MinistryContext` compatibility layer (not retired).

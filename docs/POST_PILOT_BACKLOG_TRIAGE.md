...
- SE-AS.2 ServiceEventAudienceScope Model-Only Foundation. Completed as a model-only data foundation. Runtime visibility is handled by SE-AS.4; staff UI selector/display is handled by SE-AS.5; SE-AS.6 backfill and consumer migration remain deferred until separately approved.
...
44. SE-AS.5 ServiceEvent staff audience selector UI/display completed using the SE-AS.5A contract. The selector and staff effective display are implemented; SE-AS.6 backfill, Community Activities, CS-MAP.3, CS-SETUP.1, membership visibility migration, and ministry scheduling behavior changes remain unapproved.
45. Later: SE-AS.6 backfill/compatibility cleanup and future Community Activities reuse the same `ChurchStructureUnit` audience-scope foundation; neither is authorized by SE-AS.5 completion.
...
- SE-AS.6 ServiceEvent Audience Backfill / Compatibility Cleanup. Future, separately approved milestone; do not bundle with CS-MAP work or Community Activities.

...
| Generic deployment configuration | [`GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md`](GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md) · [`Slice 5 implementation plan`](GENERIC_DEPLOYMENT_CONFIGURATION_SLICE5_PLAN.md) | Frozen architecture; 1A/2A implement Ministry Team key identity/configuration tooling. 3A adds ServiceProfile/FK schema, and 4A is production-applied/post-audit-verified for one reviewed SVCA mapping (one profile, 52 exact dual-consistent events, zero drift, revisions `1 -> 2` once). 5A-5F complete the default-off integration boundary and FK-authoritative consumer closeout. 6A/6B provide the ministry-owned static-default foundation and production-verified reviewed configuration. `GENERIC-DEPLOYMENT-CONFIG.7A` is **PRODUCTION READ-ONLY PREVIEW VERIFIED**: full year 52 / 208 / 8 / 200, with the product owner deliberately deferring history and reviewing only `2026-09-08` through `2026-12-31` at 16 / 64 / 4 / 60 and zero review evidence/blockers. `GENERIC-DEPLOYMENT-CONFIG.7B` is **IMPLEMENTED / LOCAL VERIFIED; PRODUCTION APPLY NOT RUN**: exact actor/state-bound dry-run/apply tooling creates only missing static-default RequiredTeam pairs, claims and audits changed events only, preserves existing/manual/history/Worship rows, and uses post-claim/post-write current-truth recomputation. No production RequiredTeam materialization, automatic new-event initialization, Notification, or MO-S.REQUIRED runtime has occurred. `LIGHTING-PILOT-RETIRE.1A` remains implemented; external mapping and legacy-key field retirement remain separately pending. |

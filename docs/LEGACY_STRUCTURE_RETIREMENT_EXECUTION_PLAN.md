...
- GoDaddy production is now claimed for this final legacy table-retirement
...
schema-prep candidate for `ReflectionComment.small_group_at_post` was then
...
V1 schema-retirement status:

```powershell
.venv\Scripts\python.exe manage.py audit_legacy_structure_schema_retirement_readiness
```

BS-V1-SCHEMA-RETIRE.1A removes the retired V1 `BibleStudySession`,
`BibleStudyGuide`, and `BibleStudyWorshipSong` models/tables. The final local
preflight used the hardened V1 purge command in dry-run mode only and reported
zero V1 session rows, zero V1 guide rows, zero V1 worship-song rows, zero V1
scope/FK counters, zero unexpected inbound dependency rows, and
`data_mutated: false`. The purge command was then retired with the removed
models.

The schema migration starts with a `RunPython` guard that checks the historical
V1 tables through `apps.get_model()` and aborts before removal if any target DB
still has V1 session, guide, or worship-song rows. V1 is not migrated to
membership-core or V2. V1 no longer actively blocks final `SmallGroup` /
`District` table retirement after the guarded migration is applied; there was no
V1 `MinistryContext` FK.
...
tables or the `church_structure_unit` mapping bridge FKs. The later
BS-V1-SCHEMA-RETIRE.1A slice removes the V1 `BibleStudySession` legacy scope
fields and V1 child tables behind its own guard. Ordinary visibility still runs on
...
| V1 Bible Study session visibility | `studies.models.BibleStudySession.can_be_seen_by()` (removed with V1 model/table in BS-V1-SCHEMA-RETIRE.1A) | removed / historical only | BS-V1-RETIRE.1A made app-level V1 visibility fail closed for ordinary users and managers, then BS-V1-SCHEMA-RETIRE.1A removed `BibleStudySession`, `BibleStudyGuide`, and `BibleStudyWorshipSong` behind migration `studies/0012`'s row-count guard. V1 is not migrated to membership-core or V2. |
...
| Legacy bridge mappings | `MinistryContext.church_structure_unit`, `District.church_structure_unit`, `SmallGroup.church_structure_unit` | removed / historical only | These mapping FKs were retirement-context bridge data before LEGACY-STRUCTURE-TABLE-RETIRE.1A. After the guarded object-row purge and `accounts.0015_remove_legacy_structure_tables`, the legacy models/tables and their mapping FKs are removed on local/dev and on the GoDaddy production target reported deployed on 2026-06-24. Normal Bible Study V2 generation is structure-native and no longer depends on these mappings. |
| Legacy object-row retirement inventory | `accounts.management.commands.audit_legacy_structure_object_row_retirement` | deployment verification / historical diagnostic tooling | Read-only inventory used before and after row/table retirement to prove `SmallGroup`, `District`, and `MinistryContext` rows are zero. It classifies rows as table-retirement blockers when present, not ordinary-member runtime blockers. On local/dev and the human-reported GoDaddy production deployment, these rows are already zero and the final table-retirement migration is applied. |
| Legacy object-row purge gate | `accounts.management.commands.purge_legacy_structure_object_rows` | retired guarded apply path / historical only | This guarded destructive path completed before final table removal and was retired with the active legacy surfaces. It required `--apply --confirm-legacy-structure-object-row-retirement`, deleted only `SmallGroup`, `District`, and `MinistryContext` rows, and protected `ChurchStructureUnit` plus runtime product rows. Future target DBs must still prove zero rows before applying `accounts.0015`. |
| Legacy structure parent/context links | `SmallGroup.district`, `District.ministry_context` (**removed in LEGACY-PARENT-FK-FIELD-RETIRE.1A**) | removed / historical only | **Removed in LEGACY-PARENT-FK-FIELD-RETIRE.1A** (migration `accounts/0013`) after LEGACY-OBJECT-ADMIN-FK.1A/1B retired their admin display and LEGACY-BRIDGE-RESOLVER-NARROW.1A retired the resolver fallback read; target-DB dry-run confirmed 0 links present (`data_blocker_count=0`). The canonical hierarchy is `ChurchStructureUnit.parent`. The guarded `cleanup_legacy_structure_parent_links` command was retired with the fields, and `seed_church_structure_units` no longer reconstructs the `District` / `SmallGroup` hierarchy from raw legacy links. Only immutable historical migrations still name these fields. The later `accounts.0015` migration removes the legacy object tables and mapping FKs on any target DB that passes the zero-row guard; local/dev and the human-reported GoDaddy production deployment have applied it. |
| ServiceEvent Host / Language display | `ServiceEvent.host_language_unit`, `events.templatetags.event_extras.event_host_language_label`, `events.ministry_context_display` | removed / historical only (display now structure-native) | **`ServiceEvent.ministry_context` was removed in SERVICE-EVENT-CONTEXT.1C** (migration `events/0008`). "Host / Language" display now uses structure-native `ServiceEvent.host_language_unit`, then an audience-derived `ChurchStructureUnit.parent` fallback; `host_language_unit` is display-only and does not control audience/visibility. The display-only backfill/cleanup tooling (`backfill_service_event_host_language_units`, `cleanup_service_event_ministry_context_labels`) was retired with the field. The later `MinistryContext` table retirement is complete for local/dev and for the human-reported GoDaddy production deployment via `accounts.0015`; other future targets must still apply the guarded migration before being claimed. |
...
> **Current-state supersession — legacy object blockers:** this section is retained as historical blocker anatomy. For local/dev and for the GoDaddy production target reported deployed on 2026-06-24, the guarded object-row purge completed with zero legacy rows and `accounts.0015_remove_legacy_structure_tables` removed the `SmallGroup`, `District`, and `MinistryContext` active models/tables. On any other future target DB, the same blockers still matter until the guarded deployment flow proves zero rows and applies the migration.
...
- Before the guarded object-row purge and `accounts.0015`, existing `SmallGroup` rows and FK references blocked table retirement. For local/dev and the human-reported GoDaddy production deployment, those rows are zero and the table is retired.
...
- Unmapped/inactive/wrong-type bridge mappings were readiness blockers for any non-lossy migration or final archive step before the guarded row purge/table-retirement path completed.
...
- ROW-RETIRE.1A separately counted the then-remaining `SmallGroup` rows as
  future archive candidates when they mapped to active small-group units, but it
  did not approve deletion. That historical decision was superseded by the
  guarded object-row purge and `accounts.0015` table retirement.
...
- Before the guarded object-row purge and `accounts.0015`, existing rows and FK references blocked table retirement. For local/dev and the human-reported GoDaddy production deployment, those rows are zero and the table is retired.
- Wrong/missing bridge mappings blocked safe bridge-based backfill, audit, or final archive decisions before the guarded row purge/table-retirement path completed.
...
- ROW-RETIRE.1A highlighted `District #13 未分配小组` mapped to
  ...
  later final retirement decision chose guarded deletion of the legacy row while
  preserving the canonical `ChurchStructureUnit` custom unit.
...
- Before the guarded object-row purge and `accounts.0015`, existing rows and references blocked table retirement. For local/dev and the human-reported GoDaddy production deployment, those rows are zero and the table is retired.
- `District.ministry_context` was removed in LEGACY-PARENT-FK-FIELD-RETIRE.1A (migration `accounts/0013`), removing this inbound `MinistryContext` reference. `ServiceEvent.ministry_context` was removed in SERVICE-EVENT-CONTEXT.1C (migration `events/0008`); Host / Language display now uses structure-native `ServiceEvent.host_language_unit` plus the audience-derived structure fallback, and the display-only backfill/cleanup tooling (`backfill_service_event_host_language_units`, `cleanup_service_event_ministry_context_labels`) was retired with it. The `MinistryContext` rows/table remained until the later guarded object-row purge and `accounts.0015` table-retirement path, which are complete for local/dev and the human-reported GoDaddy production deployment.
- ROW-RETIRE.1A counted all then-remaining `MinistryContext` rows as bridge/admin/
  diagnostic row-retirement blockers until the Host / Language display cleanup,
  mapping bridge, and final table-retirement path were explicitly approved and
  completed.
...
- V1 `BibleStudySession` removed-state counters: app-runtime-retired state, zero retained V1 rows/child rows, and migration-guard readiness
...
- V1 `BibleStudySession` rows are no longer ordinary or manager app-runtime blockers after BS-V1-RETIRE.1A. BS-V1-SCHEMA-RETIRE.1A removes the V1 models/tables behind migration `studies/0012`'s row-count guard after local/current dry-run preflight reported zero V1 rows, zero child rows, zero scope/FK counters, zero unexpected inbound dependency rows, and `data_mutated: false`.
- The audit now preserves the removed state: V1 app-runtime blockers, purge-pending counters, V1 child-row counters, and legacy-retirement blockers report zero in the current worktree. Target DBs with any V1 rows are blocked by the schema migration guard and need an explicitly approved cleanup plan before migration apply.
- The former BS-V1-PURGE.1A command is historical/retired with the removed models. V1 is not migrated to membership-core or V2.
...
> **Current-state supersession — PROD-LEGACY-STRUCTURE-DEPLOY-CLOSE.1A:** for local/dev and for the GoDaddy production target reported deployed on 2026-06-24, the legacy object-row purge, active surface retirement, and final `SmallGroup` / `District` / `MinistryContext` model/table migration are complete. Steps 10-13 below are retained as historical sequence guidance and as the required caution for any other future target DB that has not yet run the guarded deployment flow.

1. Treat V1 Bible Study as schema-retired, not as a membership/V2 migration target. BS-V1-SCHEMA-RETIRE.1A removes `BibleStudySession`, `BibleStudyGuide`, and `BibleStudyWorshipSong` behind migration `studies/0012`'s row-count guard. The former `purge_legacy_bible_study_v1_sessions` command is historical/retired with the models; if a target DB still has V1 rows, stop at the migration guard and plan explicit cleanup before applying the schema migration.
...
8. Legacy parent/context FK field retirement is **done/retired**: the redundant `SmallGroup.district` / `District.ministry_context` parent FKs were removed in LEGACY-PARENT-FK-FIELD-RETIRE.1A (migration `accounts/0013`) after their admin display and resolver fallback reads were retired and target-DB dry-run confirmed 0 links present (`data_blocker_count=0`). The guarded `cleanup_legacy_structure_parent_links` command (its only purpose being to clear those two fields before removal) was retired with them, and `seed_church_structure_units` no longer reconstructs the `District` / `SmallGroup` hierarchy from raw legacy links. No parent-link cleanup command remains to run; only immutable historical migrations still name the two fields. The later guarded table-retirement migration removes the `SmallGroup` / `District` / `MinistryContext` active models/tables and mapping FKs on targets that pass the guard; local/dev and the human-reported GoDaddy production deployment have applied it.
9. ServiceEvent Host / Language display retirement is **done/retired**: `ServiceEvent.ministry_context` was removed in SERVICE-EVENT-CONTEXT.1C (migration `events/0008`), and the display-only `backfill_service_event_host_language_units` / `cleanup_service_event_ministry_context_labels` tooling was retired with it. Host / Language display uses structure-native `ServiceEvent.host_language_unit` plus the audience-derived structure fallback. The later `MinistryContext` row/table retirement is complete for local/dev and the human-reported GoDaddy production deployment via `accounts.0015`; other future targets must still apply the guarded migration before being claimed.
10. Historical/future-target caution: run `audit_legacy_structure_schema_retirement_readiness --verbose --limit 50`
   before proposing field/table removal on any target that has not already
   completed the guarded table-retirement deployment. Use it to identify which
   legacy fields are blocked by stored data, app writes, admin/display
   references, diagnostic cleanup tooling, bridge decisions, or only historical
   migrations. This command is schema-removal preparation only; no removal is
   approved by its output.
11. Historical/future-target caution: run `audit_legacy_structure_object_row_retirement --verbose --limit 30`
   before proposing any final row/table action on a target that has not already
   completed the guarded object-row purge/table-retirement path. After
   LEGACY-OBJECT-SURFACE-NARROW.1A, the audit explicitly reported remaining
   object rows as final table-retirement blockers, not ordinary-member runtime
   blockers; after the guarded purge and `accounts.0015`, those blockers are
   closed for local/dev and the GoDaddy production target reported deployed.
12. Historical/future-target caution: run `purge_legacy_structure_object_rows --verbose --limit 50`
   as the guarded final row-purge dry-run before any destructive decision on a
   target that still has legacy rows. Review the row counts, protected deletion
   counters, collector safety, and `UNASSIGNED-GROUPS` decision. Apply remains
   destructive and requires `--apply --confirm-legacy-structure-object-row-retirement`;
   do not run apply automatically.
13. Historical/future-target caution: plan `SmallGroup` / `District` / `MinistryContext` table/model and

...
- Later idempotent structure seeding, membership model, and signup approval workflow only after phased planning.
...
Flexible `ChurchStructureUnit` seeding, `ChurchStructureMembership`, and signup/onboarding approval remain future phased work after CS-H.3B and before advanced mixed audience segments or CM/EM-aware `ServiceEvent` filtering. CS-F.3 is not filtering; it is only an optional ServiceEvent label.

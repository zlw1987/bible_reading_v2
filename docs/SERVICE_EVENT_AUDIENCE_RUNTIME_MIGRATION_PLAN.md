...
- **Later — legacy scope field deprecation/removal planning (future, separate approval).** Evaluation of eventual legacy `scope_type` / `district` / `small_group` field deprecation/removal after guarded cleanup has been reviewed on each target database; no destructive schema change until display/admin/backfill/audit/rollback needs are accepted.

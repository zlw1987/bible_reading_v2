...
| Consumer | File / function / model | Current source | What it controls | Safe to leave legacy for now? | Migration risk | Recommended future milestone |
| --- | --- | --- | --- | --- | --- | --- |
| Current user group helper | `reading.views.get_user_small_group()` | `user.profile.small_group` | Shared reading/reflection helper for the user's current legacy group | Yes as an explicit legacy helper. Still legacy-driven after CS-CORE.4A (docs-only) | Low | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A) |
| Group progress accessible group list | `accounts.permissions.get_accessible_progress_groups()` | `ChurchRoleAssignment` legacy `District` / `SmallGroup` scopes plus the user's own `Profile.small_group` | Which groups a user may select for group progress | Yes. This is tied to legacy role scopes and progress permissions. Still legacy-driven after CS-CORE.4A (docs-only) | High | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A); role-scope decision stays out of the 4-series |
| Group progress roster and default selected group | `reading.views.my_group_progress()` | Defaults to `Profile.small_group`; roster is `User.objects.filter(profile__small_group=selected_group)` | Member rows, enrollments, check-ins, and progress percentages for selected legacy groups | Yes, but do not migrate casually because this affects privacy-visible rosters. Still legacy-driven after CS-CORE.4A (docs-only) | High | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A) with privacy invariants and a shadow/switch sequence |
| Reflection visibility checks | `comments.models.ReflectionComment.can_be_seen_by()`; `reading.views.get_visible_reflection_filter()`; `reading.views.passage_wall()` | `Profile.small_group` compared to `ReflectionComment.small_group_at_post` | Visibility for "My Group" reflections and group tab filtering | Yes. It preserves current privacy semantics and uses a snapshot group on each comment. Still legacy-driven after CS-CORE.4A (docs-only) | High | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A); transfer/snapshot policy decided there |
| Reflection create/edit group binding | `comments.forms.ReflectionCommentForm`; `comments.forms.ReflectionCommentEditForm`; `comments.views.add_comment()`; `comments.views.edit_comment()` | `Profile.small_group` determines whether group sharing is offered and binds `small_group_at_post` | Whether users can choose group visibility and which group is stored on the post | Yes until reflection privacy is separately planned. Still legacy-driven after CS-CORE.4A (docs-only) | High | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A); preserve old posts and reply inheritance |
...
2. **Plan-only slice: reading progress and reflection privacy migration.** Completed as docs-only by CS-CORE.4A (`docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md`): reading group progress, reflection group visibility, and `small_group_at_post` are treated as one privacy-sensitive plan with privacy invariants, target options, a staged shadow/switch sequence, and no-go rules. It is still docs-only — these consumers remain legacy-driven, and no runtime migration is authorized by it. Do not implement directly from this inventory.

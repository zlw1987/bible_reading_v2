...
| Sunday Ministry Scheduling | [`SUNDAY_MINISTRY_SCHEDULING_PLAN.md`](SUNDAY_MINISTRY_SCHEDULING_PLAN.md) | Current MO-S.6 plan through implemented Board/context, Campus, Worship pool, event-planner, governed exact-event selection, operational reachability, rotation-planner preview/confirmation/shared audit, and bounded `NOTIFY.1G` direct-change notifications; workbook and importer work remain separately scoped. |
| Worship Rotation Governance | [`WORSHIP_ROTATION_GOVERNANCE_PLAN.md`](WORSHIP_ROTATION_GOVERNANCE_PLAN.md) | Canonical multi-campus Worship invariants through implemented exact-event governance, operational reachability, planner confirmation/shared audit, and the bounded `NOTIFY.1G` direct-change producer; import remains a separately approved later slice. |

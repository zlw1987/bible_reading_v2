...
class SoundRosterUpdateTests(SoundAssignmentPreviewTestBase):
    ...
    def test_rendered_english_and_chinese_make_replacement_destructive(self):
        event = self.event_for_row()
        self._assignment(event, membership=self.bob)
        self.client.force_login(self.staff)
        for language, expected in (
            ("en", "Workbook reviewed replacement:"),
            ("zh", "工作簿复核替换为："),
        ):
            with self.subTest(language=language):
                ...
                review = upload.context["mapping_review"]
                with patch(
                ...
                self.assertContains(rendered, expected)
                self.assertContains(rendered, "Bob Visible")
                self.assertNotContains(rendered, self.bob.email)
                self.assertNotContains(rendered, self.bob.notes)
                self.assertNotContains(rendered, "confirmation_note")
                self.assertEqual(
                    rendered.context["confirmation_proposal"].is_roster_update,
                    True,
                )

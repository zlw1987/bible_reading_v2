...
Status: SE-AS.3 is complete as docs-only planning. SE-AS.4 is complete as the runtime visibility rule that first introduced audience-row visibility with a zero-row legacy fallback at that time. SE-AS.5 is complete as the staff selector UI/display: staff can select optional `ChurchStructureUnit` audience rows on single create/edit and recurring create; staff detail shows effective audience source and readable labels. Historical note: at SE-AS.4/SE-AS.5 time, audience-row matching was not yet membership-core; CS-CORE.2B-A later switched ServiceEvent audience-row matching to active primary `ChurchStructureMembership`, SE-RETIRE.1B later retired the zero-row legacy runtime fallback so zero-row events now fail closed for ordinary users, and SE-FIELD-RETIRE.1A later removed the legacy `scope_type` / `district` / `small_group` fields. SE-AS.6B is complete as an audit-only dry-run command, including SE-AS.6B.1 verbose output polish. SE-AS.6C is complete as an explicit `--apply` mode on the same command (dry-run remains the default; apply creates `ServiceEventAudienceScope` rows only for parity-safe `would-create` events, never mutating legacy fields, and is idempotent). **SE-AS.6C production apply is complete:** it has been run against production, so all 37 production ServiceEvents now have audience rows; the post-apply dry-run reports skipped-existing-rows 37, would-create 0, parity-mismatch 0, and legacy-fields-mutated 0 (details below). SE-AS.7A is complete as the historical write-path guard that stopped normal create/edit/recurring flows from saving new zero-row legacy-fallback events at that time (details below); SE-SCOPE.1A later made those normal app write paths structure-audience-row native, and SE-FIELD-RETIRE.1A later removed the legacy fields. No setup/edit UI, CS-MAP.3, CS-SETUP.1, Community Activities, schema change, or migration was added by SE-AS.3/SE-AS.4.

SERVICE-EVENT-CONTEXT.1C status: complete. The legacy `ServiceEvent.ministry_context` Host / Language display FK has been removed (migration `events/0008`) after prior SERVICE-EVENT-CONTEXT slices stopped normal app writes, added `ServiceEvent.host_language_unit`, and local/dev audit confirmed zero populated `ministry_context` rows. Host / Language display now uses `ServiceEvent.host_language_unit` and, when that is blank, the audience-derived structure fallback (walking `ServiceEventAudienceScope` units up to the nearest ministry-context unit). The display-only cleanup/backfill tooling (`backfill_service_event_host_language_units`, `cleanup_service_event_ministry_context_labels`) was retired with the field. ServiceEvent visibility remains `ServiceEventAudienceScope` rows plus active primary `ChurchStructureMembership`; zero-row events stay fail-closed for ordinary users. This did not remove the `MinistryContext` table, `MinistryContext.church_structure_unit`, `ServiceEvent.host_language_unit`, or `ServiceEventAudienceScope`, and did not touch Prayer, Role, Reflection, Bible Study, Profile, SmallGroup, District, or TeamAssignment schema.
...
SE-AS.7A is complete as the historical write-path guard that stopped new zero-row legacy-fallback events by converting empty-picker saves from legacy scope fields into audience rows. SE-SCOPE.1A superseded the normal app form portion of that design, and SE-FIELD-RETIRE.1A later removed `ServiceEvent.scope_type`, `district`, and `small_group` after cleanup readiness was confirmed. Normal app saves now require at least one selected structure audience unit instead of converting empty selections through legacy fields or saving normal app zero-row events. When staff select audience units, those `ServiceEventAudienceScope` rows are the structure-native source. SE-RETIRE.1B remains the runtime rule: zero-row events fail closed for ordinary users, while manager/staff override and audience-row membership-core visibility are unchanged.
...
- `ServiceEvent.host_language_unit` — Host / Language Label / 主办/语言标签 only. Display label; never visibility, serving, or permissions. The legacy `ServiceEvent.ministry_context` display FK was removed in SERVICE-EVENT-CONTEXT.1C (migration `events/0008`); Host / Language display now uses `host_language_unit` plus the audience-derived structure fallback.
...
- Historical/superseded Bible Study pattern: `BibleStudySeriesAudienceScope` was the first narrow runtime consumer, and early generation resolved selected units to active legacy `SmallGroup` rows. Current normal V2 generation is structure-native: selected schedule units expand to active descendant/self small-group `ChurchStructureUnit` leaf targets with `generation_key`, `anchor_unit`, and meeting audience rows; ordinary visibility uses meeting audience rows plus active primary `ChurchStructureMembership`.
...
| Host / Language Label / 主办/语言标签 | `ServiceEvent.host_language_unit` plus audience-derived structure fallback. SERVICE-EVENT-CONTEXT.1C removed the legacy `ServiceEvent.ministry_context` display FK (migration `events/0008`); only immutable historical migrations still name it. | Display label only. Never visibility. |
...
- Historical/superseded: early unit matching reused the studies-level resolver wrapper so ServiceEvent and Bible Study Schedule shared the same `ChurchStructureUnit` to legacy `SmallGroup` resolution semantics. Current ServiceEvent audience matching uses active primary membership through structure audience rows, and current normal Bible Study V2 generation is structure-native.
...
**Scope note (CS-CORE.2B-A correction; later superseded for Bible Study too).** This section describes the historical **legacy `Profile.small_group` resolver** (`resolve_units_to_small_groups`) only. As shipped at SE-AS.4 this was also how ServiceEvent audience rows matched, but **that is no longer current ServiceEvent behavior**: as of CS-CORE.2B-A a ServiceEvent that has audience rows matches ordinary users through the user's single active primary `ChurchStructureMembership` via `accounts.structure_selectors.user_matches_structure_audience`, **not** through `Profile.small_group`. Later Bible Study slices also retired normal V2 generation's dependency on this resolver; current normal V2 generation is structure-native. As of SE-RETIRE.1B the ServiceEvent zero-row legacy fallback is retired and zero-row events fail closed for ordinary users (Section 14). Read this section as historical resolver semantics, not as any current ServiceEvent or normal V2 Bible Study runtime.
...
- **Resolved (SE-RETIRE.1B / SE-FIELD-RETIRE.1A).** The zero-row runtime fallback in `ServiceEvent.can_be_seen_by` is retired: zero-row events now fail closed for ordinary users (Section 14). SE-AS.7A stopped new zero-row writes at that time; SE-RETIRE.1A gated the decision with a clean production audit; SE-RETIRE.1B removed the runtime fallback; and SE-FIELD-RETIRE.1A later removed the legacy `scope_type` / `district` / `small_group` fields.

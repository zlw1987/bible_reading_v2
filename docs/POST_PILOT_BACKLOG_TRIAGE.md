...
- CS-CORE.1C Bible Study Resolver Re-home / Parity Closure. Completed; Bible Study uses the shared selector resolver through the compatibility wrapper, meeting generation targets are unchanged, and member visibility remains `Profile.small_group`-based.
...
- CS-CORE.2C-A Bible Study Membership-Core Readiness / Shadow Audit. Completed; read-only shadow comparison helpers and audit command only, with Bible Study runtime visibility unchanged and still `Profile.small_group`-driven.
...
- CS-CORE.1C is a parity closure only: Bible Study structure-audience schedules still resolve selected units to legacy `SmallGroup` rows for meeting generation through the shared selector layer; ordinary member meeting visibility still uses `Profile.small_group`; `ChurchStructureMembership` still grants no Bible Study eligibility or visibility.
- CS-CORE.2B-A is one consumer switch, not legacy retirement: ServiceEvent structure-audience matching (audience rows present) now uses the active primary `ChurchStructureMembership`; ServiceEvents with no audience rows still use legacy fallback and `Profile.small_group`; Bible Study meeting generation and member visibility, reading progress, reflection/comment privacy, group progress permissions, TeamAssignment/My Serving, and roles are unchanged; legacy `Profile.small_group`, `SmallGroup`, `District`, and `MinistryContext` are not retired.

...
- CS-CORE.2C-A is complete: a read-only Bible Study membership-core readiness/shadow audit (`studies/structure_readiness.py` helpers plus the `audit_bible_study_membership_readiness` command). CS-CORE.2C-B is complete: Bible Study v2 `BibleStudyMeeting` ordinary-member visibility and the `/studies/` / Today v2 meeting pre-filter now use active primary `ChurchStructureMembership`; `Profile.small_group` alone no longer grants v2 meeting visibility. Bible Study meeting generation still uses legacy `SmallGroup`, Bible Study schedule audience resolution still uses legacy mappings, and legacy `BibleStudySession` is unchanged as a retirement target. At 2C-B time `Profile.small_group` still drove reading/progress/privacy; those have since switched consumer-by-consumer (group-progress visible roster in CS-CORE.4F.1, permission-fenced default selected group in CS-CORE.4F.2, ordinary-member group reflection **read** path in CS-CORE.4G.2, reflection **write** path/forms in CS-CORE.4G.3). Still legacy: the group-progress permission/accessible group list. No legacy fields/tables are retired, and the readiness audit now compares old legacy Bible Study meeting visibility against current membership-core v2 meeting visibility.
...
- CS-CORE.4F.1/4F.2 are complete: group-progress visible roster and permission-fenced no-`?group=` default selected group switched to membership-core; group-progress permission and accessible group list stay legacy. CS-CORE.4G.1 is complete as a read-only reflection-snapshot readiness audit. **CS-CORE.4G.2 is complete: the ordinary-member group reflection read path (`ReflectionComment.can_be_seen_by`, `reading.views.get_visible_reflection_filter`, `passage_wall` group tab) switched to `structure_unit_at_post` + active primary `ChurchStructureMembership`; `Profile.small_group` / `small_group_at_post` no longer grant ordinary group reflection visibility; missing/inactive/wrong-type snapshot fails closed.** CS-CORE.4G.2A reconciled the docs and the field `help_text` (help_text-only migration `comments/0006_…`). **CS-CORE.4G.3 is complete: the reflection write path / form option-gating (`ReflectionCommentForm`, `ReflectionCommentEditForm`, `add_comment`, `edit_comment` via `get_user_group_reflection_write_context`) switched to the same membership-core source; `Profile.small_group` alone no longer offers or stamps group sharing; `structure_unit_at_post` is the canonical read/write snapshot; `small_group_at_post` is an optional legacy mirror; existing group posts preserve snapshots under Policy C.** The group-progress permission/access list remains legacy as the next sequenced slice.
- CS-CORE.2D-A is complete as the **structure-aware role-scope foundation** (model/helper/audit only): added a nullable `ChurchRoleAssignment.structure_unit` FK (migration `accounts/0009_churchroleassignment_structure_unit`) as the canonical future role scope while the legacy `district` / `small_group` role fields stay as compatibility scope fields; added the read-only `get_role_assignment_structure_unit` / `assignment_scope_includes_unit` helpers; and added the read-only `audit_structure_role_scopes` readiness command. **No progress-permission/access-list runtime switch:** `get_accessible_progress_groups()` / `can_view_group_progress_for()` stay legacy-driven, and ordinary `ChurchStructureMembership` still grants no progress permission. This unblocks the next slice (CS-CORE.2D-B) that switches the group-progress permission/access list to structure-aware role scopes; no group-progress roster/default, ServiceEvent, Bible Study, or reflection behavior changed.
...
- **Group-progress permission / accessible group list migration.** Switch `accounts.permissions.get_accessible_progress_groups()` / `can_view_group_progress_for()` off legacy `ChurchRoleAssignment` district/small-group scopes. Needs a separate structure-aware role/permission-model decision (CS-CORE.2D territory); ordinary `ChurchStructureMembership` must never confer progress permission.
- **Structure-aware role scopes (CS-CORE.2D).** Unit-scoped role scopes; the prerequisite decision for the group-progress permission migration above.
...
- CS-CORE.2B-A was the first consumer switch (ServiceEvent structure-audience matching → active primary `ChurchStructureMembership`; ServiceEvents with no audience rows still use legacy fallback and `Profile.small_group`). Since then, additional consumers have switched to membership-core: Bible Study v2 meeting visibility and the `/studies/` / Today v2 pre-filter (CS-CORE.2C-B), the group-progress visible roster (CS-CORE.4F.1) and permission-fenced default selected group (CS-CORE.4F.2), the ordinary-member group reflection **read** path (CS-CORE.4G.2), and the reflection **write** path/forms (CS-CORE.4G.3). Still legacy/pending: the group-progress permission/accessible group list, ServiceEvent zero-row fallback, Bible Study v2 meeting generation, structure-aware role scopes, TeamAssignment/My Serving (serving is never inferred from structure membership), and the legacy `Profile.small_group` / `SmallGroup` / `District` / `MinistryContext` compatibility layer (not retired).

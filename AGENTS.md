...
* The active V1 Django Admin surface was retired in `BS-V1-ADMIN-RETIRE.1A`: `BibleStudySessionAdmin` and the V1-only `BibleStudyGuideAdmin` / `BibleStudyWorshipSongAdmin` were unregistered in `studies/admin.py`. This was admin-only — no V1 data was deleted, no V1 model/table/field was removed, and the guarded purge was not applied. The schema-retirement audit now classifies V1 `BibleStudySession` as `blocked_by_diagnostic_tooling` (purge/audit tooling, test fixtures, immutable historical migrations) rather than `blocked_by_display_or_admin`. V1 model/table/schema removal remains a later separate slice. Do not re-register these admins; the active V2 admin surface (`BibleStudyMeeting`) is unaffected.
* The legacy parent FKs `SmallGroup.district` and `District.ministry_context` were dropped from the Django admin display in `LEGACY-OBJECT-ADMIN-FK.1A`: removed from `SmallGroupAdmin` / `DistrictAdmin` `list_display`/`list_filter`/`search_fields`/`fieldsets` in `accounts/admin.py`. This was admin-display-only — the admins stay registered, the `church_structure_unit` mapping bridge and `mapping_status` are unchanged, no field/model/table was removed, no migration was generated, and no data/runtime behavior changed. Parent/context links were already clear (`cleanup_legacy_structure_parent_links` reports 0 `SmallGroup.district` / `District.ministry_context` links present). The schema-retirement audit now reports no `admin_references` for these two parent FKs; with zero populated values they are no longer `blocked_by_display_or_admin` and advance to `blocked_by_diagnostic_tooling` (only the parent-link/seed diagnostic tooling and the legacy bridge resolver read remain). The staff `structure_mapping_review` / `structure_mapping_edit` maintenance views were intentionally left in place because they still maintain the live `church_structure_unit` bridge. Do not re-add `district` / `ministry_context` to these admins. This did not remove the `SmallGroup` / `District` / `MinistryContext` tables or the `church_structure_unit` mapping FKs.
* The legacy parent/context FK fields `SmallGroup.district` and `District.ministry_context` were removed in `LEGACY-PARENT-FK-FIELD-RETIRE.1A` (migration `accounts/0013`) after `LEGACY-OBJECT-ADMIN-FK.1A/1B` retired their admin display, `SE-EVENT-LEGACY-WARNING-RETIRE.1A` and `LEGACY-BRIDGE-RESOLVER-NARROW.1A` retired the resolver fallback reads, and target-DB dry-run confirmed parent/context links already clear (`small_group_district_links_present: 0`, `district_ministry_context_links_present: 0`, both `data_blocker_count=0`). The canonical structure hierarchy is `ChurchStructureUnit.parent` reached via the `church_structure_unit` mapping bridge. The guarded `cleanup_legacy_structure_parent_links` command and its test module — whose only purpose was to clear these two fields before removal — were retired with the fields, and the schema-retirement audit reclassifies both fields as historical-only with no active blocker. `seed_church_structure_units` no longer reconstructs the `District` / `SmallGroup` hierarchy from removed legacy links: `MinistryContext` units (parent always the root) are still seeded, already-mapped `District` / `SmallGroup` rows keep their authoritative `ChurchStructureUnit.parent`, and unmapped rows are reported as `unreconstructable` (needing manual placement) instead of being silently reparented to `UNASSIGNED-DISTRICTS` / `UNASSIGNED-GROUPS` holding units. Only immutable historical migrations still name the two fields. This did not remove the `SmallGroup` / `District` / `MinistryContext` rows/tables or the `church_structure_unit` mapping FKs. Do not reintroduce these parent FKs.

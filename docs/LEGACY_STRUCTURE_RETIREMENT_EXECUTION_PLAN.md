...
- ServiceEvent ordinary-user visibility no longer uses zero-row legacy fallback; zero-row events are fail-closed safety states, and SE-SCOPE.1A stops normal app-level writes to legacy `scope_type` / `district` / `small_group`. Existing stored values remain admin/display/audit/backfill/rollback data and cleanup blockers.
...
| ServiceEvent legacy field removal | `ServiceEvent.scope_type`, `district`, `small_group` | candidate for later field-level removal | SE-SCOPE.1A stops normal app writes, but existing stored values remain blockers. Remove only after stored data, rollback, admin forms, and backfill diagnostics have a separate approved plan. |
...
- Non-global or populated legacy fields block field-level retirement until a separate guarded cleanup/deprecation/removal plan handles existing stored data, admin forms, displays, backfill diagnostics, rollback, and schema removal. SE-SCOPE.1A prevents new normal app writes; it does not bulk-clear old values.

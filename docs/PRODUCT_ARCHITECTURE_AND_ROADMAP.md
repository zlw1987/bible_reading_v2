...
Future flexible Church Structure Foundation planning should keep current `MinistryContext`, `District`, `SmallGroup`, and `Profile.small_group` assumptions usable while leaving room for CM/EM, variable-depth branches, arbitrary future structure units, and future membership history. Long-term source of truth should be `ChurchStructureUnit` plus `ChurchStructureMembership`; short-term runtime behavior still uses existing models. Nullable legacy mapping fields, verified command-based seeding/mapping, seeded data QA closure, membership design, a model-only membership table, helper hardening, explicit command-based membership backfill, user-attested production/staging backfill verification, clearer Django Admin labeling, signup/Profile requested-unit capture, staff request review, approve/reject actions, narrow approval sync, SE-AS.1 ServiceEvent audience-scope redesign planning, SE-AS.2 model-only `ServiceEventAudienceScope` foundation, and SE-AS.4 ServiceEvent runtime visibility with legacy fallback now exist. For ServiceEvents, `ServiceEventAudienceScope` rows are the ordinary-user visibility source when present; events with no rows still use legacy `scope_type` / `district` / `small_group` plus `Profile.small_group`, and legacy fields remain preserved as fallback. `ChurchStructureMembership` still does not grant ServiceEvent visibility. Bible Study Schedule audience scope (DOCS-AS.1 direction, implemented as BS-AS.1 / BS-AS.2 / BS-AS.2A) remains a narrow runtime consumer: `BibleStudySeriesAudienceScope` joins `BibleStudySeries` to `ChurchStructureUnit`, selected units resolve to eligible legacy `SmallGroup` rows for meeting generation, generated `BibleStudyMeeting` rows still point to legacy `SmallGroup`, ordinary member visibility still uses `Profile.small_group`, and `ChurchStructureMembership` runtime visibility migration remains deferred. The SE-AS.5 staff audience UI selector, audience editing/display workflow, broad custom staff admin expansion, consumer migration from legacy fields/`Profile.small_group`, and a runtime source-of-truth switch are still future and not authorized here. See `docs/CHURCH_STRUCTURE_FOUNDATION_PLAN.md`, `docs/SERVICE_EVENT_AUDIENCE_SCOPE_REDESIGN_PLAN.md`, and `docs/SERVICE_EVENT_AUDIENCE_RUNTIME_MIGRATION_PLAN.md`.
...
- SE-AS.5, Community Activities, CS-MAP.3, and CS-SETUP.1 are not pulled forward by SE-AS.4 completion.
...
- Later: SE-AS.5 staff audience UI/display and Community Activities should reuse the same `ChurchStructureUnit` audience-scope foundation; both remain deferred and require separate approval before implementation.
...
- Later SE-AS.5 staff audience UI selector, audience editing/display workflow, and ServiceEvent consumer migration only after separate approval (numbering per `docs/SERVICE_EVENT_AUDIENCE_RUNTIME_MIGRATION_PLAN.md`).
...
PV-C.1 records pilot validation closure. Pilot validation passed on `v0.9-pilot-rc1`. CS-H.1 through CS-H.10, PP-SA.2 through PP-SA.5, MO-S.1 through MO-S.5B, SE-AS.1 through SE-AS.4, SE-AS.5A, DOCS-AS.1, BS-AS.1 through BS-AS.2A, and CS-MAP.2 are complete. SE-AS.4 is complete as the ServiceEvent runtime visibility rule with legacy fallback: events with `ServiceEventAudienceScope` rows use those rows for ordinary-user visibility, events with no rows keep legacy `scope_type` / `district` / `small_group` plus `Profile.small_group` behavior, `ChurchStructureMembership` still does not grant ServiceEvent visibility, and legacy fields remain preserved as fallback. SE-AS.5A is complete as docs-only interaction planning for the future staff audience selector: legacy fields stay editable as fallback audience settings, empty picker means legacy fallback, selected structure rows govern visibility without legacy fields acting as an extra filter, one selected audience set applies to all events in recurring create, and no backfill/migration/deprecation or ministry scheduling behavior change is authorized. SE-AS.4/SE-AS.5A did not add the SE-AS.5 staff audience selector, any ServiceEvent form/template audience picker, Community Activities, CS-MAP.3, CS-SETUP.1, schema changes, migrations, consumer migration, or changes to ServiceEvent MinistryContext, Required Ministry Teams, Rotation Anchor Team, TeamAssignment, or My Serving behavior. Runtime consumers still primarily use legacy models and `Profile.small_group`; implementation beyond the completed ServiceEvent runtime fallback rule, SE-AS.5A interaction plan, and Bible Study Schedule audience scope remains future and phased. See `docs/POST_PILOT_BACKLOG_TRIAGE.md`, `docs/FLEXIBLE_CHURCH_STRUCTURE_AND_AUDIENCE_SCOPE_DESIGN.md`, `docs/CHURCH_STRUCTURE_MAPPING_AND_MEMBERSHIP_STRATEGY.md`, `docs/CHURCH_STRUCTURE_SEEDING_VERIFICATION.md`, `docs/CHURCH_STRUCTURE_MEMBERSHIP_BACKFILL_VERIFICATION.md`, `docs/CHURCH_STRUCTURE_MEMBERSHIP_DESIGN.md`, `docs/STAFF_ADMIN_SURFACE_EXPANSION_PLAN.md`, `docs/MINISTRY_SCHEDULING_REQUIREMENTS_PLAN.md`, `docs/SERVICE_EVENT_AUDIENCE_SCOPE_REDESIGN_PLAN.md`, `docs/SERVICE_EVENT_AUDIENCE_RUNTIME_MIGRATION_PLAN.md`, and `docs/SERVICE_EVENT_AUDIENCE_SELECTOR_INTERACTION_PLAN.md`.

Future foundation planning:

`ChurchStructureUnit` seeding/mapping now exists only as an explicit management command, passed GoDaddy production/staging verification, and completed seeded structure data QA closure. SE-AS.1 records the docs-only `ServiceEvent` audience-scope redesign recommendation; SE-AS.2 adds the `ChurchStructureUnit`-linked audience scope beside legacy fields as a model-only foundation; SE-AS.4 makes those rows the ServiceEvent ordinary-user visibility source only when rows exist, with zero-row events falling back to legacy `scope_type` / `district` / `small_group` plus `Profile.small_group`. Consumer migration remains future phased work before advanced mixed audience segments or CM/EM-aware `ServiceEvent` filtering. CS-F.3 is not filtering; it is only an optional ServiceEvent label.

Large deferred items remain deferred pending feedback. MO-S.4 now supports manual team-leader scheduling, MO-S.4A completed scheduling semantic cleanup, MO-S.5A/MO-S.5B completed bounded rotation-anchor and copy-forward helper work, SE-AS.1 completed docs-only audience-scope planning, SE-AS.2 completed a model-only ServiceEvent audience-scope data foundation, SE-AS.3 completed the docs-only ServiceEvent Audience Runtime Migration Plan, SE-AS.4 completed runtime visibility with legacy fallback, and BS-AS.1 / BS-AS.2 / BS-AS.2A completed Bible Study Schedule audience scope as the first narrow runtime audience-scope consumer. The SE-AS.5 staff audience UI selector, audience editing/display workflow, ServiceEvent consumer migration, Community Activities, notifications, attendance, automatic scheduling, availability, swaps, reminders, and Checklist V1 remain deferred unless separately planned.

...
| Sunday Ministry Scheduling | [`SUNDAY_MINISTRY_SCHEDULING_PLAN.md`](SUNDAY_MINISTRY_SCHEDULING_PLAN.md) | Current MO-S.6 plan through implemented Board/context, Campus, Worship pool, event-planner, governed exact-event selection, operational reachability, rotation-planner preview/confirmation/shared audit, bounded `NOTIFY.1G` direct-change notifications, and the committed optional stable ServiceEvent profile-key foundation `MO-S.6D-PROFILE.1A`; the read-only `MO-S.6D-PROFILE-SETUP.0A` audit is implemented in the current working tree pending product-owner review/commit, while Excel preview/import runtime remains separately scoped and blocked on production schema/audit review plus separately authorized explicit target-event setup evidence. |
| Worship Rotation Governance | [`WORSHIP_ROTATION_GOVERNANCE_PLAN.md`](WORSHIP_ROTATION_GOVERNANCE_PLAN.md) | Canonical multi-campus Worship invariants through implemented exact-event governance, operational reachability, planner confirmation/shared audit, bounded `NOTIFY.1G`, and stable ServiceEvent profile identity; import remains a separately approved later slice. |

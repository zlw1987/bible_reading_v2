...
Status: SE-AS.3 is complete as docs-only planning. SE-AS.4 is complete as the runtime visibility rule that first introduced audience-row visibility with a zero-row legacy fallback at that time. SE-AS.5 is complete as the staff selector UI/display: staff can select optional `ChurchStructureUnit` audience rows on single create/edit and recurring create; staff detail shows effective audience source and readable labels. Historical note: at SE-AS.4/SE-AS.5 time, audience-row matching was not yet membership-core; CS-CORE.2B-A later switched ServiceEvent audience-row matching to active primary `ChurchStructureMembership`, and SE-RETIRE.1B later retired the zero-row legacy runtime fallback so zero-row events now fail closed for ordinary users. Legacy scope fields remain preserved/editable as display/admin/backfill/audit/rollback data until field-level retirement. SE-AS.6B is complete as an audit-only dry-run command, including SE-AS.6B.1 verbose output polish. SE-AS.6C is complete as an explicit `--apply` mode on the same command (dry-run remains the default; apply creates `ServiceEventAudienceScope` rows only for parity-safe `would-create` events, never mutating legacy fields, and is idempotent). **SE-AS.6C production apply is complete:** it has been run against production, so all 37 production ServiceEvents now have audience rows; the post-apply dry-run reports skipped-existing-rows 37, would-create 0, parity-mismatch 0, and legacy-fields-mutated 0 (details below). SE-AS.7A is complete as the write-path guard that stops normal create/edit/recurring flows from saving new zero-row legacy-fallback events (details below). No setup/edit UI, CS-MAP.3, CS-SETUP.1, Community Activities, schema change, or migration was added.
...
- Legacy scope fields remain editable and are visually grouped/labeled as stored fallback-context settings during the transition; an event with audience rows is governed by those rows. Since SE-RETIRE.1B, an event without rows fails closed for ordinary users rather than keeping legacy runtime behavior.
...
- **Later — legacy scope field retirement planning (future, separate approval).** Plan-only evaluation of eventual legacy `scope_type` / `district` / `small_group` field deprecation/removal; no destructive change until display/admin/backfill/audit/rollback needs are accepted.
...
- Non-goals (unchanged by SE-AS.7A): legacy `scope_type` / `district` / `small_group` fields are not deleted; Bible Study, TeamAssignment / My Serving, and required-team logic are unchanged; no schema, migration, or data migration was added. SE-RETIRE.1B later removed the runtime zero-row fallback in `ServiceEvent.can_be_seen_by`; zero-row events created outside these guarded forms now fail closed for ordinary users.
...
- **SE-AS.7A** — stop new zero-row legacy-fallback writes (implemented; see Section 5). Normal create/edit/recurring write paths convert an empty audience selection from the legacy fields into a structure audience row, or reject the save when the mapping is missing/inactive/ambiguous.
- **SE-RETIRE.1B** — retire the zero-row legacy runtime fallback for ordinary users (implemented; see Section 14). Zero-row events now fail closed; legacy fields remain stored/editable as display/admin/backfill/audit/rollback data.
- **Later** — legacy scope field retirement planning. No field hiding/removal before rollback and display/admin needs are separately approved.

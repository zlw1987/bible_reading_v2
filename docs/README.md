...
| Generic deployment configuration | [`GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md`](GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md) | Frozen architecture; `GENERIC-DEPLOYMENT-CONFIG.1A` locally verifies only the additive nullable Ministry Team key/setup/inventory foundation with no backfill or runtime consumer. Service Profiles, defaults/materialization, opt-in deployment adapters, and future external-system mapping remain unimplemented. |

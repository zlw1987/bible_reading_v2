...
|---|---|---|---|---|---|---|---|
| `AUDIT-DATA-001` | Active/current primary membership integrity and concurrency hardening | `MEDIUM` | Open | `ChurchStructureMembership.clean()` rejects conflicting active primary rows at the model/application layer. `accounts/structure_selectors.get_user_primary_membership_unit()` is date-window aware (`start_date <= target_date` and null-or-future `end_date`) and fails closed when multiple current active primary rows exist. `approve_membership_request`, `set_primary_structure_membership`, and delegated My Units approval are current mutation paths to evaluate. Existing tests intentionally construct ambiguous current-primary drift through bulk operations to prove fail-closed selectors. | Bulk operations, manual SQL, import drift, or concurrent mutation paths can bypass model validation and produce ambiguous current-primary belonging. Runtime fail-closed behavior protects visibility semantics, but affected users can lose scoped content until repaired. A simple `status=active AND is_primary=True` partial unique constraint may be stricter than the current lifecycle semantics because current membership also depends on dates; the exact database-enforceable invariant has not yet been established. | Run a design-first hardening slice. First determine the intended lifecycle invariant, backend/portability needs, all primary-membership mutation paths, whether application transaction/row-locking should be hardened, whether a read-only drift/readiness check is needed, whether any DB constraint can safely express the invariant, and whether a migration is necessary at all. Do not authorize a migration from this audit document. | `STRUCTURE-MEMBERSHIP-PRIMARY-INTEGRITY-HARDENING.1A` |

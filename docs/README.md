...
| Generic deployment configuration | [`GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md`](GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md) · [`Slice 5 implementation plan`](GENERIC_DEPLOYMENT_CONFIGURATION_SLICE5_PLAN.md) | Frozen architecture; 1A/2A implement Ministry Team key identity/configuration tooling. 3A adds ServiceProfile/FK schema, and 4A is production-applied/post-audit-verified for one reviewed SVCA mapping (one profile, 52 exact dual-consistent events, zero drift, revisions `1 -> 2` once). 5A-5F complete the default-off integration boundary and FK-authoritative consumer closeout. 6A/6B provide the ministry-owned static-default foundation and production-verified reviewed configuration. `GENERIC-DEPLOYMENT-CONFIG.7A` is **PRODUCTION READ-ONLY PREVIEW VERIFIED**. **`GENERIC-DEPLOYMENT-CONFIG.7B — PRODUCTION MATERIALIZATION APPLY COMPLETE / VERIFIED`**: the reviewed `2026-09-08` through `2026-12-31` scope has 16 events, 64 expected static-default pairs, 64 explicit pairs after apply, zero missing pairs, and zero blockers; 60 rows were created across 15 changed events and a fresh no-token dry-run proved idempotency. **`GENERIC-DEPLOYMENT-CONFIG.7C-1A — IMPLEMENTED / LOCAL VERIFIED`**: ordinary single and recurring creation support optional reviewed ServiceProfile default suggestions, persist the user's exact explicit RequiredTeam set through one atomic revision-zero creation service, and retain profileless creation; SQLite first-write/post-boundary recomputation fails stale without partial writes. **`MO-S.REQUIRED.1A — IMPLEMENTED / LOCAL VERIFIED / PRODUCTION RUNTIME VERIFIED`** adds the read-only effective requirement union of explicit rows plus the exact canonically eligible selected Worship Team, with exact-ID provenance/de-duplication, separate conflict/review evidence, effective coverage/gap/authorized-attention consumers, single dedicated Worship projections on Team Schedule/Sunday Board, and a bounded Event-detail readout; it changes no persisted-row writer, permission, schema, migration, or production data. The owner-run read-only production projection verified 64 explicit + 16 eligible derived Worship = 80 effective pairs across 16 future reviewed events, all with `review_required = 0`, and made no production write. `selected_unscheduled` is a valid operational gap pending a matching Worship assignment, not configuration drift. Historical backfill remains deliberately deferred; Admin/reset/adapters and the 7B closeout are unchanged. `LIGHTING-PILOT-RETIRE.1A` remains implemented; external mapping is separately deferred. **`GENERIC-DEPLOYMENT-CONFIG.LEGACY-SERVICE-PROFILE-KEY-RETIRE.0A — AUDIT / ARCHITECTURE COMPLETE`**, **`.1A — PRODUCTION VERIFIED`**, and **`.2A — PRODUCTION APPLIED / VERIFIED`** fully retire `ServiceEvent.service_profile_key`; `ServiceProfile.key` remains permanent. |
...
**PRODUCTION APPLIED / VERIFIED**. Production `events/0013` removed
`ServiceEvent.service_profile_key`; current identity is the optional protected
FK to permanent `ServiceProfile.key`, and the Stage-1 pre-drop command mode is
retired. Pre-removal evidence was 52 exact historical residues with zero
blockers; post-migration evidence is 52/52 FK-linked with zero type/integrity
blockers, passing `manage.py check`, no compatibility column, and the canonical
FK still present. The planned immediately pre-migration manual SQLite backup
was not performed because deployment automatically migrated; this operational
sequencing deviation is recorded without claiming a hosting snapshot. Schema
reversal cannot reconstruct dropped legacy values.

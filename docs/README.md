...
| Sunday Ministry Scheduling | [`SUNDAY_MINISTRY_SCHEDULING_PLAN.md`](SUNDAY_MINISTRY_SCHEDULING_PLAN.md) | Current MO-S.6 plan through implemented Board/context, Campus, Worship pool, event-planner, governed exact-event selection, operational reachability, rotation-planner preview/confirmation/shared audit, bounded `NOTIFY.1G` direct-change notifications, and the optional stable ServiceEvent profile-key foundation `MO-S.6D-PROFILE.1A`; Excel preview/import runtime remains separately scoped and blocked on foundation commit plus explicit target-event setup evidence. |

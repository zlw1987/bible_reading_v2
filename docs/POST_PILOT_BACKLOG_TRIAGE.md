...
- CS-MAP.2 Read-Only Staff Structure Map + Mapping Health. Completed at `/staff/structure/` as a permission-protected read-only staff page: active `ChurchStructureUnit` hierarchy, descendant-inclusive covered-member counts, current data mapping context from active legacy rows, setup-readiness indicators including direct active primary memberships on parent units, no member rosters, no schema/migration/runtime visibility changes, and no broad setup/edit UI. CS-MAP.3 remains optional/unapproved; broad CS-SETUP.1 setup/edit UI remains gated, with only CS-SETUP.1B rename-only edit, CS-SETUP.1C.1/1C.2 mapping review, CS-SETUP.1D.1 permitted one-row mapping edit, and CS-SETUP.1D.2 display-only warning/conflict polish complete.
...
42. BS-AS QA follow-up completed; BS-AS.2B fixed the audience picker mobile CSS no-go. CS-MAP.2 read-only Staff Structure Map + Mapping Health is completed at `/staff/structure/`; CS-SETUP.1D.1/1D.2 complete only the narrow mapping-maintenance/review-polish slices on `/staff/structure/mappings/`; SE-AS.4 is completed as the ServiceEvent runtime audience rule only, and CS-MAP/CS-SETUP work must not be bundled with SE-AS.5 or Community Activities; CS-MAP.3 remains optional/unapproved and broad CS-SETUP.1 setup/edit UI remains unapproved.

...
- `ChurchStructureMembership` (`accounts/models.py`): belonging foundation with requested/active/ended/rejected/cancelled lifecycle, one-active-primary-per-user enforcement, date windows, approval audit fields, and `active_for_user()` / `current_primary_for_user()` helpers. It is the runtime source for ServiceEvent audience-row matching after CS-CORE.2B-A and Bible Study v2 `BibleStudyMeeting` ordinary-member visibility after CS-CORE.2C-B. It is not the runtime source for Bible Study generation, legacy `BibleStudySession`, reading/progress/privacy, permissions, roles, My Serving, or legacy-fallback ServiceEvents. Requested/rejected/ended/cancelled rows grant nothing.
...
- Bible Study Schedule is the first narrow runtime consumer: `BibleStudySeriesAudienceScope` (`studies/models.py`) resolves selected units to eligible legacy `SmallGroup` rows for meeting generation. Since CS-CORE.2C-B, Bible Study v2 `BibleStudyMeeting` ordinary-member visibility uses active primary `ChurchStructureMembership`; `Profile.small_group` alone no longer grants v2 meeting visibility.
- `ServiceEventAudienceScope` (`events/models.py`) is now a runtime visibility source for ServiceEvent (SE-AS.4/SE-AS.5 complete, with CS-CORE.2B-A membership-core matching): an event with one or more audience rows matches ordinary users by active primary `ChurchStructureMembership`, and an event with zero rows falls back to legacy `scope_type` / `district` / `small_group` and `Profile.small_group` (see `docs/SERVICE_EVENT_AUDIENCE_RUNTIME_MIGRATION_PLAN.md` and `docs/CHURCH_STRUCTURE_CORE_MIGRATION_PLAN.md`).
...
Ordinary-user matching is now consumer-specific. Bible Study v2 `BibleStudyMeeting` visibility and ServiceEvent audience-row visibility use active primary `ChurchStructureMembership`; ServiceEvent zero-row legacy fallback, legacy `BibleStudySession`, reading/progress/privacy, My Serving, roles, and other legacy consumers still depend on legacy `Profile.small_group` where they did before. Bible Study Schedule uses structure audience rows plus legacy mapping resolution for meeting-generation eligibility.
...
| Church Structure Membership | `ChurchStructureMembership` | Runtime source for ServiceEvent structure-audience row matching after CS-CORE.2B-A and Bible Study v2 `BibleStudyMeeting` ordinary-member visibility after CS-CORE.2C-B. Not the runtime source for Bible Study generation, legacy `BibleStudySession`, reading progress, reflection/comment privacy, group progress permissions, roles, TeamAssignment / My Serving, or legacy-fallback ServiceEvents. Requested/rejected/ended/cancelled rows grant nothing. |
| Audience Scope / 适用范围 | `BibleStudySeriesAudienceScope` (runtime for meeting generation), `ServiceEventAudienceScope` (runtime for ServiceEvent visibility when rows exist, else legacy fallback); legacy scope fields elsewhere | Who an event/schedule is for. Per-module join models selecting units; ServiceEvent audience rows match those units against active primary `ChurchStructureMembership` after CS-CORE.2B-A, while Bible Study schedule generation still resolves selected units through legacy mappings. |
...
| Legacy `Profile.small_group` | `Profile.small_group` | Current runtime belonging source for legacy `BibleStudySession`, reading/progress/privacy consumers, and ServiceEvent zero-row legacy fallback; no longer grants ServiceEvent structure-audience row visibility after CS-CORE.2B-A or Bible Study v2 `BibleStudyMeeting` visibility after CS-CORE.2C-B. |
...
| Risk | Mitigation |
| --- | --- |
| The map makes the unit tree look like a universal runtime source of truth; staff edit units or mappings in Django Admin without understanding module-specific effects | Staff wording must stay explicit: ordinary-user matching is consumer-specific during transition. ServiceEvent audience rows now match through active primary `ChurchStructureMembership`; ServiceEvent zero-row fallback, Bible Study visibility/generation, reading/privacy/progress, permissions, and My Serving remain legacy/consumer-specific. CS-SETUP.1 remains gated until unit moves, deactivation, mapping drift, membership drift, and stored audience-row effects are explicitly designed. |
...
- **Legacy mappings remain the Bible Study generation bridge.** Bible Study generation still resolves through `Profile.small_group` and the nullable `church_structure_unit` mappings on `MinistryContext`, `District`, and `SmallGroup`; CS-CORE.2B-A ServiceEvent audience-row matching now uses active primary `ChurchStructureMembership` instead. Editing the unit tree without touching legacy rows changes one side of the Bible Study bridge only; editing legacy mappings changes which legacy `SmallGroup` rows Bible Study resolves. These are different actions with different blast radius and must not be presented as one "edit structure" button.
- **`ChurchStructureMembership` is only a partial runtime source after CS-CORE.2B-A.** Editing membership/belonging rows can affect ServiceEvent structure-audience row matching, but not Bible Study visibility/generation, reading/privacy/progress, permissions, roles, My Serving, or legacy-fallback ServiceEvents. A UI that mixes membership edits with structure edits invites staff to misread which consumer changed or to assume a structure change is "just bookkeeping" (it can move stored audiences).
...
6. **Membership / belonging changes.** Create/approve/end `ChurchStructureMembership` rows. This is the ServiceEvent audience-row matching source after CS-CORE.2B-A, but it is not the runtime source for Bible Study visibility/generation, reading/privacy/progress, permissions, roles, My Serving, or legacy-fallback ServiceEvents. It must be treated as a separate product surface from structure editing, with its own approval, precisely so staff do not read it as a universal visibility control.
...
- No runtime visibility change was added by CS-SETUP.1A/1B itself. Later CS-CORE.2B-A changed ServiceEvent audience-row matching, and CS-CORE.2C-B changed Bible Study v2 meeting visibility; reading visibility, legacy `BibleStudySession`, and ServiceEvent zero-row fallback remain untouched here.
- No automatic unit↔legacy sync (one-way or two-way).
- No new membership-driven visibility in this setup/edit slice; `ChurchStructureMembership` became the ServiceEvent audience-row matching source only later in CS-CORE.2B-A and the Bible Study v2 meeting-visibility source only later in CS-CORE.2C-B.

...
42. BS-AS QA follow-up completed; BS-AS.2B fixed the audience picker mobile CSS no-go. CS-MAP.2 read-only Staff Structure Map + Mapping Health is completed at `/staff/structure/`; CS-SETUP.1D.1/1D.2/1D.3 complete only the narrow mapping-maintenance/review-polish/conflict-filter slices on `/staff/structure/mappings/`; SE-AS.4 is completed as the ServiceEvent runtime audience rule only, and CS-MAP/CS-SETUP work must not be bundled with SE-AS.5 or Community Activities; CS-MAP.3 remains optional/unapproved and broad CS-SETUP.1 setup/edit UI remains unapproved.
...
- CS-CORE.0B.1 Read-Only Belonging Drift Audit Command (`audit_structure_belonging`). Recommended next CS-CORE slice per `docs/CHURCH_STRUCTURE_CORE_MIGRATION_PLAN.md` Section 9; read-only, no DB writes; not yet approved or implemented.

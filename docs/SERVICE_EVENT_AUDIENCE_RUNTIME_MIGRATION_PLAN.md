...
- Existing stored legacy field values are not reconciled or bulk-cleared on edit; they remain audit/cleanup blockers for a later guarded cleanup command.
...
- **SE-SCOPE.1A** — stop normal app-level legacy ServiceEvent scope field writes (implemented). Normal create/edit and recurring app flows no longer expose or save `scope_type`, `district`, or `small_group`; existing values remain stored until guarded cleanup and later field/schema retirement.
...
- Legacy `scope_type` / `district` / `small_group` field cleanup and deprecation/removal: timing and approach. SE-SCOPE.1A stops new normal app writes but does not clear existing stored values. Guarded cleanup and field/schema removal require separate approval.

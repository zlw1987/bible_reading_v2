...
| Reflection create/edit group binding | `comments.forms.ReflectionCommentForm`; `comments.forms.ReflectionCommentEditForm`; `comments.views.add_comment()`; `comments.views.edit_comment()` | `Profile.small_group` still determines whether group sharing is offered and binds `small_group_at_post` for new group shares; CS-CORE.4C.2 preserves the existing snapshot when an already-group-shared top-level post remains group-shared | Whether users can choose group visibility and which group is stored on the post | Yes until reflection privacy is separately planned. Still legacy-driven after CS-CORE.4A/4B/4C/4C.1/4C.2; Policy C is implemented only for the edit re-bind branch | High | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A/4B/4C); preserve old posts and reply inheritance. Policy C is active after CS-CORE.4C.2, but no membership-core migration, `structure_unit_at_post`, data migration, or mass rewrite happened |
...
2. **Reading progress and reflection privacy migration sequence.** CS-CORE.4A completed the docs-only privacy migration plan, CS-CORE.4B completed the read-only diagnostic command `audit_reading_privacy_membership_readiness`, and CS-CORE.4C locked the current privacy invariants in tests. These consumers remain legacy-driven after all three slices; no runtime migration is authorized by this inventory, by the audit command, or by the tests.

...
| `ChurchStructureUnit` | `BibleStudySeriesAudienceScope.unit` (series audience), **`BibleStudyMeetingAudienceScope.unit` (meeting audience, V2 runtime source of truth)**, and **`BibleStudyMeeting.anchor_unit` (display/grouping/ownership)**. The legacy `SmallGroup.church_structure_unit` bridge is removed with the legacy table by LEGACY-STRUCTURE-TABLE-RETIRE.1A. |
...
1. **Generation** is structure-unit-native since BS-STRUCT.1L and
   **structure-audience-required** since BS-STRUCT.1M: it resolves
   ...
   `anchor_unit` per target. After BS-V2-MIRROR.1B, new generated normal V2
   meetings leave `BibleStudyMeeting.small_group` null; existing old mirror
   values remain compatibility / fallback-display / diagnostic cleanup blockers
   only. **The series legacy
...
2. **Manual create/edit meeting form** — since BS-STRUCT.1O the normal manual
   ...
   (path-labelled); the legacy `small_group` select is gone. Create/edit go through `sync_normal_meeting_audience_scope_for_unit(meeting, unit)`,
...
- **BS-STRUCT.1D** — normal generation writes audience rows + `anchor_unit`.
  ...
  - It resolves `meeting.small_group.church_structure_unit`. **Only when that
    ...
    column is never rewritten). At the BS-STRUCT.1D slice time, `small_group`
    stayed set as the compatibility mirror and `meeting_kind` stayed `normal`;
    this new-write mirror behavior was later superseded by BS-V2-MIRROR.1B.
...
- **BS-STRUCT.1H** — stop the **normal manual** `BibleStudyMeetingForm`
  ...
  - **Reusable normal-meeting audience logic moved to `studies/services.py`.**
    ...
    `models` / `visibility`.
  - **Historical BS-STRUCT.1H manual-create behavior:** at that slice time,
    `BibleStudyMeetingForm` made `small_group` required, and `clean()` rejected
    (form invalid, nothing saved) a selected group whose
    `church_structure_unit` was missing, inactive, or not `UNIT_SMALL_GROUP`.
    On a valid save the create/edit views called
    `sync_normal_meeting_audience_scope` inside a `transaction.atomic()`, which
    created exactly one `BibleStudyMeetingAudienceScope(meeting, mapped unit)`
    row and set `anchor_unit` when null. At the BS-STRUCT.1H slice time,
    `small_group` stayed set as the compatibility mirror and `meeting_kind`
    stayed `normal`; BS-V2-MIRROR.1B later superseded that new-write mirror
    behavior so new manual normal creates leave `small_group` null. A normal
    manual meeting is still never left as a legacy-only zero-row meeting, and an
    invalid mapping fails validation instead of creating one.
...
- **BS-STRUCT.1O** — manual Bible Study meeting create/edit form becomes
  **structure-unit-native**. ✅ **implemented.** Code/tests/docs only; no
  ...
  - **Service change.** New `sync_normal_meeting_audience_scope_for_unit(meeting,
    ...
    `generation_key` (`normal-unit:{unit_id}`, shared with generation). After
    BS-V2-MIRROR.1B, the helper never writes a new legacy `small_group` mirror:
    it preserves an existing old mirror only if it still maps to the selected
    unit, and clears stale mismatches. The view path (`create_bible_study_meeting`
    / `edit_bible_study_meeting`) calls this helper with
  ...
  - **Tests.** Updated/added `BibleStudyModuleTests` cases: form exposes
    ...
    manual create writes row + anchor + normal kind; at BS-STRUCT.1O time it
    still set a mirror when one active legacy group mapped, and used
    `small_group=None` for a no-mirror unit; create/edit duplicate
    `(lesson, unit)` rejected; edit changes unit and replaces the stale row +
    anchor + mirror. Superseded by BS-V2-MIRROR.1B: new saves no longer write a
    new mirror, and unit changes clear stale mismatches. Multi-unit and
    higher-level edits rejected with rows untouched. The three obsolete
    BS-STRUCT.1H unmapped / inactive-unit /
  ...
  - **Boundary (as of this slice):** the zero-row fallback was still present in
    ...
    `small_group` is not removed or renamed. At BS-STRUCT.1O time it was still
    written by this form; BS-V2-MIRROR.1B later stopped new mirror writes. The small-group-keyed
...
- **BS-V2-MIRROR.1B** — stop writing new V2 normal meeting
  `BibleStudyMeeting.small_group` mirrors. ✅ **implemented.** New generated and
  manually-created normal meetings leave `small_group` unset and rely on
  `generation_key`, `anchor_unit`, and `BibleStudyMeetingAudienceScope` rows.
  Manual edits preserve an existing mirror only when it still maps to the
  selected unit; unit changes clear stale mismatches and never write a new
  mirror. Existing stored mirror values are not bulk-cleared, and duplicate
  detection still recognizes old rows by generation key, single audience row,
  or legacy mirror compatibility. A future guarded audit/cleanup slice should
  inventory and clear existing `BibleStudyMeeting.small_group` mirrors before
  later field/constraint cleanup.
- **BS-V2-MIRROR.1C** — add guarded cleanup for existing V2
  `BibleStudyMeeting.small_group` mirrors. ✅ **implemented.**
  `cleanup_bible_study_v2_small_group_mirrors` is dry-run by default, and apply
  mode requires `--apply --confirm-small-group-mirror-retirement`. It clears
  only rows that already have correct structure-native identity:
  `generation_key`, `anchor_unit`, and exactly one matching active small-group

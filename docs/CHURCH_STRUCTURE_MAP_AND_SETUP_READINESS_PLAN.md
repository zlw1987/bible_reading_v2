...
| CS-SETUP.1D.1 | Legacy mapping maintenance implementation | Implemented: staff-only per-row edit of one legacy row's `church_structure_unit` to one active matching-type unit, gated by the legacy model's Django change permission, explicit POST, type/active/duplicate validation, and `LogEntry` audit (Section 12.8). It does not directly edit members, audience rows, serving schedules, or permissions, but it edits the same legacy-to-unit bridge that Bible Study structure-audience resolution/generation still uses, so it can affect generated legacy `SmallGroup` meetings (acknowledgement retained and wording corrected in CS-CORE.2B-B) |
...
- **Some unit edits now have runtime consequences through stored audience rows.** Unit moves and deactivation can change what `ServiceEventAudienceScope` and `BibleStudySeriesAudienceScope` rows resolve to; legacy mapping edits can change Bible Study structure-audience resolution/generation. Renames and sort-order changes are lower risk because they affect display labels/order rather than matching, but they still need permission, audit, and clear staff wording. A ServiceEvent or Bible Study series that already stored audience rows pointing at a unit or its descendants can silently gain or lose audience when staff edit the tree shape; Bible Study can also change when staff edit the mapping bridge. A "setup" UI that looks cosmetic is therefore not always cosmetic.
...
2. **Legacy-to-unit mappings.** Set or clear the `church_structure_unit` on a `MinistryContext` / `District` / `SmallGroup`. Directly changes the Bible Study mapping bridge and therefore can change Bible Study structure-audience resolution/generation. Since CS-CORE.2B-A, it no longer changes ServiceEvent audience-row matching. Higher risk; needs audience-impact warnings.
...
- Do not directly edit ServiceEvent audience rows, Bible Study audience rows, reading visibility, My Serving behavior, memberships, or serving rows. Note this is not an absolute "no effect" claim: after CS-CORE.2B-A, mapping edits no longer affect ServiceEvent audience-row matching, but they still update the legacy-to-unit bridge that Bible Study structure-audience resolution/generation uses. CS-CORE.2B-B discloses this in the visible copy and keeps a staff acknowledgement before save.
...
- It shows a warning consistent with the shipped UI after CS-CORE.2B-B: the edit does not directly edit members, audience rows, serving schedules, permissions, or `Profile.small_group`; ServiceEvent audience rows now match by active primary `ChurchStructureMembership`; Bible Study structure-audience resolution and generated legacy `SmallGroup` meetings still resolve through these mappings; and zero-row ServiceEvents fail closed for ordinary users after SE-RETIRE.1B. The shipped copy starts `This updates how this current record links to a church structure unit.` / `这里会更新这条现有记录与教会结构单元的对应关系。`
- It requires a staff acknowledgement checkbox (`I understand this mapping change may affect Bible Study structure-audience resolution and generated legacy SmallGroup meetings.` / `我了解此对应关系更改仍可能影响查经安排解析和生成的小组查经聚会。`) before the save is accepted.
...
- Mapping edits can affect future helpers, setup-readiness indicators, staff interpretation, and Bible Study structure-audience resolution/generation, because Bible Study still resolves selected units through the legacy-to-unit bridge. Since CS-CORE.2B-A, mapping edits no longer affect ServiceEvent audience-row matching.

...
Status update: CS-MAP.2 is now complete (see Section 7 for the completion note), SE-AS.5B post-commit cleanup clarified the visible wording and count semantics on the shipped read-only map, and CS-MAP.2B updates the map tree to use the same hierarchical node-level expand/collapse mental model as the ServiceEvent audience picker. CS-MAP.3 remains optional and unapproved. CS-SETUP.1 remains explicitly unapproved and gated per Section 6; CS-SETUP.1A is a docs-only risk/design pass that defines the design contract a future setup/edit UI must satisfy and splits CS-SETUP.1 into separately approvable sub-milestones (see Section 12). CS-SETUP.1A changes no code, schema, migration, template, view, or runtime behavior. CS-SETUP.1B is now implemented as the lowest-risk slice only: an opt-in edit mode on `/staff/structure/` with a per-row action menu offering Rename (display name + bilingual name) and a Details (Django Admin) link; it adds no schema/migration and changes no runtime visibility (see Section 12.6). CS-SETUP.1C.1 is implemented as a mapping review page at `/staff/structure/mappings/` that lists the legacy-to-structure mappings with simple status labels; CS-SETUP.1C.2 adds read-only summary counts and filters; both add no schema/migration and change no runtime visibility (see Section 12.7). CS-SETUP.1D.0 is a docs-only plan for the next safe mapping-maintenance slice: edit one existing legacy row's mapping to one existing active structure unit, with no unit lifecycle, membership, audience-row, or runtime visibility change (see Section 12.8). CS-SETUP.1D.1 is implemented as that narrow slice: a permission-gated staff edit page that updates one legacy row's `church_structure_unit` to one active matching-type unit via explicit POST with type/active/duplicate validation and `LogEntry` audit. CS-SETUP.1D.2 is implemented as display-only warning/conflict polish on the same mapping review surface: type-mismatch overlay, duplicate-active-mapping overlay, summary warning/conflict counts, and clearer page copy. CS-SETUP.1D.3 is implemented as display-only GET filter links for those conflict overlays (`conflicts`, `type_mismatch`, `duplicate_active`), with true-total counts and edit-link/successful-edit filter roundtrip preservation. CS-SETUP.1D.1/1D.2/1D.3 add no schema/migration and do not directly change unit lifecycle, membership, ServiceEvent/Bible Study audience rows, `Profile.small_group`, ordinary visibility, normal V2 generation, or serving schedules. CS-CORE.2B-B corrected the visible review/edit copy after CS-CORE.2B-A; BS-SMALLGROUP-GENERATION-BRIDGE-RETIRE.1A later retired normal V2 generation's mapping-bridge dependency. Mapping edits now affect setup-readiness and final-retirement bridge/admin/diagnostic resolution only; the edit POST still requires a staff acknowledgement checkbox before it saves (see Section 4 and the milestone table). LEGACY-OBJECT-ROW-PURGE-GATE.1A adds `purge_legacy_structure_object_rows` as the guarded dry-run-first row-purge preflight/apply path for the remaining legacy object rows; future apply is destructive, requires `--apply --confirm-legacy-structure-object-row-retirement`, must not delete `ChurchStructureUnit` rows, and remains separate from table/model or mapping-FK schema removal. CS-SETUP.1E future structure unit lifecycle design remains unapproved.
...
- `ChurchStructureMembership` (`accounts/models.py`): belonging foundation with requested/active/ended/rejected/cancelled lifecycle, one-active-primary-per-user enforcement, date windows, approval audit fields, and `active_for_user()` / `current_primary_for_user()` helpers. It is the runtime source for ServiceEvent audience-row matching after CS-CORE.2B-A, Prayer group requests after PRAYER-STRUCT.1A, Bible Study v2 `BibleStudyMeeting` audience-row visibility / Today / role-worship pickers after BS-STRUCT.2A, and the separately switched reading/reflection/group-progress slices. It is not the runtime source for Bible Study generation, legacy `BibleStudySession`, TeamAssignment / My Serving, or remaining legacy/archive consumers. Requested/rejected/ended/cancelled rows grant nothing.
- Legacy bridge mappings: nullable `church_structure_unit` fields on `MinistryContext`, `District`, and `SmallGroup` (`accounts/models.py`). These fields are not membership/source-of-truth rows and do not directly edit audience rows. Since CS-CORE.2B-A, ServiceEvent audience rows match by active primary `ChurchStructureMembership`; since BS-SMALLGROUP-GENERATION-BRIDGE-RETIRE.1A, normal Bible Study V2 generation targets structure units directly, so mapping edits affect setup-readiness and remaining bridge/admin/diagnostic resolution only.
- Commands: `seed_church_structure_units` remains setup/diagnostic support for legacy-to-structure mappings. `purge_legacy_structure_object_rows` is the guarded dry-run-first final preflight/apply path for deleting remaining `SmallGroup`, `District`, and `MinistryContext` rows only; it reports protected deletion counters and special `UNASSIGNED-GROUPS` decision rows, requires `--apply --confirm-legacy-structure-object-row-retirement` for future destructive apply, and must not delete `ChurchStructureUnit` or runtime product rows. The old `backfill_church_structure_memberships` command was retired with `Profile.small_group` in PROFILE-SG-FIELD-RETIRE.1A; membership approval/display now uses active primary `ChurchStructureMembership`.
- Bible Study Schedule is a structure-native runtime consumer: `BibleStudySeriesAudienceScope` (`studies/models.py`) selects schedule units, normal generation expands those selections to active descendant/self `UNIT_SMALL_GROUP` `ChurchStructureUnit` leaves, and generated V2 meetings use `generation_key`, `anchor_unit`, and `BibleStudyMeetingAudienceScope` rows. Since BS-STRUCT.2A, Bible Study v2 `BibleStudyMeeting` ordinary-member visibility, `/studies/` / Today, and role/worship pickers use `BibleStudyMeetingAudienceScope` rows plus active primary `ChurchStructureMembership`; zero-row V2 meetings fail closed, and `Profile.small_group` / the removed `BibleStudyMeeting.small_group` field do not grant ordinary V2 visibility.
- `ServiceEventAudienceScope` (`events/models.py`) is now a runtime visibility source for ServiceEvent (SE-AS.4/SE-AS.5 complete, with CS-CORE.2B-A membership-core matching): audience rows match ordinary users by active primary `ChurchStructureMembership`, and zero-row events fail closed for ordinary users after SE-RETIRE.1B (see `docs/SERVICE_EVENT_AUDIENCE_RUNTIME_MIGRATION_PLAN.md` and `docs/CHURCH_STRUCTURE_CORE_MIGRATION_PLAN.md`). SE-FIELD-RETIRE.1A later removed the legacy `scope_type` / `district` / `small_group` fields.
- Staff surfaces: read-only overview at `/staff/`, membership request review/approve/reject at `/staff/membership-requests/`, moderation queue at `/staff/moderation/`. `/staff/structure/` now renders the structure hierarchy (CS-MAP.2/2B) as a permission-gated map; CS-SETUP.1B adds only an opt-in edit mode there with per-row rename (display name only) and a Details link for admin-capable staff. `/staff/structure/mappings/` lists each `MinistryContext` / `District` / `SmallGroup` row beside its mapped unit, summary counts, primary status filters, display-only warning/conflict overlays, display-only conflict overlay filters (`?status=conflicts`, `?status=type_mismatch`, `?status=duplicate_active`), and a permitted `Edit Mapping / 编辑对应关系` action for staff with the matching legacy change permission. The edit action updates only one existing legacy row's `church_structure_unit` to one active matching-type `ChurchStructureUnit`; duplicate active mappings remain backend-protected. The shared audience picker (`templates/shared/_church_structure_unit_audience_picker.html`) renders the tree as a form selector in other modules. Broader structure setup (create/move/deactivate units, districts, groups) still happens only in Django Admin.

Ordinary-user matching is now consumer-specific. ServiceEvent audience rows, Prayer group requests, Bible Study v2 audience-row meetings / Today / role-worship pickers, reading/reflection slices, and group-progress slices use their separately approved membership-core paths. Legacy `BibleStudySession`, TeamAssignment / My Serving, and remaining legacy/archive/display/audit consumers still depend on legacy models where documented. Bible Study Schedule uses structure audience rows and structure-unit-native generation; it no longer uses legacy mapping resolution for normal V2 meeting generation.
...
"Setup support" must not be read as "build broad edit UI now." Editing `ChurchStructureUnit` is no longer merely cosmetic: ServiceEvent audience rows (`ServiceEventAudienceScope`) and Bible Study Schedule audience rows (`BibleStudySeriesAudienceScope`) can depend on the unit tree and membership rows, so unit moves, deactivation, or membership drift can change resolved audiences in module-specific ways. A higher-risk staff edit UI would therefore have real runtime consequences while exposing none of the rules needed to make those changes safe — the same staff-confusion failure mode that `docs/SERVICE_EVENT_AUDIENCE_RUNTIME_MIGRATION_PLAN.md` Section 4 rejected for the audience selector. Only narrow slices are implemented so far: CS-SETUP.1B renames display labels and links to Details; CS-SETUP.1D.1 updates one existing legacy row's mapping to one existing active matching-type unit; CS-SETUP.1D.2 shows display-only mapping warnings/conflicts; CS-SETUP.1D.3 adds display-only filters for those conflict overlays. These slices do not move units, change memberships, alter audience rows, or change `Profile.small_group`. They also do not *directly* change runtime visibility; mapping edits now affect setup-readiness indicators and remaining bridge/admin/diagnostic resolution, not normal V2 generation. CS-SETUP.1E/1F and any broad create/move/deactivate or membership management UI remain unapproved. Django Admin remains the structure write surface for everything beyond approved narrow slices: it already distinguishes legacy runtime models from future foundation models (CS-H.5E), and model `full_clean()` validation enforces tree integrity there.
...
| Church Structure Membership | `ChurchStructureMembership` | Runtime source for ServiceEvent structure-audience row matching after CS-CORE.2B-A, Prayer group requests after PRAYER-STRUCT.1A, Bible Study v2 audience-row visibility / Today / role-worship pickers after BS-STRUCT.2A, and separately switched reading/reflection/group-progress slices. Not the runtime source for Bible Study generation, legacy `BibleStudySession`, roles/capabilities, TeamAssignment / My Serving, or remaining legacy/archive consumers. Requested/rejected/ended/cancelled rows grant nothing. |
| Audience Scope / 适用范围 | `BibleStudySeriesAudienceScope` (runtime for structure-native meeting generation), `BibleStudyMeetingAudienceScope` (runtime for V2 meeting visibility / Today / role-worship pickers), `ServiceEventAudienceScope` (runtime for ServiceEvent visibility) | Who an event/schedule/meeting is for. Per-module join models selecting units; ServiceEvent and Bible Study V2 meeting audience rows match those units against active primary `ChurchStructureMembership`, and Bible Study schedule generation expands selected units to active small-group structure-unit leaf targets. |
| Host / Language Label / 主办/语言标签 | `ServiceEvent.host_language_unit` plus audience-derived fallback | Display label only. Never visibility, serving, or permissions. The legacy `ServiceEvent.ministry_context` FK was removed in SERVICE-EVENT-CONTEXT.1C. |
...
| Legacy `Profile.small_group` | `Profile.small_group` | Current runtime belonging source for legacy `BibleStudySession` and remaining legacy/archive/audit consumers. It no longer grants ServiceEvent visibility (audience rows use membership and zero-row events fail closed), Prayer group-request visibility, Bible Study V2 meeting visibility / Today / role-worship picker access, reading/reflection switched-slice visibility, or group-progress access. |
| Legacy `District` / `SmallGroup` / `MinistryContext` | legacy structure models | Remaining object rows are final table-retirement blockers, not ordinary-member runtime authority. They are bridged to units via nullable mapping fields for setup/admin/diagnostic context until row purge and later table/model/mapping-FK retirement are separately approved. |
...
| SE-AS.6C | ServiceEvent audience optional apply mode, after dry-run review | Completed; production apply created/verified audience rows without mutating legacy fields |
...
| CS-SETUP.1C.1 / 1C.2 | Legacy-to-unit mapping review UI | Implemented: mapping review page plus summary counts and filters (Section 12.7); no write UI until CS-SETUP.1D.1 |
...
| CS-SETUP.1D.1 | Legacy mapping maintenance implementation | Implemented: staff-only per-row edit of one legacy row's `church_structure_unit` to one active matching-type unit, gated by the legacy model's Django change permission, explicit POST, type/active/duplicate validation, and `LogEntry` audit (Section 12.8). It does not directly edit members, audience rows, serving schedules, permissions, ordinary visibility, or normal V2 generation; mapping edits affect setup-readiness indicators and final-retirement bridge/admin/diagnostic resolution (acknowledgement retained and wording corrected after the membership-core switches) |
| CS-SETUP.1D.2 | Mapping warning polish / conflict indicators | Implemented: display-only type-mismatch and duplicate-active-mapping overlays, warning/conflict summary counts, and clearer page copy; duplicate active mapping remains backend-protected; no direct member/audience-row/`Profile.small_group` write (mapping-edit runtime-matching impact is as described for CS-SETUP.1D.1) |
| CS-SETUP.1D.3 | Mapping conflict filter UX | Implemented: display-only `?status=conflicts`, `?status=type_mismatch`, and `?status=duplicate_active` filter links for the CS-SETUP.1D.2 overlays; counts remain true totals, primary status filters remain unchanged, and edit-link / successful-edit roundtrip preserves these filter values |
| CS-SETUP.1D.4 | Mapping edit final-retirement warning + acknowledgement | Implemented: corrected the review/edit safety copy after ServiceEvent audience-row matching moved to active primary `ChurchStructureMembership` and normal Bible Study V2 generation became structure-native. The visible warning now says mapping edits are for final-retirement preparation, setup checks, admin, and diagnostic resolution only; they do not directly edit ordinary visibility or normal V2 generation. No schema/migration/URL/permission change; all CS-SETUP.1D.1 target validations, acknowledgement gating, and `?status=` roundtrip preserved |
...
2. Active non-root units with no legacy mapping at-or-beneath them: active units (excluding the root) where neither the unit nor any descendant is referenced by any legacy mapping field. These are setup/readiness concerns for remaining bridge/admin/diagnostic consumers, not normal Bible Study V2 generation blockers. For CS-CORE.2B-A ServiceEvent structure-audience matching, an unmapped/custom/fellowship unit can still match users by active primary `ChurchStructureMembership` when the membership unit is the selected unit or a descendant. Unique unit count.
...
4. Historical/superseded: users whose legacy `Profile.small_group` was unmapped. `Profile.small_group` was removed in PROFILE-SG-FIELD-RETIRE.1A, so current setup diagnostics should use active primary `ChurchStructureMembership` and legacy object-row mapping counters instead.
...
| Risk | Mitigation |
| --- | --- |
| The map makes the unit tree look like a universal runtime source of truth; staff edit units or mappings in Django Admin without understanding module-specific effects | Staff wording must stay explicit: ordinary-user matching is consumer-specific during transition. ServiceEvent audience rows, Prayer group requests, Bible Study V2 audience-row meetings, and completed reading/progress/reflection slices now match through active primary `ChurchStructureMembership`; Bible Study generation, legacy `BibleStudySession`, roles/capabilities, TeamAssignment / My Serving, and remaining legacy/archive consumers remain legacy/consumer-specific. CS-SETUP.1 remains gated until unit moves, deactivation, mapping drift, membership drift, and stored audience-row effects are explicitly designed. |
...
- **Some unit edits now have runtime consequences through stored audience rows.** Unit moves and deactivation can change what `ServiceEventAudienceScope` and `BibleStudySeriesAudienceScope` rows resolve to. Renames and sort-order changes are lower risk because they affect display labels/order rather than matching, but they still need permission, audit, and clear staff wording. A ServiceEvent or Bible Study series that already stored audience rows pointing at a unit or its descendants can silently gain or lose audience when staff edit the tree shape; mapping edits affect setup-readiness indicators and remaining bridge/admin/diagnostic resolution, not normal V2 generation. A "setup" UI that looks cosmetic is therefore not always cosmetic.
- **Legacy mappings are no longer the normal Bible Study V2 generation bridge.** Normal generation is structure-native from `BibleStudySeriesAudienceScope` rows to active small-group `ChurchStructureUnit` leaf targets, with `generation_key`, `anchor_unit`, and meeting audience rows. Legacy mappings remain setup-readiness and bridge/admin/diagnostic/table-retirement context; editing them must not be presented as changing normal V2 generation.
- **`ChurchStructureMembership` is only a partial runtime source.** Editing membership/belonging rows can affect ServiceEvent structure-audience row matching, Prayer group requests, Bible Study V2 audience-row meetings/pickers, and completed reading/progress/reflection slices, but not Bible Study generation, legacy `BibleStudySession`, roles/capabilities, TeamAssignment / My Serving, or remaining legacy/archive consumers. A UI that mixes membership edits with structure edits invites staff to misread which consumer changed or to assume a structure change is "just bookkeeping" (it can move stored audiences).
...
2. **Legacy-to-unit mappings.** Set or clear the `church_structure_unit` on a `MinistryContext` / `District` / `SmallGroup`. This affects setup-readiness indicators and remaining bridge/admin/diagnostic resolution, but normal V2 generation is structure-native and no longer resolves through this mapping bridge. Since CS-CORE.2B-A, it no longer changes ServiceEvent audience-row matching. Higher risk; needs clear staff warnings.
...
6. **Membership / belonging changes.** Create/approve/end `ChurchStructureMembership` rows. This is the source for ServiceEvent audience-row matching, Prayer group requests, Bible Study V2 audience-row meetings/pickers, and completed reading/progress/reflection slices, but it is not the runtime source for Bible Study generation, legacy `BibleStudySession`, roles/capabilities, TeamAssignment / My Serving, or remaining legacy/archive consumers. It must be treated as a separate product surface from structure editing, with its own approval, precisely so staff do not read it as a universal visibility control.
...
- **CS-SETUP.1D.1** — docs-reviewed mapping maintenance implementation. Complete: edits one mapping at a time and satisfies Section 12.8.
- **CS-SETUP.1D.2** — warning polish / conflict indicators. Complete: display-only conflict/warning overlays and summary counts on the mapping review surface.
- **CS-SETUP.1D.3** — conflict filter UX. Complete: display-only GET filter links for the CS-SETUP.1D.2 conflict overlays, with true-total counts and edit roundtrip preservation.
...
- No implementation of any edit/setup UI at CS-SETUP.1A time. Later, CS-SETUP.1B was separately approved and implements rename/detail only (see Section 12.6); CS-SETUP.1C.1/1C.2 were separately approved and implement mapping review plus read-only summary/filtering (see Section 12.7); CS-SETUP.1D.0 was docs-only planning; CS-SETUP.1D.1 implements permitted one-row mapping edit for existing legacy rows only; CS-SETUP.1D.2 implements display-only mapping warning/conflict polish; CS-SETUP.1D.3 implements display-only mapping conflict filters. CS-SETUP.1E/1F remain unapproved.
- No schema changes or migrations of any kind.
- No runtime visibility change was added by CS-SETUP.1A/1B itself. Later CS-CORE.2B-A changed ServiceEvent audience-row matching, BS-STRUCT.2A changed Bible Study V2 audience-row visibility / Today / role-worship pickers and retired V2 zero-row ordinary-user fallback, PRAYER-STRUCT.1A changed Prayer group requests, and SE-RETIRE.1B retired the ServiceEvent zero-row runtime fallback. Legacy `BibleStudySession` remains untouched here.
- No automatic unit↔legacy sync (one-way or two-way).
- No new membership-driven visibility in this setup/edit slice; `ChurchStructureMembership` became the ServiceEvent audience-row matching source only later in CS-CORE.2B-A, the Bible Study V2 audience-row visibility / Today / role-worship picker source later in BS-STRUCT.2A, and the Prayer group-request source later in PRAYER-STRUCT.1A.
- No ServiceEvent audience backfill/apply was authorized by CS-SETUP.1A/1B itself. SE-AS.6C later completed as its own separate slice.
...
### 12.7 CS-SETUP.1C.1 / 1C.2 — implemented mapping review slices

CS-SETUP.1C.1 shipped the initial read-only review slice of CS-SETUP.1C. It did not implement mapping edit at that time. CS-SETUP.1D.1 later added the separately approved one-row mapping edit action, CS-SETUP.1D.2 later added display-only warning/conflict polish, and CS-SETUP.1D.3 later added display-only conflict filter links for those overlays. Create/move/deactivate (CS-SETUP.1E) and membership management (CS-SETUP.1F) remain unapproved and unimplemented.
...
- No POST handler, no form, no inline edit, no save action, and no schema/migration. The CS-SETUP.1C mapping review slice itself introduced no runtime behavior changes: it changed no mappings, units, memberships, or audience rows. After CS-CORE.2B-A, ServiceEvent audience rows use active primary `ChurchStructureMembership`; after BS-STRUCT.2A, Bible Study V2 meeting visibility / Today / role-worship pickers use meeting audience rows plus membership. Later BS-SMALLGROUP-GENERATION-BRIDGE-RETIRE.1A retired normal V2 generation's legacy mapping bridge. Mapping review/edit copy was later corrected by CS-CORE.2B-B and LEGACY-OBJECT-SURFACE-NARROW.1A.

CS-SETUP.1C.2 is a follow-up read-only usability slice on the same page. It adds a summary count area (all rows, mapped to active unit, unmapped, mapped to inactive unit, mapped under holding/unassigned node, and a needs-review total = unmapped + mapped inactive + mapped holding) and `?status=` GET filter links (`all`, `needs_review`, `mapped_active`, `unmapped`, `mapped_inactive`, `mapped_holding`) so a long mapping list can be narrowed to the rows that need attention. The page still defaults to all rows; the filter only shows/hides already-loaded rows and, when a section has no matching rows, shows a clear empty state. CS-SETUP.1C.2 adds no POST, no form, no inline edit, no save, no mapping/unit/membership/audience write, and no schema/migration. Mapping edit belongs only to CS-SETUP.1D.1; warning/conflict polish belongs only to CS-SETUP.1D.2; display-only conflict overlay filters belong only to CS-SETUP.1D.3. None changes runtime behavior. After CS-CORE.2B-A, ServiceEvent audience rows use active primary `ChurchStructureMembership`; after BS-STRUCT.2A, Bible Study V2 visibility uses meeting audience rows plus membership; after BS-SMALLGROUP-GENERATION-BRIDGE-RETIRE.1A, normal V2 generation no longer uses the legacy mapping bridge. Mapping review/edit copy was later corrected by CS-CORE.2B-B and LEGACY-OBJECT-SURFACE-NARROW.1A.
...
- `/staff/structure/mappings/` exists as legacy mapping review for `MinistryContext`, `District`, and `SmallGroup`.
- CS-SETUP.1C.2 adds summary counts and filters so staff can find unmapped, mapped-inactive, and holding-node mappings.
- CS-SETUP.1D.1 adds a permitted one-row mapping edit action for existing legacy rows only.
- CS-SETUP.1D.2 adds display-only warning/conflict overlays and summary counts for type mismatches and duplicate active mappings.
- CS-SETUP.1D.3 adds display-only conflict overlay filters for all conflicts, type mismatches, and duplicate active mappings.
- Django Admin remains the broader technical backstop; staff UI remains limited to the approved one-row mapping edit.

Problem:

- Staff can now see unmapped rows, rows mapped to inactive units, rows under holding/unassigned nodes, type mismatches, and duplicate active mapping conflicts.
- Staff with the matching legacy change permission can correct one `MinistryContext` / `District` / `SmallGroup` to `ChurchStructureUnit` mapping from staff UI.
- Django Admin remains the technical backstop for broader review and recovery.
- The implemented fix must remain mapping maintenance only and must not blur into unit lifecycle editing, membership editing, or audience editing.

Implemented mapping-maintenance slice:

- Build a narrow mapping-maintenance UI for existing legacy rows only.
- Support only setting or changing one existing `MinistryContext`, `District`, or `SmallGroup` row's `church_structure_unit` to an existing active matching-type `ChurchStructureUnit`.
- Show display-only warning/conflict overlays for type mismatches and duplicate active mappings.
- Show display-only GET conflict filters for the warning/conflict overlays without changing the underlying totals or primary status filters.
...
- Do not directly edit ServiceEvent audience rows, Bible Study audience rows, reading visibility, My Serving behavior, memberships, or serving rows. Note this is not an absolute "no effect" claim: after CS-CORE.2B-A, mapping edits no longer affect ServiceEvent audience-row matching, and after BS-SMALLGROUP-GENERATION-BRIDGE-RETIRE.1A they no longer affect normal V2 generation; they can still affect setup-readiness indicators and remaining bridge/admin/diagnostic resolution. The visible copy keeps a staff acknowledgement before save.
...
- It shows a warning consistent with the shipped UI after LEGACY-OBJECT-SURFACE-NARROW.1A: the edit does not directly edit members, audience rows, serving schedules, permissions, ordinary visibility, or normal V2 generation; ServiceEvent audience rows now match by active primary `ChurchStructureMembership`; normal Bible Study V2 generation is structure-native; remaining final-retirement bridge/admin/diagnostic resolution can still read this mapping; and zero-row ServiceEvents fail closed for ordinary users after SE-RETIRE.1B. The shipped copy starts `This updates how this legacy record links to a church structure unit for final-retirement preparation, setup checks, and diagnostics.` / `这里会更新这条旧记录与教会结构单元的对应关系，用于最终退役准备、设置检查和诊断。`
- It requires a staff acknowledgement checkbox (`I understand this mapping change is for final-retirement preparation, setup checks, admin, or diagnostic resolution.` / `我了解此对应关系更改仅用于最终退役准备、设置检查、管理或诊断解析。`) before the save is accepted.
...
- Mapping changes do not add or expand `ChurchStructureMembership` runtime visibility beyond the separately switched ServiceEvent structure-audience and Bible Study v2 meeting-visibility consumers.
- Mapping changes are not row purge. `purge_legacy_structure_object_rows` is the separate guarded dry-run-first purge gate for final legacy object-row deletion; future apply requires explicit destructive approval and must preserve `ChurchStructureUnit` rows and runtime product rows.
...
- **CS-SETUP.1D.1 docs-reviewed implementation** — complete: one mapping at a time, with permission, type/active validation, duplicate protection, explicit POST, audit, warnings, and targeted tests.
- **CS-SETUP.1D.2 warning polish / conflict indicators** — complete: display-only type-mismatch overlay, display-only duplicate-active-mapping overlay, warning/conflict summary counts, and clearer copy. Duplicate active mappings remain backend-protected.
- **CS-SETUP.1D.3 conflict filter UX** — complete: display-only `?status=conflicts`, `?status=type_mismatch`, and `?status=duplicate_active` links for the conflict overlays. Counts remain true totals, primary status filters remain unchanged, and edit-link / successful-edit roundtrip preserves these filter values.
...
Targeted regression coverage expected for CS-SETUP.1D.1 / 1D.2 / 1D.3:
...
- Warning/conflict summary counts render type mismatches and duplicate active mappings as display-only indicators.
- Warning/conflict overlays do not bypass or replace backend duplicate-active validation.
- Conflict overlay filters (`conflicts`, `type_mismatch`, `duplicate_active`) narrow the visible rows through GET-only navigation while summary counts remain true totals.
- Edit links and successful edit redirects preserve the selected conflict overlay filter.

Risks:

- Mapping edits can affect future helpers, setup-readiness indicators, staff interpretation, and remaining bridge/admin/diagnostic resolution. Normal Bible Study V2 generation is now structure-unit-native from `BibleStudySeriesAudienceScope` rows to active small-group `ChurchStructureUnit` leaf targets, so mapping edits should not be described as changing normal V2 generation. Since CS-CORE.2B-A, mapping edits no longer affect ServiceEvent audience-row matching.
...
- Therefore CS-SETUP.1D.1 uses warning copy, a narrow edit surface, strict type/active validation, audit, and explicit non-goals; CS-SETUP.1D.2 adds display-only warning/conflict cues without changing data behavior; CS-SETUP.1D.3 adds display-only navigation filters for those cues without changing data behavior or totals.

Implementation note (CS-SETUP.1D.1, implemented): the staff mapping review page (`/staff/structure/mappings/`) now shows an `Edit Mapping / 编辑对应关系` action on each legacy row, gated by the matching Django change permission (`accounts.change_ministrycontext` / `change_district` / `change_smallgroup`) on top of `staff_member_required`. The edit page (`staff_structure_mapping_edit`) renders the legacy object name/type/status, the current mapped unit and its status, and a target dropdown limited to active units of the matching `ChurchStructureUnit` type, with the required warning copy and Save/Cancel. Save is an explicit CSRF POST that updates only `church_structure_unit` (`update_fields`) after authoritative backend validation (required / exists / active / type-match / duplicate-active, with the current row allowed to keep its existing mapping), writes a Django `LogEntry` CHANGE record with before/after unit context, shows a success message, and redirects back to the review page preserving the prior `status` filter. It does not directly edit unit lifecycle, membership, `ServiceEventAudienceScope`, `BibleStudySeriesAudienceScope`, `TeamAssignment`, `Profile.small_group`, ordinary visibility, normal V2 generation, or serving rows, and adds no schema/migration. Since CS-CORE.2B-A, ServiceEvent audience rows match by active primary `ChurchStructureMembership`; since BS-SMALLGROUP-GENERATION-BRIDGE-RETIRE.1A, normal Bible Study V2 generation no longer resolves through this mapping bridge. The remaining disclosed impact is setup-readiness and final-retirement bridge/admin/diagnostic resolution, so the visible review/edit copy keeps a staff acknowledgement checkbox before the save is accepted.

Implementation note (CS-SETUP.1D.2, implemented): the staff mapping review page now adds display-only warning/conflict polish over the same legacy mapping data. It flags type-mismatch mappings, flags duplicate active mappings, shows conflict/warning summary counts, and uses clearer page copy so staff understand the page supports review plus permitted one-row mapping edits. These indicators do not write data, do not relax backend duplicate-active protection, and do not change memberships, `Profile.small_group`, `ServiceEventAudienceScope`, `BibleStudySeriesAudienceScope`, `TeamAssignment`, serving schedules, or audience rows. As display-only review aids they change no structure-scope matching or runtime visibility by themselves; the separate CS-SETUP.1D.1 mapping edit can affect setup-readiness indicators and remaining bridge/admin/diagnostic resolution, which the visible copy discloses and gates behind a staff acknowledgement.

Implementation note (CS-SETUP.1D.3, implemented): the staff mapping review page now adds display-only filter links for the CS-SETUP.1D.2 conflict overlays: `?status=conflicts`, `?status=type_mismatch`, and `?status=duplicate_active`. These overlay filters are GET-only navigation, leave the summary counts as true totals rather than filtered counts, keep the primary status filters unchanged, and preserve the selected filter through edit links and successful edit redirects. They do not write data, do not change schema/models/migrations, do not change memberships, `Profile.small_group`, `ServiceEventAudienceScope`, `BibleStudySeriesAudienceScope`, `TeamAssignment`, serving schedules, or audience rows, and do not approve CS-SETUP.1E structure unit lifecycle editing. As display-only navigation they change no structure-scope matching or runtime visibility by themselves; the separate CS-SETUP.1D.1 mapping edit can affect setup-readiness indicators and remaining bridge/admin/diagnostic resolution, which the visible copy discloses and gates behind a staff acknowledgement.

...
- Large deferred items remain deferred pending real pilot feedback. MO-S.4 manual team-leader scheduling, MO-S.4A semantic cleanup, MO-S.5A rotation anchor metadata, MO-S.5B limited copy-forward suggestions, SE-AS.1 ServiceEvent audience-scope planning, the SE-AS.2 ServiceEventAudienceScope model-only foundation, SE-AS.4 runtime visibility with legacy fallback, SE-AS.5A staff selector interaction planning, SE-AS.5 staff selector UI/display, TODAY-HOME.1B read-only Today IA, TODAY-HOME.1D linked Bible Study role chips, and BS-ROLE.1B role form/UI polish now exist, but automatic scheduling, availability, swaps, reminders, checklist, notifications, attendance, Community Activities/invitations, SE-AS.6C apply/backfill and later legacy cleanup, ServiceEvent consumer migration, Bible Study role confirmation, and membership-driven visibility remain deferred unless separately planned and approved.

...
| Generic deployment configuration | [`GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md`](GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md) · [`Slice 5 implementation plan`](GENERIC_DEPLOYMENT_CONFIGURATION_SLICE5_PLAN.md) | Frozen architecture; 1A/2A implement Ministry Team key identity/configuration tooling. 3A adds ServiceProfile/FK schema, and 4A is production-applied/post-audit-verified for one reviewed SVCA mapping (one profile, 52 exact dual-consistent events, zero drift, revisions `1 -> 2` once). 5A audits/plans; 5B adds the default-off integration registry and lazy adapter gates; 5C adds the typed FK-authoritative runtime seam; 5D implements FK-owned readiness V2, the retained dual-safe Bethany TEST reset V2, and FK-selecting ServiceEvent Admin; 5E is IMPLEMENTED / LOCAL VERIFIED for FK/Profile-authoritative Worship XLSX matching and confirmation plus strict V2 contracts; and `GENERIC-DEPLOYMENT-CONFIG.5F` is **PRODUCTION CLOSEOUT COMPLETE / VERIFIED** with Class A legacy authority at zero. Production verification confirmed the exact 52-event FK/Profile state, zero drift/blockers, Readiness V2 READY, only the workbook integration enabled, a fresh V2 52-no-op/0-proposed/0-blocked preview, English/Chinese rendered workflow, and no confirmation write; `runtime_consumer_switched` is formally true. `GENERIC-DEPLOYMENT-CONFIG.6A` is **IMPLEMENTED / LOCAL VERIFIED**: the ministry-owned Service Profile static-team default model, validation, inactive history, Admin, read-only audit, and invalid-active readiness blocker now exist, with canonical Worship pool/child exclusion and profile identity immutability extended to requirement references. `GENERIC-DEPLOYMENT-CONFIG.6B` is **PRODUCTION CONFIGURATION APPLY COMPLETE / VERIFIED**: the reviewed SVCA profile `bethany_0930_cm` has four active valid static defaults, independently post-audited and idempotently verified. No `ServiceEventRequiredTeam` materialization occurred; Slice 7 materialization and MO-S.REQUIRED runtime remain pending. `LIGHTING-PILOT-RETIRE.1A` remains **IMPLEMENTED / LOCAL VERIFIED**; the retired adapter and key did not return, and Worship XLSX V2 contracts are unchanged. External mapping and legacy-key field retirement remain separately pending. |

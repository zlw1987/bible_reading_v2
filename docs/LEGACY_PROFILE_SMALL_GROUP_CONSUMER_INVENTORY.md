...
| Reflection create/edit group binding | `comments.forms.ReflectionCommentForm`; `comments.forms.ReflectionCommentEditForm`; `comments.views.add_comment()`; `comments.views.edit_comment()` | `Profile.small_group` determines whether group sharing is offered and binds `small_group_at_post` | Whether users can choose group visibility and which group is stored on the post | Yes until reflection privacy is separately planned. Still legacy-driven and unchanged after CS-CORE.4A/4B/4C/4C.1 | High | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A/4B/4C); preserve old posts and reply inheritance. The top-level edit re-bind policy decision (Policy C, future runtime slice) is recorded docs-only in `docs/REFLECTION_EDIT_REBIND_POLICY_DECISION.md` (CS-CORE.4C.1); runtime re-bind behavior is unchanged |

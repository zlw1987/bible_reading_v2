...
- CS-CORE.2C-A is complete: a read-only Bible Study membership-core readiness/shadow audit (`studies/structure_readiness.py` helpers plus the `audit_bible_study_membership_readiness` command). CS-CORE.2C-B is complete: Bible Study v2 `BibleStudyMeeting` ordinary-member visibility and the `/studies/` / Today v2 meeting pre-filter now use active primary `ChurchStructureMembership`; `Profile.small_group` alone no longer grants v2 meeting visibility. Bible Study meeting generation still uses legacy `SmallGroup`, Bible Study schedule audience resolution still uses legacy mappings, and legacy `BibleStudySession` is unchanged as a retirement target. At 2C-B time `Profile.small_group` still drove reading/progress/privacy; those have since switched consumer-by-consumer (group-progress visible roster in CS-CORE.4F.1, permission-fenced default selected group in CS-CORE.4F.2, ordinary-member group reflection **read** path in CS-CORE.4G.2). Still legacy: the reflection write path/forms and the group-progress permission/accessible group list. No legacy fields/tables are retired, and the readiness audit now compares old legacy Bible Study meeting visibility against current membership-core v2 meeting visibility.
...
- **Reflection write path / forms migration.** Move `ReflectionCommentForm` / `ReflectionCommentEditForm` group option-gating and the `comments.views.add_comment()` / `edit_comment()` snapshot stamping off legacy `Profile.small_group` / `small_group_at_post` onto membership-core. The reflection **read** path already switched in CS-CORE.4G.2; this is the next reflection slice (preserve old posts and reply inheritance).
...
- CS-CORE.2B-A was the first consumer switch (ServiceEvent structure-audience matching → active primary `ChurchStructureMembership`; ServiceEvents with no audience rows still use legacy fallback and `Profile.small_group`). Since then, additional consumers have switched to membership-core: Bible Study v2 meeting visibility and the `/studies/` / Today v2 pre-filter (CS-CORE.2C-B), the group-progress visible roster (CS-CORE.4F.1) and permission-fenced default selected group (CS-CORE.4F.2), and the ordinary-member group reflection **read** path (CS-CORE.4G.2). Still legacy/pending: the reflection write path/forms, group-progress permission/accessible group list, ServiceEvent zero-row fallback, Bible Study v2 meeting generation, structure-aware role scopes, TeamAssignment/My Serving (serving is never inferred from structure membership), and the legacy `Profile.small_group` / `SmallGroup` / `District` / `MinistryContext` compatibility layer (not retired).

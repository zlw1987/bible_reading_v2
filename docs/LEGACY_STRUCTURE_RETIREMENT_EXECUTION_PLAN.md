...
BS-V1-SCHEMA-RETIRE-GATE.1A hardens this dry-run as the future purge preflight:
it reports matched V1 session rows, V1 guide rows, V1 worship-song rows, session
counts by `scope_type`, sessions with `district_id`, sessions with
`small_group_id`, and unexpected inbound dependency rows. V1 app runtime/admin
remain retired; these are cleanup/schema blockers, not ordinary-member runtime
visibility blockers. `BibleStudySession.small_group` blocks final `SmallGroup`
table retirement, `BibleStudySession.district` blocks final `District` table
retirement, and there is no V1 `MinistryContext` FK.
...
- The audit preserves that split: `bible_study_v1_app_runtime_legacy_blockers` remains `0`, while `bible_study_v1_purge_pending` now includes V1 session rows plus V1 child guide/worship rows and contributes to `bible_study_legacy_retirement_blockers` as a data/table-retirement blocker.

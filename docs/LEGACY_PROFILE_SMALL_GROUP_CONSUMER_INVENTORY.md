...
| Consumer | File / function / model | Current source | What it controls | Safe to leave legacy for now? | Migration risk | Recommended future milestone |
| --- | --- | --- | --- | --- | --- | --- |
| Current user group helper | `reading.views.get_user_small_group()` | `user.profile.small_group` | Shared reading/reflection helper for the user's current legacy group | Yes as an explicit legacy helper. Still legacy-driven after CS-CORE.4A/4B | Low | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A/4B) |
| Group progress accessible group list | `accounts.permissions.get_accessible_progress_groups()` | `ChurchRoleAssignment` legacy `District` / `SmallGroup` scopes plus the user's own `Profile.small_group` | Which groups a user may select for group progress | Yes. This is tied to legacy role scopes and progress permissions. Still legacy-driven after CS-CORE.4A/4B | High | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A/4B); role-scope decision stays out of the 4-series |
| Group progress roster and default selected group | `reading.views.my_group_progress()` | Defaults to `Profile.small_group`; roster is `User.objects.filter(profile__small_group=selected_group)` | Member rows, enrollments, check-ins, and progress percentages for selected legacy groups | Yes, but do not migrate casually because this affects privacy-visible rosters. Still legacy-driven after CS-CORE.4A/4B | High | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A/4B) with privacy invariants and a shadow/switch sequence |
| Reflection visibility checks | `comments.models.ReflectionComment.can_be_seen_by()`; `reading.views.get_visible_reflection_filter()`; `reading.views.passage_wall()` | `Profile.small_group` compared to `ReflectionComment.small_group_at_post` | Visibility for "My Group" reflections and group tab filtering | Yes. It preserves current privacy semantics and uses a snapshot group on each comment. Still legacy-driven after CS-CORE.4A/4B | High | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A/4B); transfer/snapshot policy decided there |
| Reflection create/edit group binding | `comments.forms.ReflectionCommentForm`; `comments.forms.ReflectionCommentEditForm`; `comments.views.add_comment()`; `comments.views.edit_comment()` | `Profile.small_group` determines whether group sharing is offered and binds `small_group_at_post` | Whether users can choose group visibility and which group is stored on the post | Yes until reflection privacy is separately planned. Still legacy-driven after CS-CORE.4A/4B | High | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A/4B); preserve old posts and reply inheritance |
| Reading/reflection privacy readiness audit | `reading.management.commands.audit_reading_privacy_membership_readiness` | Compares legacy `Profile.small_group` / `small_group_at_post` reflection visibility and legacy progress rosters against a membership-core candidate | Read-only diagnostic support for CS-CORE.4B; reports drift/risk categories only | Yes. This is diagnostic-only after CS-CORE.4B and writes nothing; reading/progress/reflection runtime remains legacy-driven | Low | Keep as gate evidence for future CS-CORE.4C/4F+ slices; do not use it from request/runtime code |
...
2. **Reading progress and reflection privacy migration sequence.** CS-CORE.4A completed the docs-only privacy migration plan, and CS-CORE.4B completed the read-only diagnostic command `audit_reading_privacy_membership_readiness`. These consumers remain legacy-driven after both slices; no runtime migration is authorized by this inventory or by the audit command.

...
- Bible Study Schedule is the first narrow runtime consumer: `BibleStudySeriesAudienceScope` (`studies/models.py`) resolves selected units to eligible legacy `SmallGroup` rows for meeting generation. Since BS-STRUCT.2A, Bible Study v2 `BibleStudyMeeting` ordinary-member visibility, `/studies/` / Today, and role/worship pickers use `BibleStudyMeetingAudienceScope` rows plus active primary `ChurchStructureMembership`; zero-row V2 meetings fail closed, and `Profile.small_group` / `BibleStudyMeeting.small_group` do not grant ordinary V2 visibility.
...
| Audience Scope / 适用范围 | `BibleStudySeriesAudienceScope` (runtime for meeting generation), `BibleStudyMeetingAudienceScope` (runtime for V2 meeting visibility / Today / role-worship pickers), `ServiceEventAudienceScope` (runtime for ServiceEvent visibility); legacy scope fields elsewhere | Who an event/schedule/meeting is for. Per-module join models selecting units; ServiceEvent and Bible Study V2 meeting audience rows match those units against active primary `ChurchStructureMembership`, while Bible Study schedule generation still resolves selected units through legacy mappings. |
...
- **Legacy mappings remain the Bible Study generation bridge.** Bible Study generation still resolves through legacy `SmallGroup` rows and the nullable `church_structure_unit` mappings on `MinistryContext`, `District`, and `SmallGroup`; CS-CORE.2B-A ServiceEvent audience-row matching and BS-STRUCT.2A Bible Study V2 meeting visibility now use active primary `ChurchStructureMembership` against audience rows instead. Editing the unit tree without touching legacy rows changes one side of the Bible Study generation bridge only; editing legacy mappings changes which legacy `SmallGroup` rows Bible Study generation resolves. These are different actions with different blast radius and must not be presented as one "edit structure" button.

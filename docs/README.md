...
| Generic deployment configuration | [`GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md`](GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md) · [`Slice 5 implementation plan`](GENERIC_DEPLOYMENT_CONFIGURATION_SLICE5_PLAN.md) | Frozen architecture; 1A/2A implement Ministry Team key identity/configuration tooling. 3A adds ServiceProfile/FK schema, and 4A is production-applied/post-audit-verified for one reviewed SVCA mapping (one profile, 52 exact dual-consistent events, zero drift, revisions `1 -> 2` once). 5A audits/plans; 5B adds the default-off integration registry and lazy adapter gates; 5C adds the typed FK-authoritative runtime seam; and `GENERIC-DEPLOYMENT-CONFIG.5D` is IMPLEMENTED / LOCAL VERIFIED for FK-owned readiness V2, the retained dual-safe Bethany TEST reset V2, and FK-selecting ServiceEvent Admin with read-only compatibility key and one normal save/revision. No reset was executed. Workbook matching/signing remains legacy-authoritative and untouched pending 5E, so `runtime_consumer_switched` remains false globally. Profile defaults/materialization, MO-S.REQUIRED runtime, external mapping, 5E/5F closure, and legacy-key retirement remain pending. |

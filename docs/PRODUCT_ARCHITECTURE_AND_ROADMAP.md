...
Future flexible Church Structure Foundation planning should keep current `MinistryContext`, `District`, `SmallGroup`, and `Profile.small_group` assumptions usable while leaving room for CM/EM, variable-depth branches, arbitrary future structure units, and future membership history. Long-term source of truth should be `ChurchStructureUnit` plus `ChurchStructureMembership`; short-term runtime behavior still uses existing models. Nullable legacy mapping fields, verified command-based seeding/mapping, seeded data QA closure, membership design, a model-only membership table, helper hardening, explicit command-based membership backfill, user-attested production/staging backfill verification, clearer Django Admin labeling, signup/Profile requested-unit capture, staff request review, approve/reject actions, narrow approval sync, SE-AS.1 ServiceEvent audience-scope redesign planning, SE-AS.2 model-only `ServiceEventAudienceScope` foundation, SE-AS.4 ServiceEvent runtime visibility with legacy fallback, and SE-AS.5 ServiceEvent staff audience selector UI/display now exist. For ServiceEvents, `ServiceEventAudienceScope` rows are the ordinary-user visibility source when present; events with no rows still use legacy `scope_type` / `district` / `small_group` plus `Profile.small_group`, and legacy fields remain preserved/editable as fallback settings. `ChurchStructureMembership` still does not grant ServiceEvent visibility. Bible Study Schedule audience scope (DOCS-AS.1 direction, implemented as BS-AS.1 / BS-AS.2 / BS-AS.2A) remains a narrow runtime consumer: `BibleStudySeriesAudienceScope` joins `BibleStudySeries` to `ChurchStructureUnit`, selected units resolve to eligible legacy `SmallGroup` rows for meeting generation, generated `BibleStudyMeeting` rows still point to legacy `SmallGroup`, ordinary member visibility still uses `Profile.small_group`, and `ChurchStructureMembership` runtime visibility migration remains deferred. SE-AS.6 backfill/cleanup, broad custom staff admin expansion, consumer migration from legacy fields/`Profile.small_group`, and a runtime source-of-truth switch are still future and not authorized here. See `docs/CHURCH_STRUCTURE_FOUNDATION_PLAN.md`, `docs/SERVICE_EVENT_AUDIENCE_SCOPE_REDESIGN_PLAN.md`, `docs/SERVICE_EVENT_AUDIENCE_RUNTIME_MIGRATION_PLAN.md`, and `docs/SERVICE_EVENT_AUDIENCE_SELECTOR_INTERACTION_PLAN.md`.
...
- SE-AS.4 ServiceEvent Audience Runtime Visibility Rule completed: events with `ServiceEventAudienceScope` rows use audience rows for ordinary-user visibility; events with no rows keep legacy `scope_type` / `district` / `small_group` plus `Profile.small_group` behavior; `ChurchStructureMembership` still does not grant ServiceEvent visibility; legacy fields remain preserved as fallback; no SE-AS.5 selector UI, form/template audience picker, Community Activities, CS-MAP.3, or CS-SETUP.1 was added.
...
- SE-AS.5 ServiceEvent Staff Audience Selector UI and Display completed: single create/edit and recurring create now expose an optional `ChurchStructureUnit` audience picker; selected units save/replace `ServiceEventAudienceScope` rows; clearing the picker restores legacy fallback; recurring preview writes no rows; recurring create applies one selected audience set only to newly created events; staff detail shows Structure audience vs Legacy fallback audience plus readable labels and unmapped-selection warning; ordinary detail does not expose audience architecture terms. No SE-AS.6 backfill, Community Activities, CS-MAP.3, CS-SETUP.1, schema/migration, `ChurchStructureMembership` visibility migration, legacy-field removal/deprecation, or Required Ministry Teams / Rotation Anchor / TeamAssignment / My Serving behavior change was added.
- SE-AS.5B post-commit cleanup completed: ServiceEvent audience pickers on single create/edit and recurring create are collapsed by default with selected-count summaries, fallback copy now explains that legacy fields apply only when no structure audience is selected, and the read-only staff structure map now uses clearer `Church Structure & Setup Check` wording plus descendant-inclusive covered-member counts. No runtime visibility, schema, migration, backfill, setup/edit UI, roster, or membership-source migration was added.
...
- CS-MAP.2 read-only Staff Structure Map + Mapping Health completed at `/staff/structure/`: permission-protected read-only staff page rendering the active `ChurchStructureUnit` hierarchy with bilingual names, descendant-inclusive covered-member counts, current data mapping context from active `MinistryContext` / `District` / `SmallGroup` rows, and setup-readiness indicators including direct active primary memberships on parent units; no write actions, no member rosters, no schema/migration changes, no runtime visibility changes, no setup/edit UI. Django Admin remains the structure write surface.
- SE-AS.5 is complete as the bounded staff selector/display implementation; Community Activities, CS-MAP.3, CS-SETUP.1, SE-AS.6 backfill, and consumer migration are not pulled forward by SE-AS.5 completion.
...
- Later: SE-AS.6 backfill/compatibility cleanup and Community Activities should reuse the same `ChurchStructureUnit` audience-scope foundation; both remain deferred and require separate approval before implementation.
...
- Later SE-AS.6 backfill/compatibility cleanup and ServiceEvent consumer migration only after separate approval (numbering per `docs/SERVICE_EVENT_AUDIENCE_RUNTIME_MIGRATION_PLAN.md`).

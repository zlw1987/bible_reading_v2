...
| Generic deployment configuration | [`GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md`](GENERIC_DEPLOYMENT_CONFIGURATION_ARCHITECTURE.md) | Frozen architecture; 1A/2A implement Ministry Team key identity/configuration tooling and the product owner reports the current SVCA production deployment at 11 configured teams with 0 unconfigured/integrity problems (deployment data only; no key-text runtime behavior). 3A implements the exact ServiceProfile table plus nullable protected ServiceEvent FK expansion. 4A is implementation-complete, locally verified, and production-applied/post-audit-verified for one reviewed SVCA mapping: one profile, 52 exact dual-consistent events, zero drift, and revisions advanced `1 -> 2` exactly once. This is deployment evidence only; `runtime_consumer_switched` is false, the legacy key remains current runtime authority, and Slice 5 integration gating/consumer switch remains pending. Profile defaults/materialization, MO-S.REQUIRED runtime, external-system mapping, and legacy-key retirement remain unimplemented. |

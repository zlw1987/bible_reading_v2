...
- Ordinary runtime belonging is `Profile.small_group` everywhere except ServiceEvent structure-audience rows (switched in CS-CORE.2B-A), Bible Study v2 `BibleStudyMeeting` ordinary-member visibility (switched in CS-CORE.2C-B), and the group-progress visible roster / `member_rows` (switched in CS-CORE.4F.1): legacy-fallback ServiceEvents, legacy `BibleStudySession` visibility, reflection-wall group privacy (`small_group_at_post`), and the group-progress permission / accessible group list / selected/default group still read `Profile.small_group`.
- `ChurchStructureMembership` is a runtime visibility source for exactly three consumers: ServiceEvent structure-audience matching (CS-CORE.2B-A), Bible Study v2 `BibleStudyMeeting` ordinary-member visibility (CS-CORE.2C-B), and the group-progress visible roster (CS-CORE.4F.1). For everything else it remains signup/request/approval, diagnostics, and backfill data only.
...
1. `Profile.small_group` remains the ordinary-member runtime belonging source for current consumers except ServiceEvent structure-audience matching (switched by CS-CORE.2B-A), Bible Study v2 `BibleStudyMeeting` ordinary-member visibility (switched by CS-CORE.2C-B), and the group-progress visible roster / `member_rows` (switched by CS-CORE.4F.1, roster only — group-progress permission and selected/default group stay legacy).

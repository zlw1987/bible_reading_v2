...
| Sunday Ministry Scheduling | [`SUNDAY_MINISTRY_SCHEDULING_PLAN.md`](SUNDAY_MINISTRY_SCHEDULING_PLAN.md) | Current MO-S.6 plan through implemented Board/context work and `MO-S.6D-1A` semantic Campus / Site foundation, plus docs-only workbook readiness and governed Worship Team selection/import prerequisites. |
| Worship Rotation Governance | [`WORSHIP_ROTATION_GOVERNANCE_PLAN.md`](WORSHIP_ROTATION_GOVERNANCE_PLAN.md) | Canonical MO-S.6D-0A-FU1/FU2 governance decisions and implemented `MO-S.6D-1A` flexible Campus / Site semantic foundation; Worship-specific pools, required exact-event planner responsibility, event-level Worship Team ownership, exact-roster consistency, narrow authority, reachability, batch-change safeguards, and notifications remain future slices. |

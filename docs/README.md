...
Status: canonical documentation entry point, current through `NOTIFY.1F`
...
| Notification V0 | [`NOTIFICATIONS_V0_PLAN.md`](NOTIFICATIONS_V0_PLAN.md) | Canonical boundary through implemented `NOTIFY.1F`: registered/gateable notifications app, model/admin, Core directed-delivery port, notifications-owned persistence, recipient center/bell/read state, recipient-scoped POST Open to the safe stored internal target, 25-row center pagination, and retain-for-now policy without a cleanup command; plus the three approved ministry, studies, and Community Activities producers. The product owner passed deployed `NOTIFY.1F` manual QA for the implemented recipient read/open/pagination UI, and the current limited-trial Notification V0 scope is closed through `NOTIFY.1F`. Broader future Notification product work remains deferred; this is not browser automation or a broad production-readiness claim. |
| Sunday Ministry Scheduling | [`SUNDAY_MINISTRY_SCHEDULING_PLAN.md`](SUNDAY_MINISTRY_SCHEDULING_PLAN.md) | Current MO-S.6 plan through implemented Board/context, Campus, Worship pool, event-planner, governed exact-event selection, operational reachability, and read-only rotation-planner preview, plus docs-only workbook and confirmation contracts. |
| Worship Rotation Governance | [`WORSHIP_ROTATION_GOVERNANCE_PLAN.md`](WORSHIP_ROTATION_GOVERNANCE_PLAN.md) | Canonical multi-campus Worship invariants through implemented exact-event governance, operational reachability, and read-only planner preview; locked confirmation, notifications, and import remain separately approved later slices. |
| Worship Rotation Planner | [`WORSHIP_ROTATION_PLANNER_PLAN.md`](WORSHIP_ROTATION_PLANNER_PLAN.md) | `MO-S.6D-1D-D-0A` V1 contract plus implemented read-only `1A` contextual proposal/preview: exact bounded shift semantics, no tail loss, destination eligibility, per-event authority, roster blocker, roster-free downstream impact, deterministic fingerprints, and user-bound signing. Locked confirmation/audit `1B` remains unimplemented and blocked on downstream event-first serialization closure. |

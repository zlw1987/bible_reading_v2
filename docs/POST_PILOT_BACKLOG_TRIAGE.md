...
- CS-CORE.2B-A is complete: ServiceEvent structure-audience matching (events with `ServiceEventAudienceScope` rows) now uses the active primary `ChurchStructureMembership` as its runtime source; events with no audience rows still use legacy fallback and `Profile.small_group`; `audit_structure_belonging` gained a read-only `--fail-on-drift` guard. One consumer switch only — no other module changed and no legacy field is retired.
...
- **ServiceEvent zero-row legacy fallback deprecation.** Plan-only deprecation of legacy `scope_type` / `district` / `small_group` fallback on `ServiceEvent`, after audience-row coverage and rollback requirements are accepted. No field hiding/removal without approval.

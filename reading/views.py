...
def home(request):
    ...
    for enrollment in enrollments:
        ...
        if not is_not_started and not is_ended:
            plan_day = (
                ReadingPlanDay.objects
                .filter(plan=active_plan.plan, day_number=current_day_number)
                .first()
            )
        ...
        today_items.append({
            ...
            "memory_passages": parse_memory_verse_text(plan_day.memory_verse) if plan_day and plan_day.memory_verse else [],
        })
...
def active_plan_detail(request, active_plan_id):
    ...
    checked_plan_day_ids = set(
        CheckIn.objects
        .filter(user=request.user, active_plan=active_plan)
    ...
    for day_number in range(1, max_day_number + 1):
        ...
        calendar_date = active_plan.start_date + timezone.timedelta(
            days=day_number - 1
        )
    ...
    progress_percent = round((checked_days / total_reading_days) * 100) if total_reading_days else 0
    ...
    return render(request, "reading/active_plan_detail.html", context)
...
def passage_reader(request, active_plan_id, plan_day_id, passage_index):
    ...
    return render(
        request,
        "reading/passage_reader.html",
        {
            "reader_title": "Scripture Reader",
            ...
            "is_last_passage": is_last_passage,
...
def memory_verse_reader(request, active_plan_id, plan_day_id, passage_index):
    ...
    return render(
        ...
        "reading/passage_reader.html",
        {
            "reader_title": "Memory Verse",
            "source_label": "Memory Verse",
            "active_plan": active_plan,
            "plan_day": plan_day,
            "passage": passage,
            "passage_index": passage_index,
...
def my_group_progress(request):
    ...
    group_members = (
        User.objects
        .filter(profile__small_group=selected_group)
        .order_by("username")
    )
    ...
    plan_days = list(
        ReadingPlanDay.objects
        .filter(plan=selected_active_plan.plan)
        .order_by("day_number")
    )
    ...
    enrolled_user_ids = set(
        PlanEnrollment.objects
        .filter(active_plan=selected_active_plan, user__in=group_members)
    ...
    return render(request, "reading/group_progress.html", context)
...
def leave_active_plan(request, active_plan_id):
    ...
    enrollment = PlanEnrollment.objects.filter(
        user=request.user,
        active_plan_id=active_plan_id,
    ).select_related("active_plan").first()
    ...
    messages.success(
        request,
        f"You left the reading plan: {active_plan_name}."
    )

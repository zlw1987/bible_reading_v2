...
`NOTIFY.1A` (notification app/model/admin/Core delivery-port foundation; no
producer or member-facing notification UI).
...
| `notifications` | `notifications` | Notifications / 通知 | Registered and default-enabled gateable module owning directed Notification persistence. Source modules resolve recipients through the Core delivery port and have no hard dependency on Notifications. `NOTIFY.1A` contributes no ordinary nav, Today, setup/readiness, Staff Overview, or member-facing notification UI. |
...
9. **Notifications is a registered module behind a small Core delivery port
   (`NOTIFY.1A`).** Module key `notifications` owns the Notification model/rows,
   initial migration, operational admin, and idempotent persistence sink. It
   will also own future read/unread UI, retention, rendering, center, and bell.
   ...
   No source module emits through the port in 1A. There is no notification
   route, template, center, bell, unread/mark-read UI, Today, Calendar, My
   Serving, Staff Overview, external delivery, scheduler, or background job.

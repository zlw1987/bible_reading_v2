...
- BS-AS.1 Bible Study Schedule Audience Scope Using ChurchStructureUnit. Completed. `BibleStudySeriesAudienceScope` joins `BibleStudySeries / 查经安排` to `ChurchStructureUnit`; selected units resolve to eligible legacy `SmallGroup` rows for meeting generation; generated `BibleStudyMeeting` rows still point to legacy `SmallGroup`; as of CS-CORE.2C-B, ordinary v2 `BibleStudyMeeting` visibility uses active primary `ChurchStructureMembership`; legacy scope fields remain compatibility/fallback.
...
40. BS-AS.1 Bible Study Schedule audience scope using `ChurchStructureUnit` is complete, as the first narrow runtime consumer. It resolves selected `ChurchStructureUnit` rows to eligible legacy `SmallGroup` rows for meeting generation; generated `BibleStudyMeeting` rows still point to legacy `SmallGroup`. As of CS-CORE.2C-B, ordinary v2 `BibleStudyMeeting` visibility uses active primary `ChurchStructureMembership`; schedule resolution and meeting generation still use the legacy mapping bridge.
...
- CS-CORE.2B-A Membership-Core Runtime Source for ServiceEvent Structure Audience. Completed; one consumer switch only, with legacy fallback events unchanged and a read-only `--fail-on-drift` audit guard.
...
- CS-CORE.1C was a parity closure only: Bible Study structure-audience schedules resolve selected units to legacy `SmallGroup` rows for meeting generation through the shared selector layer (still the case). At 1C time ordinary member meeting visibility was `Profile.small_group`-based; that was **later switched to membership-core by CS-CORE.2C-B**. Bible Study meeting **generation** still targets legacy `SmallGroup` rows.
- CS-CORE.2B-A was the first consumer switch (ServiceEvent structure-audience matching → active primary `ChurchStructureMembership`; at 2B-A time ServiceEvents with no audience rows still used the legacy fallback and `Profile.small_group`, but SE-RETIRE.1B later retired that zero-row runtime fallback, so zero-row events now fail closed for ordinary users). Since then, additional consumers have switched to membership-core: Bible Study v2 meeting visibility and the `/studies/` / Today v2 pre-filter (CS-CORE.2C-B), the group-progress visible roster (CS-CORE.4F.1) and permission-fenced default selected group (CS-CORE.4F.2), the ordinary-member group reflection **read** path (CS-CORE.4G.2), the reflection **write** path/forms (CS-CORE.4G.3), and the group-progress permission/accessible group list (CS-CORE.2D-B, foundation CS-CORE.2D-A). Still legacy/pending: ServiceEvent legacy scope **field** retirement (the zero-row runtime fallback itself is already retired per SE-RETIRE.1B; the `scope_type` / `district` / `small_group` fields remain stored/editable as admin/display/audit data), Bible Study v2 meeting generation, TeamAssignment/My Serving (serving is never inferred from structure membership), field-level retirement of the legacy `district` / `small_group` role-scope fields (the `structure_unit` backfill shipped in CS-CORE.2D-C and the role-scope **runtime** fallback was retired in ROLE-RETIRE.1B), and the legacy `Profile.small_group` / `SmallGroup` / `District` / `MinistryContext` compatibility layer (not retired).
- CS-CORE.2C-A was the shadow/readiness audit; CS-CORE.2C-B is complete as the Bible Study v2 visibility switch. The `audit_bible_study_membership_readiness` command and `studies/structure_readiness.py` helpers remain read-only and now compare preserved old `Profile.small_group` Bible Study v2 meeting visibility against current membership-core visibility. Bible Study still uses legacy `SmallGroup` meeting generation, and legacy `BibleStudySession` remains unchanged.
- Unrelated large product items remain deferred pending real pilot feedback (this is about non-structure feature scope, not the Church Structure migration, which is being actively continued). MO-S.4 manual team-leader scheduling, MO-S.4A semantic cleanup, MO-S.5A rotation anchor metadata, MO-S.5B limited copy-forward suggestions, SE-AS.1 ServiceEvent audience-scope planning, the SE-AS.2 ServiceEventAudienceScope model-only foundation, SE-AS.4 runtime visibility with legacy fallback, SE-AS.5A staff selector interaction planning, SE-AS.5 staff selector UI/display, CS-SETUP.1D.1 permitted one-row legacy mapping edit, CS-SETUP.1D.2 display-only mapping warning/conflict polish, CS-SETUP.1D.3 display-only mapping conflict filter UX, TODAY-HOME.1B read-only Today IA, TODAY-HOME.1D linked Bible Study role chips, DATETIME-UX.1A/1B member-facing datetime display planning and implementation, DATETIME-UX.1C staff/report/table datetime formatting planning, BS-ROLE.1B role form/UI polish, and the membership-core consumer switches (CS-CORE.2B-A ServiceEvent audience, CS-CORE.2C-B Bible Study v2 visibility, CS-CORE.4F.1/4F.2 group-progress roster/default, CS-CORE.4G.2 reflection read path) now exist. Still deferred unless separately planned and approved: automatic scheduling, availability, swaps, reminders, checklist, notifications, attendance, Community Activities/invitations, SE-AS.6C apply/backfill and later legacy cleanup, Bible Study role confirmation, member timezone preferences, staff/report/table datetime implementation, broad structure setup/edit UI, structure unit lifecycle editing, and membership roster management. The remaining **Church Structure** runtime switches (ServiceEvent legacy scope field retirement, Bible Study v2 generation bridge, field-level retirement of the legacy role-scope fields now that the role-scope runtime fallback was retired in ROLE-RETIRE.1B, legacy compatibility-layer retirement) are sequenced migration work, not indefinitely deferred — each is its own approved slice. (The reflection read and write paths are both already membership-core after CS-CORE.4G.2 / CS-CORE.4G.3, and the group-progress permission/access list switched in CS-CORE.2D-B.)
- `ChurchStructureUnit` model-only foundation exists and has hardened cycle validation. CS-H.3 records the mapping/membership strategy, CS-H.3B adds nullable legacy mapping fields, CS-H.3C adds explicit command-based seeding/mapping, CS-H.3D verifies GoDaddy production/staging seeding with a clean second dry-run, CS-H.3E closes seeded structure data QA, CS-H.4 records the membership design, CS-H.5A adds the membership model-only foundation, CS-H.5B hardens helpers/validation, CS-H.5C adds explicit command-based membership backfill, CS-H.5D records user-attested GoDaddy production/staging backfill verification, CS-H.5E improves Django Admin clarity, CS-H.6 records signup requested-unit flow design, CS-H.6A/CS-H.6B add signup request capture planning and implementation, CS-H.6D adds Profile request capture, CS-H.7 records admin approval workflow design, CS-H.7A records approval implementation planning, CS-H.7B/C adds the membership-management capability plus pending request list, CS-H.7D adds request detail plus minimal approve/reject actions, CS-H.7E syncs `Profile.small_group` only for exactly one active legacy small-group mapping, CS-H.8 records the integration checkpoint, CS-H.9 records membership request UX hardening, CS-H.10 records the CMS hardening checkpoint, PP-SA.2 records the read-only staff overview completion, PP-SA.3 records the staff membership request workflow polish completion, PP-SA.4 records the read-only staff moderation queue completion, and PP-SA.5 records the read-only ministry ops health indicator completion. Multiple ordinary-user runtime consumers are now membership-driven: ServiceEvent structure-audience rows (CS-CORE.2B-A), Bible Study v2 meeting visibility and the `/studies/` / Today v2 meeting pre-filter (CS-CORE.2C-B), the group-progress visible roster (CS-CORE.4F.1) and permission-fenced default selected group (CS-CORE.4F.2), the ordinary-member group reflection **read** path (CS-CORE.4G.2), the reflection **write** path / form option-gating (CS-CORE.4G.3), and the group-progress permission/accessible group list (CS-CORE.2D-B). Still legacy: My Serving / TeamAssignment (serving is never inferred from structure membership). (The ServiceEvent zero-row runtime fallback was retired in SE-RETIRE.1B: events with no `ServiceEventAudienceScope` rows now fail closed for ordinary users and no longer read `scope_type` / `district` / `small_group` or `Profile.small_group`; those legacy fields remain stored/editable only until field-level retirement.) Bible Study Schedule audience selection is implemented (BS-AS.1 / BS-AS.2 / BS-AS.2A): selected `ChurchStructureUnit` rows resolve to legacy `SmallGroup` for meeting **generation** (still a legacy bridge), while ordinary v2 meeting **visibility** switched to active primary `ChurchStructureMembership` in CS-CORE.2C-B. Today now shows the v2 small-group Bible Study meeting plus personalized read-only Bible Study role chips only from linked `BibleStudyMeetingRole.user` rows on the already-visible primary meeting; display-name-only roles remain meeting-detail fallback and are not personalized on Today. ServiceEvent audience selector UI/editing/display workflow is implemented in SE-AS.5, while SE-AS.6C apply/backfill and later legacy cleanup, ServiceEvent consumer migration, Community Activities, Checklist V1, reminders, scheduling, swaps, availability, attendance, notifications, announcements, care workflows, activities operations, file center, and finer permission matrix work should not start without a separate planning decision.

...
* Keep approved runtime consumers on `ChurchStructureUnit` / `ChurchStructureMembership`.
* Do not reintroduce legacy runtime authority from:
...
Do not reintroduce removed legacy fields/models/tables or delete any remaining historical/diagnostic references just because they look old. Further field/table retirement still requires audit, backfill/purge readiness, explicit approval, targeted tests, and a separate migration slice.
...
V1 `BibleStudySession` is retired from app runtime and schema:
...
* Do not describe V1 pilot/archive rows as active current schema. Only immutable historical migrations and historical docs should mention them.

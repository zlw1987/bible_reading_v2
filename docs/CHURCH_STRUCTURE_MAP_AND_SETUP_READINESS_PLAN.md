...
| CS-SETUP.1D.1 | Legacy mapping maintenance implementation | Implemented: staff-only per-row edit of one legacy row's `church_structure_unit` to one active matching-type unit, gated by the legacy model's Django change permission, explicit POST, type/active/duplicate validation, and `LogEntry` audit (Section 12.8). It does not directly edit members, audience rows, serving schedules, permissions, or normal V2 generation; mapping edits affect setup-readiness indicators and remaining bridge/admin/diagnostic resolution (acknowledgement retained and wording corrected after the membership-core switches) |
...
| CS-SETUP.1D.4 | Mapping edit audience-impact warning + acknowledgement | Implemented: corrected the review/edit safety copy to drop the too-absolute "does not change who can see content / visibility" framing, stating instead that a mapping edit does not *directly* edit members/audience rows/schedules/permissions but may affect structure-based ServiceEvent / Bible Study matching under the pre-CS-CORE.2B-A runtime rule; the edit POST requires a staff acknowledgement checkbox before it saves. CS-CORE.2B-B later updated the visible copy after ServiceEvent audience-row matching moved to active primary `ChurchStructureMembership`, leaving Bible Study mapping-bridge impact as the remaining acknowledgement reason. No schema/migration/URL/permission change; all CS-SETUP.1D.1 target validations and `?status=` roundtrip preserved |
...
- It shows a warning consistent with the shipped UI after BS-SMALLGROUP-GENERATION-BRIDGE-RETIRE.1A: the edit does not directly edit members, audience rows, serving schedules, permissions, or `Profile.small_group`; ServiceEvent audience rows now match by active primary `ChurchStructureMembership`; normal Bible Study V2 generation is structure-native; remaining Bible Study bridge/admin/diagnostic resolution can still read this mapping; and zero-row ServiceEvents fail closed for ordinary users after SE-RETIRE.1B. The shipped copy starts `This updates how this current record links to a church structure unit.` / `这里会更新这条现有记录与教会结构单元的对应关系。`
- It requires a staff acknowledgement checkbox (`I understand this mapping change may affect remaining Bible Study bridge/admin/diagnostic resolution.` / `我了解此对应关系更改可能影响查经桥接、管理或诊断解析。`) before the save is accepted.

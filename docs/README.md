...
| Sunday Ministry Scheduling | [`SUNDAY_MINISTRY_SCHEDULING_PLAN.md`](SUNDAY_MINISTRY_SCHEDULING_PLAN.md) | Current MO-S.6 plan through implemented Board/context work plus docs-only workbook readiness and governed Worship Team selection/import prerequisites. |
| Worship Rotation Governance | [`WORSHIP_ROTATION_GOVERNANCE_PLAN.md`](WORSHIP_ROTATION_GOVERNANCE_PLAN.md) | Canonical MO-S.6D-0A-FU1/FU2 decisions for flexible Campus semantics, Worship-specific pools, required exact-event planner responsibility, event-level Worship Team ownership, exact-roster consistency, narrow authority, reachability, batch-change safeguards, and future notification boundaries. |

...
* The legacy parent FKs `SmallGroup.district` and `District.ministry_context` were dropped from the Django admin display in `LEGACY-OBJECT-ADMIN-FK.1A`: removed from `SmallGroupAdmin` / `DistrictAdmin` `list_display`/`list_filter`/`search_fields`/`fieldsets` in `accounts/admin.py`. This was admin-display-only — the admins stay registered, the `church_structure_unit` mapping bridge + `mapping_status` are unchanged, no field/model/table was removed, no migration, and no data/runtime change. The staff `structure_mapping_review` / `structure_mapping_edit` views were intentionally left in place because they maintain the still-live `church_structure_unit` bridge. Do not re-add `district` / `ministry_context` to these admins.
* The legacy parent/context FK fields `SmallGroup.district` and `District.ministry_context` were removed in `LEGACY-PARENT-FK-FIELD-RETIRE.1A` (migration `accounts/0013`) after `LEGACY-OBJECT-ADMIN-FK.1A/1B`, `SE-EVENT-LEGACY-WARNING-RETIRE.1A`, and `LEGACY-BRIDGE-RESOLVER-NARROW.1A` retired their admin/display/resolver reads and target-DB dry-run confirmed parent/context links already clear (0 present, `data_blocker_count=0`). The canonical hierarchy is `ChurchStructureUnit.parent` via the `church_structure_unit` mapping bridge. The guarded `cleanup_legacy_structure_parent_links` command (and its test module), whose only purpose was to clear these two fields before removal, were retired with them. `seed_church_structure_units` no longer reconstructs the `District`/`SmallGroup` hierarchy from legacy links: `MinistryContext` units are still seeded under the root, already-mapped `District`/`SmallGroup` rows keep their authoritative `ChurchStructureUnit.parent`, and unmapped rows are reported as `unreconstructable` (manual placement needed) rather than silently reparented to `UNASSIGNED-DISTRICTS` / `UNASSIGNED-GROUPS` buckets. This did not remove the `SmallGroup`/`District`/`MinistryContext` rows/tables or the `church_structure_unit` mapping FKs. Do not reintroduce these two parent FKs.
...
* `SmallGroup.district` / `District.ministry_context` removed in `LEGACY-PARENT-FK-FIELD-RETIRE.1A` (migration `accounts/0013`); the canonical hierarchy is `ChurchStructureUnit.parent` reached via the `church_structure_unit` mapping bridge. The guarded `cleanup_legacy_structure_parent_links` command was retired with them, and `seed_church_structure_units` no longer reconstructs the `District`/`SmallGroup` hierarchy from legacy links (it reports unmapped rows as `unreconstructable` instead of reparenting to unassigned buckets). This did not remove the `SmallGroup`/`District`/`MinistryContext` tables or the `church_structure_unit` mapping FKs.

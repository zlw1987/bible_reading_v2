...
after `REPOSITORY-AUDIT-CLOSEOUT.1A`), plus the docs-only `NOTIFY.1G-0A`
Direct Worship Team Change Notification Contract. `NOTIFY.1G` runtime remains
unimplemented.
...
| Notification V0 | [`NOTIFICATIONS_V0_PLAN.md`](NOTIFICATIONS_V0_PLAN.md) | Canonical runtime boundary through implemented `NOTIFY.1F`: registered/gateable notifications app, model/admin, Core directed-delivery port, notifications-owned persistence, recipient center/bell/read state, recipient-scoped POST Open to the safe stored internal target, 25-row center pagination, and retain-for-now policy without a cleanup command; plus the three implemented ministry, studies, and Community Activities producers. The current limited-trial V0 runtime scope is closed through `NOTIFY.1F`. Docs-only `NOTIFY.1G-0A` closes the exact Direct Worship Team Change Notification Contract, while `NOTIFY.1G` runtime remains unimplemented and separately approvable. This is not browser automation or a broad production-readiness claim. |
| Sunday Ministry Scheduling | [`SUNDAY_MINISTRY_SCHEDULING_PLAN.md`](SUNDAY_MINISTRY_SCHEDULING_PLAN.md) | Current MO-S.6 plan through implemented Board/context, Campus, Worship pool, event-planner, governed exact-event selection, operational reachability, and rotation-planner preview plus optimistic batch confirmation/shared audit; `NOTIFY.1G-0A` direct-change notification contract is docs complete, while notification runtime, workbook, and importer work remain separately scoped. |
| Worship Rotation Governance | [`WORSHIP_ROTATION_GOVERNANCE_PLAN.md`](WORSHIP_ROTATION_GOVERNANCE_PLAN.md) | Canonical multi-campus Worship invariants through implemented exact-event governance, operational reachability, and planner confirmation/shared audit, plus the docs-complete `NOTIFY.1G-0A` direct-change notification contract; `NOTIFY.1G` runtime and import remain separately approved later slices. |
...
- `NOTIFY.1A` implements the registered/default-enabled `notifications` app,
  ...
  `NOTIFY.1G-0A` separately closes the docs-only direct Worship Team change
  contract: exact selector/`1B-B` actual-change triggers, exact active/date-valid
  ...
  target. `NOTIFY.1G` runtime remains unimplemented; no runtime/schema/
  permission/UI/data behavior is added by the contract.

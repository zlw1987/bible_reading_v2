...
- CS-CORE.2C-A is complete: a read-only Bible Study membership-core readiness/shadow audit (`studies/structure_readiness.py` helpers plus the `audit_bible_study_membership_readiness` command). It compares current legacy `Profile.small_group` Bible Study member visibility against the future membership-core candidate and does not switch Bible Study runtime visibility; Bible Study still uses legacy `SmallGroup` meeting generation and `Profile.small_group` member visibility, and a future separately approved milestone may switch only after clean audit results.
...
- CS-CORE.2C-A Bible Study Membership-Core Readiness / Shadow Audit. Completed as a read-only shadow layer only: `studies/structure_readiness.py` comparison helpers (legacy vs membership-core meeting visibility, `same_visible`/`same_hidden`/`would_gain`/`would_lose` classifications with reason codes) plus the read-only `audit_bible_study_membership_readiness` command (`--verbose`, `--limit`, `--include-past`, `--fail-on-drift`; no `--apply`; writes nothing; never prints membership notes). It does not switch Bible Study runtime visibility: meeting generation still resolves to legacy `SmallGroup` rows, member visibility still uses `Profile.small_group`, and legacy `BibleStudySession` is documented out of audit scope as a retirement target. A future separately approved milestone may switch Bible Study visibility only after clean audit results; no schema/migration, UI/CSS, permission, ServiceEvent, or mapping-edit change.
...
- CS-CORE.2C-A is a shadow/readiness audit only, not a Bible Study visibility switch: the `audit_bible_study_membership_readiness` command and `studies/structure_readiness.py` helpers compare current legacy `Profile.small_group` Bible Study member visibility against the future membership-core candidate read-only; Bible Study still uses legacy `SmallGroup` meeting generation and `Profile.small_group` member visibility, and any future switch is a separate, separately approved milestone gated on clean audit results.

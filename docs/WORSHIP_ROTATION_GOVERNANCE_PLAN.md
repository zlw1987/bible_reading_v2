...
### Effective required-team semantic — runtime implemented / production verified

`MO-S.REQUIRED.0A` froze the effective-required-team contract and superseded
only the earlier statement that a valid selected Worship Team is never required
coverage. `MO-S.REQUIRED.1A` later implemented that contract as a read-only
runtime union without persisting a `ServiceEventRequiredTeam` row. The current
...
| 7A | **Stable ServiceEvent profile identity — IMPLEMENTED historical expansion (`MO-S.6D-PROFILE.1A`), superseded for runtime authority by generic configuration 5C-5F** | Historical transitional optional `ServiceEvent.service_profile_key` field; current technical selector is `ServiceEvent.service_profile` FK to permanent `ServiceProfile.key`; the transitional field was removed by production-applied `events/0013`; grants no audience/permission/serving/recurrence meaning | lexical validation, duplicate profile reuse, ordinary-form exclusion, scheduling-revision advance/rollback, zero cross-domain side effects | No browser QA; no non-Admin surface changed |
...
| 11 | **Projection assignment zero-write preview — IMPLEMENTED / LOCAL VERIFIED (`MO-S.6F.2A`); COUNT CONTRACT SUPERSEDED FOR FUTURE WORK** | Separate Column-E/`projector` adapter and contract; exact active assignable `main.cm.digital.projection`; current local code accepts one or two distinct ASCII-slash tokens and performs exact set-equal no-op review; historical/blank safe skip; staff/superuser only; no confirmation or write path | Existing local grammar/identity/roster/signature/privacy/zero-write coverage remains implementation evidence | Product owner later confirmed generic 1..N complete-roster semantics. Refactor before writer approval; real-workbook/browser/production verification remains outstanding and 2A is not production-ready |
| 12 | **Generic Team Roster Workbook Import architecture — DOCS/READ-ONLY COMPLETE; RUNTIME UNIMPLEMENTED (`MO-S.6F.GENERAL.0A`)** | One team-neutral 1..N roster grammar; adapter auto-match hints are convenience; reviewed exact observed column -> active/assignable team ID+key is authority, followed separately by exact-team membership review/reconciliation; new generic mapping/preview/confirmation contracts; row-preserving safe diff; SQLite CAS/conditional barriers; atomic rollback; privacy-bounded audit; zero Notification | Future parameterized 1/2/3/>3 grammar, column mapping, duplicate-team rejection, person identity, set diff, create/add/remove/protected-state, history/audience, signature, concurrency, rollback, privacy, idempotency, and adapter tests | Unknown columns default Ignore; exact event matching remains unchanged; no generic arbitrary ETL, runtime/schema/dependency/data/production mutation in this slice |

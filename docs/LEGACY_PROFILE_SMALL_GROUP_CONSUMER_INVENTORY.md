...
| Group progress accessible group list | `accounts.permissions.get_accessible_progress_groups()` | `ChurchRoleAssignment` legacy `District` / `SmallGroup` scopes plus the user's own `Profile.small_group` | Which groups a user may select for group progress | Yes. This is tied to legacy role scopes and progress permissions. Still legacy-driven after CS-CORE.4A/4B/4C/4E; CS-CORE.4E added only a shadow comparison and no permission/source switch | High | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A/4B/4C/4E); role-scope decision stays out of the 4-series |
| Group progress roster and default selected group | `reading.views.my_group_progress()` | Defaults to `Profile.small_group`; roster is `User.objects.filter(profile__small_group=selected_group)` | Member rows, enrollments, check-ins, and progress percentages for selected legacy groups | Yes, but do not migrate casually because this affects privacy-visible rosters. Still legacy-driven at runtime after CS-CORE.4A/4B/4C/4E; CS-CORE.4E adds only a comparison-only membership-core shadow (`reading/group_progress_shadow.py`, internal `group_progress_shadow` context, not rendered) with no source/permission switch | High | Planned in `docs/READING_PROGRESS_REFLECTION_PRIVACY_MIGRATION_PLAN.md` (CS-CORE.4A/4B/4C/4E) with privacy invariants and a shadow/switch sequence; CS-CORE.4E is the shadow step |
